...
class ailab_OmniGen:
    ...
    def generation(self, preset_prompt, model_precision, prompt, memory_management, num_inference_steps, guidance_scale,
        ...
        try:
            ...
            # Print VRAM status before pipeline creation
            print(f"Current VRAM usage: {torch.cuda.memory_allocated()/1024**2:.2f}MB") if torch.cuda.is_available() else None
            ...
            # Additional safety checks for position embeddings
            if hasattr(pipe, 'text_encoder') and hasattr(pipe.text_encoder, 'position_embedding'):
                try:
                    import transformers
                    if isinstance(pipe.text_encoder, transformers.models.phi.modeling_phi.PhiModel):
                        print("Checking position embeddings initialization...")
                        if not hasattr(pipe.text_encoder, 'rotary_emb'):
                            print("Warning: Initializing rotary embeddings")
                            pipe.text_encoder._init_rope()
                except ImportError:
                    print("Warning: Could not check transformers model type")

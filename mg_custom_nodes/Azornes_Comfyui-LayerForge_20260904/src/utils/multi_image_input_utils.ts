...
export interface LayerForgeImageInputLink {
    source_id: number;
    source_slot: number;
    source_type?: string;
}

export function getLayerForgeImageInputLinks(node: any): LayerForgeImageInputLink[] {
    const rawLinks = node?.properties?.[LAYERFORGE_IMAGE_LINKS_PROPERTY];
    if (!Array.isArray(rawLinks)) return [];

    const links: LayerForgeImageInputLink[] = [];
    const seen = new Set<string>();

    for (const rawLink of rawLinks) {
        const sourceId = Number(rawLink?.source_id);
        const sourceSlot = Number(rawLink?.source_slot ?? 0);
        if (!Number.isFinite(sourceId) || !Number.isFinite(sourceSlot)) continue;
        ...
        if (links.length >= LAYERFORGE_MAX_IMAGE_INPUTS) break;
    }

    return links;
}

export function setLayerForgeImageInputLinks(node: any, links: LayerForgeImageInputLink[]): void {
    node.properties ||= {};
...
export function addLayerForgeImageInputLink(
    node: any,
    link: LayerForgeImageInputLink,
): boolean {
    ...
    if (!Number.isFinite(sourceId) || links.length >= LAYERFORGE_MAX_IMAGE_INPUTS) return false;
...
export function removeLayerForgeImageInputLink(node: any, index: number): boolean {
    const links = getLayerForgeImageInputLinks(node);
...
export function clearLayerForgeImageInputLinks(node: any): number {
    const links = getLayerForgeImageInputLinks(node);
...
export function getLayerForgeImageInputSlot(node: any): any | null {
    return node?.inputs?.find((input: any) => input?.name === 'input_image')
        ?? node?.inputs?.[0]
        ?? null;
}

export function getLayerForgeMaskInputSlot(node: any): any | null {
    return node?.inputs?.find((input: any) => input?.name === 'input_mask')
        ?? node?.inputs?.[1]
        ?? null;
}

export function hasLayerForgeImageInput(node: any): boolean {

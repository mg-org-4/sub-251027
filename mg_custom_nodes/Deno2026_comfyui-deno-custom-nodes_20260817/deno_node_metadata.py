...
NODE_INPUT_TOOLTIPS = {
    ...
    "DenoIdeogramDirector": {
        ...
        "input_width": "Optional connected output width. Snaps to a multiple of 16 and wins when width and height are both connected.",
        "input_height": "Optional connected output height. Snaps to a multiple of 16 and wins when width and height are both connected.",
        "input_megapixels": "Optional connected megapixel budget. Used with the current aspect ratio when width and height are not both connected.",
    },
    "DenoLocalLLMRefiner": {
        "provider": "Choose the local provider to call: Ollama, LM Studio, llama.cpp, vLLM, Custom, llama-swap, or Unsloth. Select Unsloth only for an Unsloth Studio server; select LM Studio when an Unsloth GGUF is running inside LM Studio.",
        ...
        "custom_server_url": "Local OpenAI-compatible server URL used by llama.cpp, vLLM, Custom, llama-swap, or Unsloth Studio. Unsloth defaults to http://127.0.0.1:8888/v1 and requires DENO_LOCAL_LLM_UNSLOTH_API_KEY; the key is never stored in the workflow or PNG metadata.",
        "custom_model": "Model name to call on the selected local OpenAI-compatible server.",
        "system_prompt": "Optional instructions that steer how the local LLM rewrites or reviews the prompt.",
        "thinking": "Ask for model thinking/reasoning when the selected local model supports it. If LM Studio rejects its optional reasoning-control field before output starts, DENO retries once without that field; the server then decides its default reasoning behavior.",
        ...
        "keep_minutes": "Requested keep duration where the provider supports a TTL. For Unsloth it tracks warm-state/swap decisions but does not schedule a timed unload; use Unload after run or Unload LLM. llama-swap uses its own server-side unload timeout.",

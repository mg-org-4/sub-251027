...
const SAVE_RESTORE_REV = "multi-lora-save-reload-v1";
const MARGIN = 10;
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.configure = function (info) {
            const savedValues = captureMultiLoraSerializedWidgetValues(info);
            if (savedValues) {
                this.__denoMultiLoraConfiguredWidgetValues = savedValues;
            }
            const result = configure?.apply(this, arguments);
            if (savedValues) {
                applyMultiLoraSerializedValuesToWidgets(this, savedValues);
                syncMultiLoraSerializedWidgetValues(this);
                this.properties = this.properties || {};
                this.properties.__deno_multi_lora_save_restore = SAVE_RESTORE_REV;
            }
            return result;
        };
        ...
        nodeType.prototype.onConfigure = function () {
            ...
            queueMicrotask(() => {
                ...
                if (this.__denoMultiLoraConfiguredWidgetValues) {
                    ...
                    syncMultiLoraSerializedWidgetValues(this);
                    rebuildUi(this);
                    delete this.__denoMultiLoraConfiguredWidgetValues;
                }
            });
            return result;
        };
...
function multiLoraSerializedWidgetNames() {
    const names = ["active_loras"];
    for (let index = 1; index <= MAX_SLOTS; index += 1) {
        names.push(`enabled_${index}`, `lora_${index}`, `model_strength_${index}`, `clip_strength_${index}`);
    }
    for (let index = 1; index <= MAX_SLOTS; index += 1) {
...
function multiLoraLegacySerializedWidgetNames() {
    const names = ["active_loras"];
    for (let index = 1; index <= MAX_SLOTS; index += 1) {
        names.push(`enabled_${index}`, `lora_${index}`, `model_strength_${index}`, `clip_strength_${index}`);
    }
...
function captureMultiLoraSerializedWidgetValues(info) {
    const values = info?.widgets_values;
    if (!Array.isArray(values)) {
        return null;
    }
    const names = multiLoraSerializedWidgetNames();
    if (values.length >= names.length) {
        return Object.fromEntries(names.map((name, index) => [name, values[index]]));
    }
    const legacyNames = multiLoraLegacySerializedWidgetNames();
...
function applyMultiLoraSerializedValuesToWidgets(node, savedValues) {
    if (!node || !savedValues) {
        return;
    }
    for (const name of multiLoraSerializedWidgetNames()) {
...
function syncMultiLoraSerializedWidgetValues(node) {
    if (!node) {
        return;
    }
    const values = multiLoraSerializedWidgetNames().map((name) => getValue(node, name, defaultSerializedValue(name)));
...
if (typeof window !== "undefined" && typeof window.__DENO_MULTI_LORA_TEST_HOOK__ === "function") {
    window.__DENO_MULTI_LORA_TEST_HOOK__({
        coerceBooleanValue,
        normalizeBool,
    });
}

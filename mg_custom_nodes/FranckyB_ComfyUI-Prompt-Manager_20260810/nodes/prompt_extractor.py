...
def _resolve_source_base_dir(source):
    if source == 'output':
...
def _resolve_media_path(filename, source='input', allow_temp_fallback=False):
    """Resolve a relative-or-absolute media path and return absolute path or None."""
    file_value = (filename or '').strip()
    if not file_value:
        return None

    # Absolute selections from browse-anywhere mode.
    if os.path.isabs(file_value):
        abs_path = _safe_abspath(file_value)
        return abs_path if os.path.exists(abs_path) else None

    base_dir = _resolve_source_base_dir(source)
    candidate = os.path.join(base_dir, file_value.replace('/', os.sep))
    if os.path.exists(candidate) and _is_within(base_dir, candidate):
        return os.path.realpath(candidate)

    if allow_temp_fallback:
        temp_dir = folder_paths.get_temp_directory()
        candidate = os.path.join(temp_dir, file_value.replace('/', os.sep))
        if os.path.exists(candidate) and _is_within(temp_dir, candidate):
            return os.path.realpath(candidate)
...
def _probe_media_meta(file_path, ext):
...
async def extract_video_metadata_api(request):
    ...
    try:
        ...
        file_path = _resolve_media_path(filename, source=source, allow_temp_fallback=False)
...
@server.PromptServer.instance.routes.get("/prompt-extractor/path-browser/list")
async def prompt_extractor_path_browser_list(request):
    """Read-only directory browser for media files."""
    try:
        path = request.query.get('path', '').strip()
        kind = request.query.get('kind', 'media')
        allowed = _exts_for_kind(kind)

        roots = _default_browser_roots()

        if not path:
            return server.web.json_response({"ok": True, "mode": "roots", "roots": roots})
...
@server.PromptServer.instance.routes.get("/prompt-extractor/path-browser/file")
async def prompt_extractor_path_browser_file(request):
    """Serve a media file by absolute path (extension-guarded)."""
    try:
        path = request.query.get('path', '').strip()
        if not path:
            return server.web.json_response({"ok": False, "error": "Missing path"}, status=400)

        real_path = os.path.realpath(_safe_abspath(path))
        if not os.path.isfile(real_path):
            return server.web.json_response({"ok": False, "error": "File not found"}, status=404)

        ext = os.path.splitext(real_path)[1].lower()
        if ext not in ALL_MEDIA_EXTS:
            return server.web.json_response({"ok": False, "error": "Unsupported file type"}, status=403)

        return server.web.FileResponse(real_path, headers={"Cache-Control": "no-cache"})
    except Exception as e:
        return server.web.json_response({"ok": False, "error": str(e)}, status=500)
...
async def list_input_files(request):
    ...
    try:
        ...
        include_meta = request.rel_url.query.get('include_meta', '0') in ('1', 'true', 'yes')
        if source == 'output':
        ...
        files = []
        allowed_extensions = _exts_for_kind(kind)
        # Legacy behavior: media listings also include json files used by PromptExtractor.
        if kind in ('media', 'all'):
            allowed_extensions = set(allowed_extensions) | {'.json'}
        ...
        if include_meta:
            files.sort(key=lambda f: str(f.get('path', '')).lower())
        else:
            files.sort()

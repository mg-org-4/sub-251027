"""
High-level network blocks for MatAnyone2.
"""
...
class PixelEncoder(nn.Module):
    """Pixel/feature encoder (ResNet backbone)."""
    ...
    def forward(self, x: torch.Tensor, seq_length=None) -> tuple:
        f1 = x
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
class KeyProjection(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *, need_s: bool, need_e: bool) -> tuple:
...
class MaskEncoder(nn.Module):
    """Mask encoder with ResNet backbone."""
...
class PixelFeatureFuser(nn.Module):
    """Fuses pixel and sensory memory features."""
...
class MaskDecoder(nn.Module):
    """Mask decoder with multi-scale upsample."""

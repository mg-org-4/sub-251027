"""
Positional encoding for MatAnyone2 transformer.
"""
...
def get_emb(sin_inp: torch.Tensor) -> torch.Tensor:
    """Gets base embedding for one dimension with sin and cos intertwined."""
    emb = torch.stack((sin_inp.sin(), sin_inp.cos()), dim=-1)
    return torch.flatten(emb, -2, -1)


class PositionalEncoding(nn.Module):
    """Sinusoidal positional encoding for 2D features."""
    ...
    def forward(self, tensor: torch.Tensor) -> torch.Tensor:
        if len(tensor.shape) not in (4, 5):
            raise RuntimeError(
                "The input tensor has to be 4/5d, got %s!" % tensor.shape
            )

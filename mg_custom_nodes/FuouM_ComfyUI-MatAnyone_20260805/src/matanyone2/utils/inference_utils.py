"""
Inference utilities for MatAnyone2 (erosion, dilation, frame loading).
"""
...
def read_frame_from_videos(frame_root):
    """Reads frames from video file or image folder. Returns (frames, fps, length, name)."""
...
def get_video_paths(input_root):
    """Collects video file paths under input_root."""
    video_paths = []
...
def str_to_list(value):
    """Parses comma-separated string to list of ints."""
    return list(map(int, value.split(",")))


def gen_dilate(alpha, min_kernel_size, max_kernel_size):
    """Dilates alpha mask with random kernel size."""
...
def gen_erosion(alpha, min_kernel_size, max_kernel_size):
    """Erodes alpha mask with random kernel size."""
    kernel_size = random.randint(min_kernel_size, max_kernel_size)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))

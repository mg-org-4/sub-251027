"""
Tensor utilities for MatAnyone2.
"""
...
from .device import safe_autocast


def pad_divide_by(in_img: torch.Tensor, d: int) -> tuple[torch.Tensor, Iterable[int]]:
    """Pads image so dimensions are divisible by d."""
    h, w = in_img.shape[-2:]
...
def aggregate(prob: torch.Tensor, dim: int) -> torch.Tensor:
    """Aggregates probability tensor along dimension."""

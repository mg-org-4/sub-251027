...
// Global registry to track all IAMCCS_WDC_LTXKeyframer nodes across all subgraphs
window._IAMCCS_WDC_LTXKeyframerGlobalNodes = window._IAMCCS_WDC_LTXKeyframerGlobalNodes || new Set();
...
// Finds all other IAMCCS_WDC_LTXKeyframer nodes globally and mirrors the value to them
function syncWidgetAcrossNodes(sourceNode, widgetName, value) {
    if (!window._IAMCCS_WDC_LTXKeyframerGlobalNodes) return;
    
    for (const targetNode of window._IAMCCS_WDC_LTXKeyframerGlobalNodes) {
        // Target all OTHER IAMCCS_WDC_LTXKeyframer nodes by direct object reference
...
app.registerExtension({
    name: "Comfy.IAMCCS_WDC_LTXKeyframer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "IAMCCS_WDC_LTXKeyframer") return;

        // Register this node instance globally
        window._IAMCCS_WDC_LTXKeyframerGlobalNodes.add(node);
        ...
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "IAMCCS_WDC_MultiImageLoader") {
                    return originNode;
                }
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode && innerNode.comfyClass === "IAMCCS_WDC_MultiImageLoader") {
                            return innerNode;
                        }
                    } catch (e) {
            ...
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "IAMCCS_WDC_MultiImageLoader") {
        ...
        node.onRemoved = function() {
            ...
            window._IAMCCS_WDC_LTXKeyframerGlobalNodes.delete(node);

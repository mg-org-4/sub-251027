...
def _find_timeline(cine_linx: Dict[str, Any], explicit_timeline_data: Any = "") -> Dict[str, Any]:
    ...
    for value in (
        resources.get("cine_backend_timeline_data_json"),
        outputs.get("backend_timeline_data"),
        payload.get("backend_timeline_data"),
        payload.get("timeline_data"),
        resources.get("cine_v4_timeline_data_json"),
        outputs.get("timeline_data"),
    ):
        candidate = _json_loads(value, None)
...
def _derive_prompt_parts(timeline: Dict[str, Any], local_prompts: str, segment_lengths: str) -> Tuple[str, str, str]:
    ...
    lengths = []
    strengths = []
    for seg in _segments_from_timeline(timeline, "segments"):
        ...
        if prompt:
            ...
            lengths.append(str(max(1, _safe_int(seg.get("length", 1), 1))))
...
def _build_guide_data(
    ...
    segments = [
        ...
        and (_segment_media_path(seg, prefer_video=str(seg.get("type", "")).lower() == "video") or str(seg.get("imageB64", seg.get("image_b64", "")) or ""))
    ...
    for index, raw_seg in enumerate(segments):
        ...
        strength = strengths[index] if index < len(strengths) else _safe_float(seg.get("guideStrength", seg.get("guide_strength", seg.get("strength", seg.get("force", 1.0)))), 1.0)
    ...
    if not guide_data["images"] and optional_latent is None:
        ...
        retake_video = timeline.get("retakeVideo") if isinstance(timeline.get("retakeVideo"), dict) else {}
        if _safe_bool(timeline.get("retakeMode", False), False) and retake_video:
...
def _audio_segments_for_mode(timeline: Dict[str, Any], override_audio: bool, duration_frames: int) -> Tuple[List[Dict[str, Any]], str]:
    if _safe_bool(timeline.get("retakeMode", False), False) and isinstance(timeline.get("retakeVideo"), dict):
        retake = timeline.get("retakeVideo") or {}
...
def _encode_audio_latent(audio_vae: Any, audio_out: Dict[str, Any], timeline: Dict[str, Any], start_frame: int, duration_frames: int, frame_rate: float, inpaint_audio: bool, override_audio: bool, active_custom_audio: bool) -> Dict[str, Any]:
    ...
    try:
        ...
        if _safe_bool(timeline.get("retakeMode", False), False):
            mask = torch.zeros((batch, frames, height), dtype=torch.float32, device=latent_samples.device)
            retake_start = float(timeline.get("retakeStart", 0) or 0)
            retake_len = float(timeline.get("retakeLength", 0) or 0)
...
class IAMCCS_CineShotboardV4Backend:
    ...
    def execute(
        ...
        fps = float(frame_rate) if float(frame_rate or 0.0) > 0 else _safe_float(timeline.get("frame_rate", resources.get("cine_frame_rate", outputs.get("frame_rate", payload.get("frame_rate", 24)))), 24.0)
        fps = max(1.0, fps)
        start = int(start_frame or timeline.get("normalStartFrame", payload.get("start_frame", 0)) or 0)
        raw_duration = int(duration_frames or timeline.get("normalDurationFrames", timeline.get("duration_frames", payload.get("duration_frames", outputs.get("max_frames", resources.get("cine_max_frames", 0))))) or 0)
        if raw_duration <= 0:
        ...
        prompt = str(global_prompt or timeline.get("global_prompt", resources.get("cine_global_prompt", outputs.get("global_prompt", payload.get("global_prompt", "")))) or "")
        if _safe_bool(timeline.get("retakeMode", False), False) and not prompt:
            prompt = str(timeline.get("retake_global_prompt", "") or "")
        ...
        local_prompts, segment_lengths, guide_strength = _derive_prompt_parts(timeline, local_prompts, segment_lengths)
        segment_lengths = _pad_promptrelay_lengths_to_ltx(segment_lengths, raw_duration, ltxv_length)

        width = int(custom_width or timeline.get("custom_width", resources.get("cine_image_width", payload.get("image_width", 0))) or 0)
        height = int(custom_height or timeline.get("custom_height", resources.get("cine_image_height", payload.get("image_height", 0))) or 0)
        method = _normalize_resize_method(resize_method or timeline.get("resize_method", resources.get("cine_image_resize_method", payload.get("image_resize_method", "crop"))))
        divisor = max(1, _safe_int(divisible_by or resources.get("cine_image_multiple_of", payload.get("image_multiple_of", 32)), 32))
        compression = max(0, _safe_int(img_compression or resources.get("cine_img_compression", payload.get("img_compression", 0)), 0))
        ...
        effective_custom_audio = bool(use_custom_audio or _safe_bool(timeline.get("use_custom_audio", False), False) or timeline_has_audio or effective_override_audio or _safe_bool(timeline.get("retakeMode", False), False))
        ...
        audio_out = _build_combined_audio(timeline, start, raw_duration, fps, effective_override_audio)
        audio_for_latent = _build_combined_audio(timeline, start, ltxv_length, fps, effective_override_audio)
...
class IAMCCS_CineShotboardV4Guide:
    ...
    def execute(
        ...
        retake_active = bool(retake_mode) or _safe_bool(timeline.get("retakeMode", False), False)

        if retake_active:
            target_width = int(latent_width * 32)
            target_height = int(latent_height * 32)
            retake_start = _safe_int(timeline.get("retakeStart", 0), 0)
            retake_len = _safe_int(timeline.get("retakeLength", 0), 0)
            retake_strength = _safe_float(timeline.get("retakeStrength", 1.0), 1.0)

...
function nodePayload(session) {
    ...
    const duration = Math.max(.01, numberValue(root, "v2vfDuration", Number(read(node, "duration_seconds", 5.0625))));
    const fps = Math.max(1, numberValue(root, "v2vfFps", Number(read(node, "fps", 16))));
    ...
    const overlap = Math.min(chunk - 4, frame41(numberValue(root, "v2vfOverlap", 5), 1));
    const identity = session.identityMode;
    return {
        ...
        generation_width: Math.max(64, Math.floor(numberValue(root, "v2vfWidth", 512))),
        generation_height: Math.max(64, Math.floor(numberValue(root, "v2vfHeight", 896))),
        scail_identity_mode: identity,
        show_reference_background: Boolean(field(root, "v2vfShowBackground")?.checked),
        interpolate_to_32fps: Boolean(field(root, "v2vfInterpolate32")?.checked),
        chunk_frames: chunk,
        ...
        generation_seed: Math.max(0, Math.floor(numberValue(root, "v2vfSeed", 123))),
        ...
        output_prefix: String(field(root, "v2vfPrefix")?.value || "IAMCCS/SCAIL2_EASY_SHOTBOARD"),
...
function commit(session) {
    ...
    const timeline = {
        ...
        backend_mode: "scail2",
        backend_profile: payload.scail_identity_mode === "multi_person_identity" ? "scail2_multi_person_identity" : "scail2_single_person",
        chunk_length_frames: payload.chunk_frames,
        chunk_overlap_frames: payload.overlap_frames,
        show_reference_background: payload.show_reference_background,
        interpolate_to_32fps: payload.interpolate_to_32fps,
    };
...
function syncBackend(session, payload) {
    ...
    let fpsTargets = 0;
    const videoNodes = nodesByType(session.node, ["VHS_LoadVideo", "VHS_LoadVideoPath"]);
    for (const target of videoNodes.slice(0, 2)) {
        touched += Number(writeAny(target, ["video"], payload.source_video_path));
        touched += Number(writeAny(target, ["frame_load_cap"], payload.frame_load_cap));
        touched += Number(writeAny(target, ["skip_first_frames"], Math.max(0, Math.round(payload.trim_start_s * payload.fps))));
        touched += Number(writeAny(target, ["force_rate"], payload.fps));
    }
    ...
    for (const target of nodesByType(session.node, ["IAMCCS_ScailExtends"])) {
        ...
        touched += Number(writeAny(target, ["overlap"], payload.overlap_frames));
    ...
    for (const target of graphNodes().filter((item) => nodeClass(item) === "PrimitiveNode" && String(item.title || "").trim() === "replacement_mode")) {
        ...
        if (primitive) {
            ...
            try { primitive.callback?.(primitive.value, null, target); } catch {}
            target.setDirtyCanvas?.(true, true);
            touched++;
        }
    }
    for (const target of graphNodes()) {
        ...
        if (type === "VHS_VideoCombine") {
            ...
            let outputFps = null;
            if (title.includes("final output 32 fps")) {
                suffix = `_${Number((payload.fps * 2).toFixed(3))}FPS`;
                outputFps = payload.fps * 2;
            } else if (title.includes("output 16 fps")) {
                suffix = `_${Number(payload.fps.toFixed(3))}FPS`;
                outputFps = payload.fps;
            } else if (title.includes("pose video mask preview")) {
                suffix = "_POSE_MASK_PREVIEW";
                outputFps = payload.fps;
            }
            if (outputFps !== null && writeAny(target, ["frame_rate"], outputFps)) {
                fpsTargets++;
                touched++;
            }
            if (suffix && writeAny(target, ["filename_prefix"], `${payload.output_prefix}${suffix}`)) {
                outputPrefixTargets++;
                touched++;
            }
        }
    }
    session.backendSync = { positivePromptTargets, negativePromptTargets, outputPrefixTargets, fpsTargets, touched };
    return touched;
}

function auditBackendTruth(session, payload) {
    const issues = [];
    const videoNodes = nodesByType(session.node, ["VHS_LoadVideo", "VHS_LoadVideoPath"]).slice(0, 2);
    const skipFrames = Math.max(0, Math.round(payload.trim_start_s * payload.fps));
    if (!videoNodes.length) issues.push("source video loader missing");
    for (const [index, target] of videoNodes.entries()) {
        expectWidget(issues, target, ["video"], payload.source_video_path, `video loader ${index + 1} path`);
        expectWidget(issues, target, ["frame_load_cap"], payload.frame_load_cap, `video loader ${index + 1} frame cap`);
        expectWidget(issues, target, ["skip_first_frames"], skipFrames, `video loader ${index + 1} trim in`);
        expectWidget(issues, target, ["force_rate"], payload.fps, `video loader ${index + 1} fps`);
    }
    ...
    const bus = nodesByType(session.node, ["IAMCCS_V2VBusToScail"])[0];
    const router = nodesByType(session.node, ["IAMCCS_CineInfoV2VBackendRouter"])[0];
    if (!router) {
        issues.push("V2V backend router missing");
    } else if (!linkedFrom(router, "cine_linx", NODE_CLASS)) {
        issues.push("Shotboard to backend router link missing");
    }
    if (!bus) {
        issues.push("SCAIL UI bus missing");
    } else {
        if (!linkedFrom(bus, "media_bus", "IAMCCS_CineInfoV2VBackendRouter")) issues.push("router media bus to SCAIL link missing");
        if (!linkedFrom(bus, "control_bus", "IAMCCS_CineInfoV2VBackendRouter")) issues.push("router control bus to SCAIL link missing");
    }

    const scailNodes = nodesByType(session.node, ["IAMCCS_ScailExtends"]);
    if (!scailNodes.length) issues.push("SCAIL generation node missing");
    for (const target of scailNodes) {
        if (!linkedFrom(target, "width", "IAMCCS_V2VBusToScail")) issues.push("generation width bus link missing");
        if (!linkedFrom(target, "height", "IAMCCS_V2VBusToScail")) issues.push("generation height bus link missing");
        ...
        expectWidget(issues, target, ["overlap"], payload.overlap_frames, "overlap frames");
    ...
    if (!negative.length || negative.some((target) => !linkedFrom(target, "text", "IAMCCS_V2VBusToScail"))) issues.push("negative prompt bus link missing");
    ...
    for (const target of combines) {
        ...
        else if (title.includes("output 16 fps") || title.includes("pose video mask preview")) expectWidget(issues, target, ["frame_rate"], payload.fps, "backend output fps");
    }
...
function timelineDuration(session) {
    return Math.max(.01, numberValue(session.root, "v2vfDuration", 5.0625));
}
...
function play(session, monitor = "program") {
    ...
    const fps = Math.max(1, numberValue(session.root, "v2vfFps", 16));
...
function controlMarkup(node) {
    return `
        ...
        <div class="iamccs-v2vf-form">
            <label>WIDTH<input id="v2vfWidth" type="number" min="64" step="8" value="${read(node, "generation_width", 512)}"></label>
            <label>HEIGHT<input id="v2vfHeight" type="number" min="64" step="8" value="${read(node, "generation_height", 896)}"></label>
            <label>FPS<input id="v2vfFps" type="number" min="1" step="0.01" value="${read(node, "fps", 16)}"></label>
            ...
            <label>OVERLAP (4N+1)<input id="v2vfOverlap" type="number" min="1" step="4" value="${read(node, "overlap_frames", 5)}"></label>
            ...
            <label>SEED<input id="v2vfSeed" type="number" min="0" value="${read(node, "generation_seed", 123)}"></label>
            ...
            <label class="iamccs-v2vf-toggle"><input id="v2vfShowBackground" type="checkbox" ${read(node, "show_reference_background", true) ? "checked" : ""}><span>SHOW REFERENCE BACKGROUND</span></label>
            <label class="iamccs-v2vf-toggle"><input id="v2vfInterpolate32" type="checkbox" ${read(node, "interpolate_to_32fps", true) ? "checked" : ""}><span>FINAL OUTPUT x2 FPS (FILM)</span></label>
            ...
            <label class="wide">OUTPUT PREFIX<input id="v2vfPrefix" value="${read(node, "output_prefix", "IAMCCS/SCAIL2_EASY_SHOTBOARD")}"></label>
            <div class="iamccs-v2vf-models wide"><div class="iamccs-v2vf-kicker">LIVE BACKEND MODEL WIDGETS</div><div id="v2vfModels"></div></div>
        </div>
        <div id="v2vfBackendNotice" class="iamccs-v2vf-notice">Inspecting the SCAIL graph...</div>
...
function openShotboard(node) {
    ...
    root.innerHTML = `
        <header class="iamccs-v2vf-header">
            <div class="iamccs-v2vf-brand"><div class="iamccs-v2vf-logo"><span>IAM</span><span>CCS</span></div><div><b>V2V SHOTBOARD EASY</b><small>SCAIL-2 EDITION / COMFYUI NODE</small></div></div>
            <div class="iamccs-v2vf-modebar"><button type="button" data-mode="single_person">SCAIL-2 SINGLE</button><button type="button" data-mode="multi_person_identity">SCAIL-2 MULTI</button></div>
        ...
        <main class="iamccs-v2vf-shell">
            <aside id="v2vfLeft" class="iamccs-v2vf-column"><div class="iamccs-v2vf-section-head"><div><div class="iamccs-v2vf-kicker">01 / MEDIA DECK</div><strong>Inputs & output</strong></div><span>SCAIL</span></div></aside>
            <section class="iamccs-v2vf-center">
                <div class="iamccs-v2vf-monitors">
                    <div class="iamccs-v2vf-section-head"><div><div class="iamccs-v2vf-kicker">02 / SHOTBOARD VIEW</div><strong>SCAIL-2 source and program</strong></div><span>LIVE</span></div>
                    <div class="iamccs-v2vf-monitor-grid">
                        ...
                        <article id="v2vfProgramMonitor" class="iamccs-v2vf-monitor"><div class="iamccs-v2vf-monitor-head"><span>PROGRAM</span><span id="v2vfProgramLabel">SELECTED: SOURCE</span><button id="v2vfFull" class="iamccs-v2vf-full" type="button">FULL</button></div><div id="v2vfProgram" class="iamccs-v2vf-media"></div><div class="iamccs-v2vf-transport"><button data-program-step="-1">STEP -</button><button data-program-play>PLAY</button><button data-program-stop>STOP</button><button data-program-step="1">STEP +</button></div><div class="iamccs-v2vf-monitor-foot"><span>SCAIL-2</span><span id="v2vfProgramTime">00:00.000</span></div></article>
                    </div>
                </div>
    ...
    const session = {
        node, root,
        identityMode: String(read(node, "scail_identity_mode", "single_person")),
        trimIn: Number(read(node, "trim_start_s", 0)),
        trimOut: Number(read(node, "trim_end_s", read(node, "duration_seconds", 5.0625))),
    ...
    hiddenDuration.value = String(read(node, "duration_seconds", 5.0625));
    root.appendChild(hiddenDuration);

    const lanes = field(root, "v2vfLanes");
    lanes.append(lane("SOURCE", "VIDEO + AUDIO", "source", true), lane("POSE", "DWPose / control", "pose"), lane("MASK", "SAM 3.1 / identity", "mask"), lane("INTERMEDIATE", "tracking / generation", "intermediate"), lane("OUTPUT", "final SCAIL video", "output"));
    ...
    for (const laneId of ["pose", "mask", "intermediate", "output"]) renderStrip(session, laneId, null);
    renderProgram(session);
        ...
        try {
            ...
            if (kind === "video") {
                ...
                if (probe) probe.addEventListener("loadedmetadata", () => {
                    hiddenDuration.value = String(Math.max(.01, Number(probe.duration || 5.0625)));
            ...
            commit(session);
            setProgress(session, "IDLE", 0, 1, `${file.name} ready`);
        } catch (error) {
    ...
    $$(root, "[data-apply]").forEach((button) => button.addEventListener("click", () => {
        ...
        commit(session); renderProgram(session);
    }));
    ...
    function dragTrim(which, event) {
        ...
        const move = (moveEvent) => {
            ...
            const value = Math.max(0, Math.min(numberValue(root, "v2vfDuration", 5.0625), (moveEvent.clientX - rect.left) / rect.width * numberValue(root, "v2vfDuration", 5.0625)));
    ...
    field(root, "v2vfFit").addEventListener("click", () => { session.trimIn = 0; session.trimOut = numberValue(root, "v2vfDuration", 5.0625); session.playhead = 0; renderTimeline(session); });
    ...
    backendSummary(session);
    renderTimeline(session);
    setProgress(session, "IDLE", 0, 1, "Ready. Load media and render the existing SCAIL workflow.");
    void (async () => {
        await hydrateLatestOutputs(session);
...
function configureNode(node) {
    if (node._iamccsV2VFreeReady) return;
    node._iamccsV2VFreeReady = true;
    ...
    const info = node.addWidget("text", "edition", "EASY / SCAIL-2", () => {});
    info.disabled = true;
    node.size = [330, 112];

...
class IAMCCS_MiniMaxH3MotionContext:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "freeze_safe_handover": ("BOOLEAN", {"default": True}),
                "freeze_safety_margin_frames": ("INT", {"default": 3, "min": 0, "max": 24, "step": 1}),
            }
        }
    ...
    def configure(self, enabled=False, context_frames="22", continue_audio=True, freeze_safe_handover=True, freeze_safety_margin_frames=3):
        ...
        config = {
            ...
            "freeze_safe_handover": bool(freeze_safe_handover),
            "freeze_safety_margin_frames": max(0, int(freeze_safety_margin_frames)),
        }
        report = (
            f"H3 motion context | {'on' if config['enabled'] else 'off'} | {frames}f | "
            f"audio={'on' if config['continue_audio'] else 'off'} | "
            f"freeze_safe_handover={'on' if config['freeze_safe_handover'] else 'off'}"
        )
...
def _cache_path(render_id: str, segment_index: int) -> Path | None:
    ...
    if not safe or int(segment_index) < 0:
        return None
    return Path(folder_paths.get_output_directory()) / "minimax_h3_shotboard" / "motion_context" / f"{safe}_seg_{int(segment_index):04d}.pt"


def _rgb_frames(images: torch.Tensor, max_edge: int = 192) -> torch.Tensor:
    """Downsample decoded IMAGE frames to [T,3,H,W] for cheap motion analysis."""
    # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
    x = images[..., :3].detach().to(dtype=torch.float32).movedim(-1, 1)
...
def detect_motion_freeze_start(
    images: torch.Tensor,
    *,
    reference_frames: int = 15,
    safety_margin_frames: int = 3,
    analysis_window: int = 96,
    final_mean_diff_threshold: float = 0.012,
    transition_mean_diff_threshold: float = 0.002,
    min_final_match_ratio: float = 0.75,
    min_static_transition_ratio: float = 0.70,
) -> int | None:
    """Find where H3 has already converged onto a static held final pose.

    FL2VA-style tasks routinely reach the requested last keyframe early and
    then hold that pose for the remaining tail of the segment. Pinning that
    frozen tail as "motion context" gives the next chunk no real trajectory to
    continue, so generation is free to invent an unrelated composition right
    after the pin. Returns a safe cutoff frame index (exclusive) that a
    continuity context window must not cross, or ``None`` when no sustained
    freeze is detected within the analysis window (native motion throughout).
    """
    # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
    total = int(images.shape[0])
    reference_frames = max(1, int(reference_frames))
    if total < max(8, reference_frames + 2):
        return None
    window_start = max(0, total - max(reference_frames + 2, int(analysis_window)))
    rgb = _rgb_frames(images[window_start:])
    reference = rgb[-min(reference_frames, rgb.shape[0]):].median(dim=0, keepdim=True).values
    final_diff = (rgb - reference).abs().mean(dim=(1, 2, 3))
    transitions = (rgb[1:] - rgb[:-1]).abs().mean(dim=(1, 2, 3))
    is_final_match = (final_diff <= final_mean_diff_threshold).tolist()
    is_static = (transitions <= transition_mean_diff_threshold).tolist()

    n = len(is_final_match)
    freeze_start_local = None
    for start in range(n - 1, -1, -1):
        final_tail = is_final_match[start:]
        static_tail = is_static[start:] if start < len(is_static) else []
        final_ratio = sum(final_tail) / float(len(final_tail))
        static_ratio = (sum(static_tail) / float(len(static_tail))) if static_tail else 1.0
        if not is_final_match[start] or final_ratio < min_final_match_ratio or static_ratio < min_static_transition_ratio:
            break
        freeze_start_local = start
    if freeze_start_local is None:
        return None

    freeze_start = window_start + freeze_start_local
    safe_cutoff = freeze_start - max(0, int(safety_margin_frames))
    if safe_cutoff <= 0:
        return None
    return int(safe_cutoff)
...
def save_sampled_av(
    ...
    export_start_frames: int,
    export_frames: int,
    sample_frames: int,
    motion_safe_end_frames: int | None = None,
) -> bool:
    ...
    payload = {
        ...
        "motion_safe_end_frames": int(motion_safe_end_frames) if motion_safe_end_frames is not None else None,
    }
...
def load_sampled_av(render_id: str, segment_index: int) -> dict[str, Any] | None:
    # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
    path = _cache_path(render_id, segment_index)
    ...
    if (
        ...
        or not torch.is_tensor(payload.get("video"))
        or not torch.is_tensor(payload.get("audio"))
    ):
        return None
    return payload
...
def _phase_aligned_tail(
    ...
    """Choose a full-grid prior AV window ending at or before a motion-safe cutoff.

    ``motion_safe_end_frames`` (when available) caps the window before any
    detected FL2VA freeze/hold tail, so the pinned frames carry real motion
    instead of a static held pose.
    """
    # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
    ...
    if motion_safe_end_frames is not None:
        export_end = min(export_end, max(0, int(motion_safe_end_frames)))
    best_start = None
    best_end = -1
...
def apply_context(
    ...
    continue_audio: bool,
    sample_frames: int,
    freeze_safe_handover: bool = True,
) -> tuple[Any, str]:
    ...
    motion_safe_end_frames = previous.get("motion_safe_end_frames") if freeze_safe_handover else None
    ...
    freeze_note = "n/a" if motion_safe_end_frames is None else f"{motion_safe_end_frames}f"
    return result, (
        f"pinned={context}f video=sampled_av steps={len(keyframes)} "
        f"audio={'40t' if refs else 'off'} pin_end={pin_end} "
        f"phase_tail_trim={trim_previous_export}f freeze_safe_cutoff={freeze_note}"
    )

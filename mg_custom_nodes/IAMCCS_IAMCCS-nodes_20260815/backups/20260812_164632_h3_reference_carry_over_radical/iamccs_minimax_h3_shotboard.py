...
class IAMCCS_MiniMaxH3NativeCheckpointSave:
    ...
    def checkpoint(
        ...
        if isinstance(motion_context, dict) and bool(motion_context.get("enabled")) and isinstance(sampled_latent, dict):
            try:
                from .iamccs_minimax_h3_continuity import detect_motion_freeze_start, save_sampled_av
                motion_safe_end_frames = None
                if bool(motion_context.get("freeze_safe_handover", True)):
                    try:
                        motion_safe_end_frames = detect_motion_freeze_start(
                            images,
                            safety_margin_frames=int(motion_context.get("freeze_safety_margin_frames", 3)),
                        )
                    except Exception as exc:
                        LOG.warning("MiniMax H3 freeze-safe handover detection skipped: %s", exc)
                motion_saved = save_sampled_av(
                    ...
                    int(images.shape[0]),
                    int(motion_state.get("sample_frames", images.shape[0])) if isinstance(motion_state, dict) else int(images.shape[0]),
                    motion_safe_end_frames,
                )
                LOG.info(
                    "MiniMax H3 freeze-safe handover | render=%s segment=%d motion_safe_end_frames=%s",
                    active_render_id,
                    current_segment + 1,
                    motion_safe_end_frames if motion_safe_end_frames is not None else "none (no freeze detected)",
                )
            except Exception as exc:

...
Reference-mode carry-over: the previous chunk's own decoded video/audio become
the next chunk's REF2VA conditioning input (whole clip as ref_video, last
frame as a keyframe anchor, audio tail as ref_audio). This is the same
continuity design used by community "Reference mode" H3 director tools and
needs no MiniMax H3 runtime patching: every input is a stock, documented
MiniMaxH3ReferenceToVideo argument.
"""
...
class IAMCCS_MiniMaxH3MotionContext:
    """Explicit, append-only configuration node for H3 reference-mode carry-over."""
    ...
    def configure(
        ...
        config = {
            ...
            "schema_version": 4,
            "method": "director_dual_continuity",
...
def save_reference_media(render_id: str, segment_index: int, images: torch.Tensor, audio: dict[str, Any] | None) -> bool:
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "IAMCCS_MiniMaxH3MotionContext": "MiniMax H3 Motion Context (Reference Carry-Over)",
}

"""Legacy-compatible MiniMax H3 continuity settings.

FL2VA Director parity is now defined exclusively by the planned image-pair
bridges. This node keeps saved workflow widget positions valid but cannot
replace a planned keyframe with REF2VA continuation media.
"""
...
class IAMCCS_MiniMaxH3MotionContext:
    """Compatibility node retained for workflows saved before the FL2VA reset."""
    ...
    def configure(
        ...
        config = {
            ...
            "schema_version": 5,
            "method": "director_fl2v_keyframe_parity",
            "enabled": False,
        ...
        # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
        report = "H3 Director FL2V parity | planned image-pair bridges | REF2VA carry disabled"
        return config, report


def is_active(config: Any, segment_index: Any) -> bool:
    del config, segment_index
    return False
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "IAMCCS_MiniMaxH3MotionContext": "MiniMax H3 Motion Context (Legacy Disabled for FL2V)",
}

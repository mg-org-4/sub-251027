...
class IAMCCS_MiniMaxH3NativeCheckpointSave:
    ...
    def checkpoint(
        ...
        locked_overlap_frames = 0
        if current_segment > 0 and trim_count > 0:
            try:
                from .iamccs_minimax_h3_continuity import load_reference_media
                # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
                previous_media = load_reference_media(active_render_id, current_segment - 1)
                # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
                previous_frames = previous_media.get("images") if isinstance(previous_media, dict) else None
                # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
                if torch.is_tensor(previous_frames) and previous_frames.ndim == 4:
                    locked_overlap_frames = min(trim_count, int(images.shape[0]), int(previous_frames.shape[0]))
                    if tuple(previous_frames.shape[1:]) != tuple(images.shape[1:]):
                        raise ValueError("decoded FLF overlap dimensions do not match the current chunk")
                    # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
                    images_to_save = images.clone()
                    images_to_save[:locked_overlap_frames] = previous_frames[-locked_overlap_frames:].to(
                        device=images.device,
                        dtype=images.dtype,
                    )
                    # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
                    LOG.info(
                        "MiniMax H3 locked decoded FLF overlap | render=%s segment=%d frames=%d",
                        active_render_id,
                        current_segment + 1,
                        locked_overlap_frames,
                    )
                else:
                    raise FileNotFoundError("previous decoded FLF tail cache is missing")
            except Exception as exc:
                raise RuntimeError(
                    f"MiniMax H3 cannot lock the FLF overlap for segment {current_segment + 1}: {exc}"
                ) from exc
        audio_to_save = audio
        ...
        motion_saved = False
        if current_segment + 1 < total_segments or (
            isinstance(motion_context, dict) and bool(motion_context.get("enabled"))
        ):
            try:
                ...
                LOG.info(
                    "MiniMax H3 classic FLF tail cache | render=%s segment=%d saved=%s",
                    active_render_id,
                    current_segment + 1,
                    motion_saved,
                )

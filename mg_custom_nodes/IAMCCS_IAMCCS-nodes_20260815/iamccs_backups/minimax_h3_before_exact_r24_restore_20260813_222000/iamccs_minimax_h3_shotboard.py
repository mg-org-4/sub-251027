...
class IAMCCS_MiniMaxH3ShotPlanner:
    @classmethod
    def INPUT_TYPES(cls):
        """Expose the original stable 75-widget Shotboard contract.

        Runtime-only V2V/continuity fields intentionally live in CineLinX and
        use method defaults.  They must not be appended to the positional
        ComfyUI widget list because an already-saved 66/67/73/75-field board
        would then be interpreted with a shifted schema before the node gets
        a chance to build its project plan.
        """
        ...
        turbo_loras = list(dict.fromkeys(turbo_loras))
        all_loras = list(dict.fromkeys(["", *folder_paths.get_filename_list("loras")]))
        crisp = next((name for name in all_loras if "ltx" in name.lower() and "crisp" in name.lower()), "")
        return {
            "required": {
                ...
                "acceleration": (["low_vram_auto", "native", "h3_sage", "sol_low_vram", "sol_adaptive_safe", "sol_adaptive_balanced", "adaptive_safe", "spectrum", "sage_spectrum", "auto_3060", "sage", "sage_sol"], {"default": "low_vram_auto"}),
                ...
                "text_encoder_device": (["auto", "cpu_safe_12gb"], {"default": "auto"}),
                "performance_profile": (["low_vram_draft", "low_vram_balanced", "low_vram_turbo", "h3_turbo_quality", "h3_native_quality", "custom", "rtx3060_draft", "rtx3060_balanced", "rtx3060_turbo"], {"default": "low_vram_balanced"}),
                ...
                "ltx_detailer_lora_name": (all_loras, {"default": crisp}),
            ...
            "optional": {
                "cine_linx": (SUPERNODE_LINX_TYPE, {"tooltip": "Connect IAMCCS_Prompter here. Its target is resolved against this Shotboard's real timeline slots."}),
            },
        }

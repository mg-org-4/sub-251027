    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                # These fields deliberately remain in the single serialized
                # widget tail.  ComfyUI can interleave OPTIONAL widget inputs
                # ahead of late REQUIRED controls when a workflow is restored,
                # which would move e.g. `raw_only` into an LTX INT field.
                # Appending them here gives every historical board the same
                # stable 85-value contract; absent legacy values use defaults.

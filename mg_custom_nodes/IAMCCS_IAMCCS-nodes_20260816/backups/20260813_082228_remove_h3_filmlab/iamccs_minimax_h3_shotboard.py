...
class IAMCCS_MiniMaxH3ShotPlanner:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "seed_control_after_generate": ("STRING", {"default": "fixed"}),
            ...
            "optional": {
                ...
                # Muse Minimax Director voice-clone convention: 0=off, 1-4
                # pairs the reference audio's voice to that <Picture N>.
                # Appended last so it never shifts any earlier widget position.
    ...
    def plan(
        ...
        seed=42,
        seed_control_after_generate="fixed",
        seed_stride=1,
        steps=16,
...
class IAMCCS_MiniMaxH3NativeCheckpointSave:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "render_id": ("STRING", {"default": "minimax_h3_render"}),
                # Retained only to keep existing workflow widget positions.
                # Every multi-segment H3 plan always runs through the native
                # queue, regardless of task mode.
    ...
    def checkpoint(
        ...
        motion_saved = False
        if isinstance(motion_context, dict) and bool(motion_context.get("enabled")):
            try:
                from .iamccs_minimax_h3_continuity import save_reference_media
                motion_saved = save_reference_media(active_render_id, current_segment, images, audio)
                LOG.info(
                    "MiniMax H3 reference carry-over cache | render=%s segment=%d saved=%s",
                    active_render_id,
                    current_segment + 1,
                    motion_saved,
                )
        ...
        filmlab_saved = False
        filmlab_trimmed = 0
        if isinstance(motion_state, dict) and str(motion_state.get("method", "")) == "filmlab_av_latent_pin":
            try:
                ...
                previous_trim = max(0, int(motion_state.get("previous_export_trim", 0)))
                previous_segment = int(motion_state.get("previous_segment", current_segment - 1))
                if previous_trim > 0 and previous_segment >= 0:
                    trimmed = trim_cached_export(active_render_id, previous_segment, previous_trim, fps=fps)
                    if trimmed is not None:
                        previous_images, previous_audio = trimmed
                        previous_name = f"{base_name}_{active_render_id}_{safe_stage_label}_seg_{previous_segment + 1:04d}.mp4"
                        previous_path = output_folder / previous_name
                        _encode_images(previous_images, previous_audio, fps, previous_path)
                        _write_segment_metadata(previous_path, int(previous_images.shape[0]), fps, "cut")
                        filmlab_trimmed = previous_trim
                filmlab_saved = save_av_handoff(
                    active_render_id,
                    current_segment,
                    sampled_latent,
                    images,
                    audio,
                    int(motion_state.get("trim_frames", 0)),
                    int(images.shape[0]),
                )
            except Exception as exc:
                raise RuntimeError(f"Filmlab AV handoff checkpoint failed: {exc}") from exc

        messages = [f"{safe_stage_label.upper()} checkpoint saved: {segment_name}"]
        if filmlab_saved:
            messages.append("Filmlab AV handoff saved")
        if filmlab_trimmed:
            messages.append(f"Filmlab phase-aligned prior export trimmed: -{filmlab_trimmed}f")
        ...
        if next_segment < total_segments:
            ...
            h3_segment_nodes = {
                ...
                "IAMCCS_MiniMaxH3FilmlabModelRouter",
                "IAMCCS_MiniMaxH3FilmlabConditioning",
            }
            ...
            # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
            LOG.warning(
                "FILMLAB TRACE queue-next | render=%s | current=%d | next=%d | conditioning_nodes=%d | checkpoint_updated=%s",
        ...
        LOG.info(
            ...
            trim_count,
            "filmlab_saved" if filmlab_saved else ("saved" if motion_saved else "off"),
...
from .iamccs_minimax_h3_filmlab_motion import (
    NODE_CLASS_MAPPINGS as _FILMLAB_NODE_CLASS_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as _FILMLAB_NODE_DISPLAY_NAME_MAPPINGS,
)
...
NODE_CLASS_MAPPINGS.update(_FILMLAB_NODE_CLASS_MAPPINGS)
NODE_DISPLAY_NAME_MAPPINGS.update(_FILMLAB_NODE_DISPLAY_NAME_MAPPINGS)

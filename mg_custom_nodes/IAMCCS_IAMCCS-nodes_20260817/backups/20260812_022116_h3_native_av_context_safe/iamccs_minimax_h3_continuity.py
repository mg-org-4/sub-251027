...
def _ensure_runtime_patches() -> None:
    ...
    if not getattr(current_layout, _LAYOUT_MARKER, False):
        if getattr(current_layout, "_h3_motion_context_layout_patch", False) or getattr(current_layout, "_h3_director_continuity_layout_patch", False):
    ...
    if getattr(current_payload, _PAYLOAD_MARKER, False):
        return
    if getattr(current_payload, "_h3_motion_context_payload_patch", False) or getattr(current_payload, "_h3_director_continuity_payload_patch", False):
    ...
    def payload_patch(self, **kwargs):
        ...
        payload["cond_video_latents"] = [item["latent"] for item in keyframes if torch.is_tensor(item.get("latent"))] + [item["latent"] for item in refs if torch.is_tensor(item.get("latent"))]

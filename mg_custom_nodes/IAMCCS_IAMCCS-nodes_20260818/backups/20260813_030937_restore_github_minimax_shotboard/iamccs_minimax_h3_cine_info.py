...
def _normalise_audio_mode(value: Any) -> str:
    mode = str(value or "h3_native_generated").strip().lower()
    mode = R21_AUDIO_MODE_ALIASES.get(mode, mode)
    return mode if mode in R21_AUDIO_MODES else "h3_native_generated"
    ...
    for key in list(resources):
        if key.startswith(RESOURCE_PREFIX) and (
            ...
            or key in {
                ...
                f"{RESOURCE_PREFIX}custom_audio",
            }
        ):
...
class IAMCCS_CineInfoH3:
        ...
        return {
            "required": {
                ...
                "reference_resize_policy": (
                    ["from_shotboard", "off", "canvas_crop", "canvas_pad", "total_pixels"],
                    {"default": "from_shotboard"},
                ),
            ...
            "optional": {
                ...
                "custom_audio": (
                    ...
                    {
                        "tooltip": (
                            "R21 custom audio drive. This is distinct from REF2VA reference audio: "
                            "an atomic backend may encode it into the AV latent before sampling."
                        ),
                    },
                ),
            },
    ...
    def attach(
        ...
        shotplan = _h3_plan(cine_linx)
        selected_audio_mode = _normalise_audio_mode(shotplan.get("audio_mode"))
        ...
        reference_audio_meta = _audio_meta(reference_audio)
        custom_audio_meta = _audio_meta(custom_audio)
        manifest = {
            ...
            "custom_audio_drive": {
                **custom_audio_meta,
                "selected": selected_audio_mode == "h3_custom_audio_drive",
                "resource": f"{RESOURCE_PREFIX}custom_audio",
                "purpose": "pre_sampler_audio_latent_replacement",
                "final_track_policy": "original_source_preferred",
            },
            "audio_route": {
                "selected_mode": selected_audio_mode,
                "reference_resource": f"{RESOURCE_PREFIX}ref_audio",
                "custom_drive_resource": f"{RESOURCE_PREFIX}custom_audio",
        ...
        reference_audio_active = bool(reference_audio_meta.get("connected") and str(reference_audio_role) != "off")
        active_audio = bool(
            reference_audio_active
            or isinstance(reference_video_audio, dict)
        )
        custom_audio_connected = bool(custom_audio_meta.get("connected"))
        audio_conflict = bool(reference_audio_active and custom_audio_connected)
        custom_audio_required_missing = bool(
            selected_audio_mode == "h3_custom_audio_drive" and not custom_audio_connected
        )
        manifest["active_reference_count"] = active_images + int(active_video) + int(active_audio)
        manifest["custom_audio_connected"] = custom_audio_connected
        manifest["audio_conflict"] = audio_conflict
        manifest["custom_audio_required_missing"] = custom_audio_required_missing
        ...
        report = (
            ...
            f"audio_ref={'on' if active_audio else 'off'} | custom_drive={'on' if custom_audio_connected else 'off'} | "
            f"audio_mode={selected_audio_mode} | conflict={'yes' if audio_conflict else 'no'} | "
            f"required_missing={'yes' if custom_audio_required_missing else 'no'} | ref_size={ref_image_size} | "
        ...
        config = {
            ...
            "custom_audio": custom_audio_meta,
            "audio_conflict": audio_conflict,
            "custom_audio_required_missing": custom_audio_required_missing,
            "ref_image_size": str(ref_image_size),
        ...
        resources = {
            ...
            f"{RESOURCE_PREFIX}custom_audio": custom_audio,
        }
        out_linx = build_stage_linx_payload(
            ...
            policies={
                ...
                "custom_audio_is_distinct_from_reference_audio": True,

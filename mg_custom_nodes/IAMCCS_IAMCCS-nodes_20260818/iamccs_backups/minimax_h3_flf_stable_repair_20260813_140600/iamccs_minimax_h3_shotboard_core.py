...
def _chunk_task(task_mode: str, audio_mode: str, has_first: bool, has_last: bool) -> str:
    ...
    if requested in {"ref2va", "ref2va_audio", "ref2va_reference", "v2va_object_swap"}:
        return "ref2va"
...
def build_shotplan(
    *,
    ...
    explicit_ref2v_mode = requested_task_mode in {
        "ref2va", "ref2va_audio", "ref2va_reference", "v2va_object_swap",
    }
    ...
    ref2v_hard_cut_mode = bool(explicit_ref2v_mode and len(slots) > 1)
    ...
    for slot_index, slot in enumerate(slots):
        ...
        # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
        # FLF anchors stay authoritative, while the rendered previous tail is
        # the continuity input for every non-initial bridge.
        bridge_first = bool(
            chunk_index > 0
            and not hard_cut_start
            and (flf_anchor_mode or not first_path)
        )
        ...
        chunk = {
            ...
            "audio_handoff_silence_seconds": 0.0 if slot_index + 1 >= len(slots) else 1.0,
            "audio_handoff_silence_head_seconds": 0.0 if slot_index == 0 else 1.0,
            "audio_handoff_silence_tail_seconds": 0.0 if slot_index + 1 >= len(slots) else 1.0,
    ...
    return {
        ...
        "continuation_mode": (
            "flf_image_center_bridges"
            if flf_anchor_mode
            else (
                "i2v_hard_cuts"
                if i2v_hard_cut_mode
                else ("ref2va_independent_hard_cuts" if ref2v_hard_cut_mode else "timeline_keyframe_adjacency")
            )
        ),
        ...
        "audio_handoff_policy": {
            "scope": "both_sides_of_every_internal_independent_chunk_boundary",
            "speech_free_head_seconds_after_first": 1.0,
            "speech_free_tail_seconds": 1.0,
            "final_chunk_restricted": False,
            "purpose": "keep dialogue and new vocalisations outside AV edit and overlap handles",
        },

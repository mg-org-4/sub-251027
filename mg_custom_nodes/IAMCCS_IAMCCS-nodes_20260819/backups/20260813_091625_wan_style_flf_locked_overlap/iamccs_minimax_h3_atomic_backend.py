...
def _uses_classic_flf_tail_carry(cine_linx: dict[str, Any], segment_index: int) -> bool:
    """Use the prior decoded FLF tail as stock Ref2VA motion conditioning."""
    chunk = _chunk(cine_linx, segment_index)
    return (
        int(segment_index) > 0
        and _effective_task(cine_linx, chunk) == "fl2va"
        and bool(chunk.get("uses_bridge_first_frame"))
    )
...
class IAMCCS_MiniMaxH3AtomicModelRouter:
    ...
    def _input_name(cine_linx, segment_index, motion_context=None):
        ...
        if _uses_classic_flf_tail_carry(cine_linx, segment_index):
            return "ref2va_model"
        try:
...
class IAMCCS_MiniMaxH3AtomicConditioningBackend:
    ...
    def prepare(
        ...
        classic_flf_carry = _uses_classic_flf_tail_carry(cine_linx, segment_index)
        if task == "fl2va" and (classic_flf_carry or motion_context is not None):
            try:
                ...
                carry_enabled = classic_flf_carry or is_active(motion_context, segment_index)
                if carry_enabled:
                    previous_media = load_reference_media(render_id, int(segment_index) - 1)
                    if previous_media is None and classic_flf_carry:
                        raise FileNotFoundError(
                            f"FLF motion tail missing for chunk {int(segment_index) + 1}. "
                            "Start the render from chunk 1 so the queue can save its preceding frames."
                        )
                    if previous_media is not None:
                        tail_frames = max(5, min(24, int(shotplan.get("flf_overlap_frames", 9))))
                        carry = build_reference_carry_over(
                            previous_media,
                            continue_audio=bool(motion_context.get("continue_audio", True)) if isinstance(motion_context, dict) else True,
                            audio_tail_seconds=float(motion_context.get("audio_tail_seconds", 4.0)) if isinstance(motion_context, dict) else 4.0,
                            motion_tail_frames=tail_frames,
                            max_video_frames=visible_frames,
                        )
                        motion_state.update({
                            "active": True,
                            "strategy": "reference_motion_carry" if classic_flf_carry else continuity_strategy(motion_context),
                            "carry": carry,
                            "method": "classic_flf_tail_carry",
                        })
            except Exception as exc:
                if classic_flf_carry:
                    raise
        ...
        if bool(chunk.get("uses_bridge_first_frame")) and task == "fl2va":
            ...
            if continuity_bridge is None:
                raise FileNotFoundError(
                    f"FLF bridge frame missing for chunk {int(segment_index) + 1}. "
                    "Run the queue from chunk 1 so its generated final frame can seed this chunk."
                )
        del motion_context
        ...
        if task == "t2va":
            ...
            elif last is None or first is None:

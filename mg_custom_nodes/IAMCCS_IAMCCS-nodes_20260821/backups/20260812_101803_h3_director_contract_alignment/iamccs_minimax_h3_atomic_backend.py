...
class IAMCCS_MiniMaxH3AtomicConditioningBackend:
    ...
    def prepare(
        ...
        visible_frames = int(chunk.get("frame_count", 124))
        frames = visible_frames
        motion_state = {"active": False, "trim_frames": 0, "export_frames": visible_frames}
        ...
        if task == "fl2va" and motion_context is not None:
            try:
                ...
                if is_active(motion_context, segment_index):
                    previous_motion = load_sampled_av(render_id)
                    if previous_motion is not None:
                        frames, trim_frames = frame_budget(visible_frames, motion_context.get("frames", 22))
                        export_frames = max(5, visible_frames - int(chunk.get("trim_head_frames", 0)))
                        motion_state = {
                            "active": True,

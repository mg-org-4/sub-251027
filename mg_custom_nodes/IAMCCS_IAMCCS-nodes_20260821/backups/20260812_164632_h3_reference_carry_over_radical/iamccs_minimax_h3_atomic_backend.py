...
class IAMCCS_MiniMaxH3AtomicConditioningBackend:
    ...
    def prepare(
        ...
        if task == "fl2va" and motion_context is not None:
            try:
                from .iamccs_minimax_h3_continuity import frame_budget, is_active, load_sampled_av
                if is_active(motion_context, segment_index):
                    previous_motion = load_sampled_av(render_id, int(segment_index) - 1)
                    if previous_motion is not None:
                        frames, trim_frames = frame_budget(visible_frames, motion_context.get("frames", 22))
                        motion_state = {
                            "active": True,
                            ...
                            "segment_index": int(segment_index),
                            "frames": int(motion_context.get("frames", 22)),
                            "continue_audio": bool(motion_context.get("continue_audio", True)),
                            "freeze_safe_handover": bool(motion_context.get("freeze_safe_handover", True)),
                        }
            except Exception as exc:
                LOG.warning("MiniMax H3 temporal context unavailable: %s", exc)
        # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
        ...
        if task == "t2va":
            ...
            if last is None or (first is None and not motion_state["active"]):
            ...
            result, text_encoder_report = _run_h3_conditioning_with_cpu_fallback(
                ...
                lambda active_clip: MiniMaxH3ImageToVideo.execute(
                    clip=active_clip, vae=video_vae, prompt=prompt,
                    width=width, height=height, length=frames,
                    first_frame=None if motion_state["active"] else first, last_frame=last,
                ),
            )
            if motion_state["active"]:
                manifest.append({"label": "<Picture 1>", "role": "final_keyframe"})
            else:
                manifest.extend([
        ...
        if motion_state["active"]:
            try:
                from .iamccs_minimax_h3_continuity import apply_context
                positive, motion_report = apply_context(
                    positive, previous_motion, motion_state["frames"],
                    motion_state["continue_audio"], frames,
                    motion_state.get("freeze_safe_handover", True),
                )
            except Exception as exc:
                LOG.warning("MiniMax H3 temporal context fell back to PNG bridge: %s", exc)
                motion_state = {
                    "active": False,
                    "trim_frames": 0,
                    "export_frames": visible_frames,
                ...
                motion_report = "fallback_bridge"
        if motion_state["active"]:
            LOG.info(
                "MiniMax H3 continuity | segment=%d source_segment=%d sample=%df trim=%df export=%df %s",
                int(segment_index) + 1,
                int(segment_index),
                int(motion_state["sample_frames"]),
                int(motion_state["trim_frames"]),
                int(motion_state["export_frames"]),
                motion_report,
            )
        LOG.info(

...
class IAMCCS_MiniMaxH3AtomicConditioningBackend:
    ...
    def prepare(
        ...
        if task == "fl2va" and motion_context is not None:
            try:
                from .iamccs_minimax_h3_continuity import build_reference_carry_over, continuity_strategy, is_active, load_reference_media
                ...
                LOG.warning("MiniMax H3 reference carry-over unavailable: %s", exc)
        # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
        ...
        if task == "fl2va" and motion_state["active"]:
            prompt = _context_owned_fl2va_prompt(prompt)
        ...
        # The generated tail is continuity conditioning; the Shotboard first
        # anchor remains the deterministic fallback if the bridge is absent.
        ...
        if task == "t2va":
            ...
            if motion_state["active"]:
                ...
                strategy = str(motion_state.get("strategy", "reference_motion_carry"))
                if strategy == "reference_motion_carry":
                    ...
                    if torch.is_tensor(last):
                        ...
                        target_line = (
                            "`<Picture 2>` is the exact final frame target for this chunk. Arrive at its "
                            "framing, pose, subject placement, and camera distance by the final frame."
                        )
                    ...
                    carry_over_prompt = "\n\n".join([
                        ...
                        "`<Picture 1>` is the exact final frame of the previous chunk. `<Video 1>` is the final motion tail immediately before it. Continue forward from their final moment: no restart, no new take, no replay.",
                        target_line,
                        prompt,
                    ])
                    result, text_encoder_report = _run_h3_conditioning_with_cpu_fallback(
                    ...
                    carry_over_prompt = "\n\n".join([
                        ...
                        "The opening frame is hard locked to the exact final frame of the previous chunk. "
                        "Continue its ongoing action naturally from that framing and pose: no restart, no new "
                        "take, no new camera setup, and no replay of the preceding footage. Advance forward in time.",
                        prompt,
                    ])
                    result, text_encoder_report = _run_h3_conditioning_with_cpu_fallback(
                        clip,
                        shotplan,
                        lambda active_clip: MiniMaxH3ImageToVideo.execute(
                            clip=active_clip, vae=video_vae, prompt=carry_over_prompt,
                            width=width, height=height, length=frames,
                            first_frame=carry["ref_image"], last_frame=last,
                        ),
                    )
                    prompt = carry_over_prompt
            else:
        ...
        motion_report = "off"
        if motion_state["active"]:
            carry = motion_state["carry"] or {}
            motion_tail_frames = int(carry.get("ref_video", _black(width, height)).shape[0])
            target_end = any(item.get("role") == "target_end_keyframe" for item in manifest)
            motion_report = (
                f"{motion_state.get('strategy', 'reference_motion_carry')} "
                f"motion_tail={motion_tail_frames}f target_end={'yes' if target_end else 'no'}"
            )
            LOG.info(
                "MiniMax H3 continuity | segment=%d source_segment=%d method=%s",
                int(segment_index) + 1,
                int(segment_index),
                motion_report,
            )
        ...
        return (
            ...
            0 if motion_state["active"] else int(chunk.get("trim_head_frames", 0)),
            report,
            motion_state,
        )

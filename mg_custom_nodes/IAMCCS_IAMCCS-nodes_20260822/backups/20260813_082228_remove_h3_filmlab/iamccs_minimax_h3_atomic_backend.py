...
class IAMCCS_MiniMaxH3AtomicModelRouter:
    ...
    def _input_name(cine_linx, segment_index, motion_context=None):
        ...
        try:
            from .iamccs_minimax_h3_continuity import uses_reference_motion_carry
            if task == "fl2va" and uses_reference_motion_carry(motion_context, segment_index):
                return "ref2va_model"
        except Exception:
            pass
...
class IAMCCS_MiniMaxH3AtomicConditioningBackend:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "motion_context": ("IAMCCS_H3_MOTION_CONTEXT",),
                "filmlab_context": ("IAMCCS_H3_MOTION_CONTEXT",),
            },
        }
    ...
    def prepare(
        ...
        prompt_override="",
        motion_context=None,
        filmlab_context=None,
    ):
        from comfy_extras.nodes_minimax_h3 import MiniMaxH3ImageToVideo, MiniMaxH3ReferenceToVideo
        from . import iamccs_minimax_h3_filmlab_motion as filmlab_runtime
        from .iamccs_minimax_h3_filmlab_motion import (
            apply_motion_context as apply_filmlab_motion_context,
            generation_frame_budget as filmlab_generation_frame_budget,
            is_continuity_active as filmlab_is_continuity_active,
            lock_generated_video_prefix as lock_filmlab_generated_video_prefix,
            load_av_handoff as load_filmlab_av_handoff,
            continuation_prompt_body as filmlab_continuation_prompt_body,
            reinforce_fl2va_prompt as reinforce_filmlab_fl2va_prompt,
        )
        ...
        filmlab_trim_frames = 0
        # FLF continuation uses decoded-frame REF2VA carry-over. Keep this
        # input for graph compatibility, but do not let experimental AV-latent
        # pinning replace the normal continuation route.
        filmlab_enabled = False
        filmlab_active = False
        # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
        LOG.warning(
            "FILMLAB TRACE conditioner-entry | source=%s | rev=%s | segment=%s | active=%s | render=%s | task=%s",
            filmlab_runtime.__file__, filmlab_runtime.SOURCE_REVISION, segment_index, filmlab_active, render_id, task,
        )
        filmlab_previous = None
        if filmlab_active:
            if task != "fl2va":
                raise ValueError("Filmlab motion continuity supports planned FL2VA chunks only")
            if not str(render_id or "").strip():
                raise ValueError("Filmlab motion continuity requires the checkpoint render_id")
            filmlab_previous = load_filmlab_av_handoff(str(render_id), int(segment_index) - 1)
            if filmlab_previous is None:
                raise FileNotFoundError(
                    f"Filmlab AV handoff missing for segment {int(segment_index)}. "
                    "Start the queue from segment 1."
                )
            frames, filmlab_trim_frames = filmlab_generation_frame_budget(
                visible_frames,
                int(filmlab_context.get("context_frames", 22)),
            )
        motion_state = {
            "active": bool(filmlab_active),
            "trim_frames": int(filmlab_trim_frames),
        ...
        if task == "fl2va" and motion_context is not None:
            try:
                from .iamccs_minimax_h3_continuity import (
                    build_reference_carry_over,
                    continuity_strategy,
                    is_active,
                    load_reference_media,
                )
                if is_active(motion_context, segment_index):
                    previous_media = load_reference_media(render_id, int(segment_index) - 1)
                    if previous_media is not None:
                        carry = build_reference_carry_over(
                            previous_media,
                            continue_audio=bool(motion_context.get("continue_audio", True)),
                            audio_tail_seconds=float(motion_context.get("audio_tail_seconds", 4.0)),
                            motion_tail_frames=int(motion_context.get("motion_tail_frames", 22)),
                            max_video_frames=visible_frames,
                        )
                        motion_state.update({
                            "active": True,
                            "strategy": continuity_strategy(motion_context),
                            "carry": carry,
                            "method": "decoded_frame_reference_motion_carry",
                        })
            except Exception as exc:
                LOG.warning("MiniMax H3 FLF reference carry-over unavailable: %s", exc)
        # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
        ...
        if filmlab_active:
            prompt = filmlab_continuation_prompt_body(
                str(chunk.get("creative_prompt", "")),
                str(chunk.get("audio_handoff_prompt", "")),
            ) or prompt
        ...
        if bool(chunk.get("uses_bridge_first_frame")) and task == "fl2va":
            continuity_bridge = bridge_frame[:1] if torch.is_tensor(bridge_frame) else _load_flf_bridge(render_id)
        del motion_context
        # Director conditions continued FL2VA chunks with only the terminal
        # keyframe. The AV context becomes the sole frame-zero anchor after
        # stock conditioning, avoiding a competing Shotboard opening wide shot.
        first = continuity_bridge if torch.is_tensor(continuity_bridge) else planned_first
        if filmlab_active:
            first = None
        if not filmlab_active and first is None and torch.is_tensor(first_frame_override):
        ...
        if filmlab_active:
            # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
            # A future Shotboard keyframe would pull this continuation back to its
            # authored composition. The predecessor AV tail owns the full chunk.
            last = None
        ...
        if not filmlab_active and first is None and task in {"i2va", "fl2va"} and torch.is_tensor(ref_image_1):
            first = ref_image_1[:1]
        if last is None and task == "fl2va" and torch.is_tensor(ref_image_2):
            last = ref_image_2[:1]

        if filmlab_enabled and not filmlab_active and task == "fl2va":
            prompt = reinforce_filmlab_fl2va_prompt(
                prompt,
        ...
        if task == "t2va":
            ...
            if motion_state["active"] and motion_state.get("strategy") == "reference_motion_carry":
                carry = motion_state["carry"]
                refs = {"ref_image_1": carry["ref_image"]}
                ref_videos = {"ref_video_1": carry["ref_video"]}
                ref_audios = {"ref_audio_1": carry["ref_audio"]} if carry.get("ref_audio") is not None else None
                manifest.extend([
                    {"label": "<Picture 1>", "role": "continuation_anchor"},
                    {"label": "<Video 1>", "role": "continuation_motion_source"},
                ])
                target_line = ""
                if torch.is_tensor(last):
                    refs["ref_image_2"] = last
                    manifest.append({"label": "<Picture 2>", "role": "target_end_keyframe"})
                    target_line = "`<Picture 2>` is the exact final frame target for this chunk."
                if ref_audios is not None:
                    manifest.append({"label": "<Audio 1>", "role": "continuation_audio_tail"})
                carry_over_prompt = "\n\n".join([
                    "[reference generation + video continuation + keyframe completion]",
                    "`<Picture 1>` is the exact final frame of the previous chunk. `<Video 1>` is its final motion tail. Continue forward from that final moment: no restart, no new take, no replay.",
                    target_line,
                    prompt,
                ])
                result, text_encoder_report = _run_h3_conditioning_with_cpu_fallback(
                    ...
                    lambda active_clip: MiniMaxH3ReferenceToVideo.execute(
                        clip=active_clip, vae=video_vae, audio_vae=audio_vae,
                        prompt=carry_over_prompt, width=width, height=height, length=frames,
                        ref_image_size=str(shotplan.get("ref_image_size", "match")),
                        ref_images=refs, ref_videos=ref_videos,
                        ref_video_audios=None, ref_audios=ref_audios,
                    ),
                )
                prompt = carry_over_prompt
            elif (last is None or first is None) and not filmlab_active:
        ...
        if filmlab_active:
            positive, trim_frames, previous_export_trim = apply_filmlab_motion_context(
                positive,
                latent,
                filmlab_previous,
                context_frames=int(filmlab_context.get("context_frames", 22)),
                continue_audio=bool(filmlab_context.get("continue_audio", True)),
                sample_frames=frames,
            )
            latent, locked_steps = lock_filmlab_generated_video_prefix(
                latent,
                filmlab_previous,
                context_frames=int(filmlab_context.get("context_frames", 22)),
            )
            lock_mask = latent.get("noise_mask")
            mask_parts = lock_mask.unbind() if getattr(lock_mask, "is_nested", False) else ()
            # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
            LOG.warning(
                "FILMLAB TRACE conditioner-exit | segment=%s | lock_steps=%d | samples_nested=%s | "
                "mask_nested=%s | mask_shapes=%s",
                segment_index, locked_steps, getattr(latent.get("samples"), "is_nested", False),
                getattr(lock_mask, "is_nested", False), [tuple(part.shape) for part in mask_parts],
            )
            motion_state.update({
                # The hard copied predecessor tail is context-only. Decoding
                # drops it so the visible output is newly generated motion.
                "trim_frames": int(trim_frames),
                "previous_export_trim": int(previous_export_trim),
                "previous_segment": int(segment_index) - 1,
                "locked_steps": int(locked_steps),
            })
            motion_report = f"filmlab_hidden_context={trim_frames}f+generated_prefix_lock={locked_steps}steps"
        LOG.info(
        ...
        return (
            ...
            0 if filmlab_active else int(chunk.get("trim_head_frames", 0)),
            report,
            motion_state,
        )
...
class IAMCCS_MiniMaxH3FilmlabModelRouter(IAMCCS_MiniMaxH3AtomicModelRouter):
    """Filmlab FL2VA-only lazy model route."""


class IAMCCS_MiniMaxH3FilmlabConditioning(IAMCCS_MiniMaxH3AtomicConditioningBackend):
    """Filmlab FL2VA conditioning with AV-latent motion pinning."""
...
class IAMCCS_MiniMaxH3GenerationBackendV2:
    ...
    def render(
        ...
        filmlab_mode = isinstance(motion_state, dict) and str(motion_state.get("method", "")) == "filmlab_av_latent_pin"
        context_active = isinstance(motion_state, dict) and bool(motion_state.get("active"))
        actual_seed = (
            int(seed)
            if context_active or filmlab_mode
            else (int(seed) + int(chunk_index) * int(seed_stride))
        ) & 0xFFFFFFFFFFFFFFFF
        seed_contract = "filmlab_base_seed" if filmlab_mode else "shotboard_seed_stride"
        ...
        active_mask = latent.get("noise_mask") if isinstance(latent, dict) else None
        mask_parts = active_mask.unbind() if getattr(active_mask, "is_nested", False) else ()
        # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
        LOG.warning(
            "FILMLAB TRACE sampler-entry | chunk=%s | active=%s | seed=%s | sampler=%s | scheduler=%s | "
            "mask_nested=%s | mask_shapes=%s | video_mask_prefix=%s",
            chunk_index, context_active, actual_seed, sampler_name, scheduler, getattr(active_mask, "is_nested", False),
            [tuple(part.shape) for part in mask_parts],
            (float(mask_parts[0].min()), float(mask_parts[0].max())) if mask_parts else None,
        )
        ...
        LOG.warning("FILMLAB TRACE sampler-exit | chunk=%s | sampled_nested=%s | sample_shapes=%s", chunk_index, getattr(sampled.get("samples"), "is_nested", False), [tuple(part.shape) for part in sampled["samples"].unbind()] if getattr(sampled.get("samples"), "is_nested", False) else [])
        ...
        if isinstance(motion_state, dict) and bool(motion_state.get("active")):
            ...
            from .iamccs_minimax_h3_filmlab_motion import trim_context_prefix as trim_filmlab_context_prefix

            native_frames, native_audio = trim_filmlab_context_prefix(
                native_frames,
                native_audio,
                trim_frames,
                fps=H3_FPS,
            )
            native_frames = native_frames[:export_frames, ...]
            if isinstance(native_audio, dict) and torch.is_tensor(native_audio.get("waveform")):
                ...
                native_audio = dict(native_audio)
                native_audio["waveform"] = native_audio["waveform"][..., :audio_length]
            ...
            # By Carmine Cristallo Scalzi AI research (IAMCCS) - patreon.com/IAMCCS - carminecristalloscalzi.com
            LOG.warning(
                "FILMLAB TRACE decode-export | chunk=%s | sample_frames=%s | trim=%s | "
                "export_frames=%s | decoded_visible_frames=%s | locked_steps=%s",
                chunk_index,
                motion_state.get("sample_frames"),
                trim_frames,
                export_frames,
                int(native_frames.shape[0]),
                motion_state.get("locked_steps", 0),
            )
...
NODE_CLASS_MAPPINGS = {
    ...
    "IAMCCS_MiniMaxH3FilmlabModelRouter": IAMCCS_MiniMaxH3FilmlabModelRouter,
    "IAMCCS_MiniMaxH3FilmlabConditioning": IAMCCS_MiniMaxH3FilmlabConditioning,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "IAMCCS_MiniMaxH3FilmlabModelRouter": "MiniMax H3 Filmlab FL2VA Router",
    "IAMCCS_MiniMaxH3FilmlabConditioning": "MiniMax H3 Filmlab AV Motion Conditioning",

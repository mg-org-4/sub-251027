...
const NODE_CLASSES = new Set(["IAMCCS_ShotboardPlannerV2V", "IAMCCS_CineShotboardPlannerV5V2V"]);
const VERSION = "2026-07-14-shotboard-v5-v2v-ui1";
const WIDTH = 1840;
...
function removeExistingShotboardDom(node) {
    ...
    for (let i = node.widgets.length - 1; i >= 0; i--) {
        ...
        if (
            item?.type === "iamccs_shotboard_planner_v2v" ||
            item?.type === "iamccs_cine_shotboard_v3" ||
            item?.name === "Shotboard Planner V2V" ||
            item?.name === "Cine Shotboard V5"
        ) {
...
function widgetNames(name) {
    if (name === "fps") return ["fps", "frame_rate"];
    return [name];
}

function read(node, name, fallback = "") {
    for (const itemName of widgetNames(name)) {
        const value = widget(node, itemName)?.value;
...
function write(node, name, value) {
    for (const itemName of widgetNames(name)) {
        const item = widget(node, itemName);
        if (!item) continue;
        item.value = value;
...
function resolveBackendMode(node) {
    const raw = String(read(node, "backend_mode", "") || read(node, "v2v_mode", "") || read(node, "backend_profile", "ltx23_v2v_infinite_lipsync"));
    if (BACKEND_MODES[raw]) return raw;
    if (raw === "ltx_v2v" || raw === "ltx_loop") return "ltx_simple";
    if (raw === "scail_single" || raw === "scail_multi") return "scail2";
    if (raw === "wananimate_bg_locked") return "wananimate";
...
function backendModeToPlannerMode(mode, scailIdentity = "single_person") {
    if (mode === "scail2") return scailIdentity === "multi_person_identity" ? "scail_multi" : "scail_single";
    if (mode === "wananimate") return "wananimate_bg_locked";
    if (mode === "pose_transfer") return "pose_transfer";
    return "ltx_v2v";
}
...
function renderShotboardV2V(node) {
    ...
    function timelinePayload() {
        ...
        return {
            ...
            schema_version: 4,
            v2v_mode: backendModeToPlannerMode(activeBackendMode(), scailIdentity.value),
    ...
    function commit() {
        ...
        write(node, "v2v_mode", backendModeToPlannerMode(activeBackendMode(), scailIdentity.value));
    ...
    function chooseBackendMode(mode) {
        ...
        write(node, "v2v_mode", backendModeToPlannerMode(mode, scailIdentity.value));
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (!NODE_CLASSES.has(String(nodeData?.name || nodeData?.class_type || ""))) return;

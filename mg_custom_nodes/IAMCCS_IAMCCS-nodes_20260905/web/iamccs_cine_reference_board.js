...
app.registerExtension({
    name: "iamccs.cine.reference_board",
    async nodeCreated(node) {
        if (node.comfyClass !== "IAMCCS_CineReferenceBoard") return;
        ...
        uploadBtn.innerText = "Add References";
        uploadBtn.style.cssText = `
            background: ${CINE_FILM_LAB.button}; color: ${CINE_FILM_LAB.text}; border: 1px solid ${CINE_FILM_LAB.border}; 
            padding: 3px 8px; border-radius: 3px; cursor: pointer; font-size: 10px;
        `;

        const removeAllBtn = document.createElement("button");
        removeAllBtn.innerText = "Clear Board";
        removeAllBtn.style.cssText = `
            background: ${CINE_FILM_LAB.danger}; color: ${CINE_FILM_LAB.text}; border: 1px solid ${CINE_FILM_LAB.dangerDark}; 
            padding: 3px 8px; border-radius: 3px; cursor: pointer; font-size: 10px;
            transition: background 0.2s;
        `;
        removeAllBtn.onmouseenter = () => { removeAllBtn.style.background = CINE_FILM_LAB.dangerDark; };
        removeAllBtn.onmouseleave = () => { removeAllBtn.style.background = CINE_FILM_LAB.danger; };
        ...
        function getPreviewUrl(path) {
            ...
            if (isAbsoluteLocalPath(cleanPath)) {
                return `/api/iamccs/cine/view_image?path=${encodeURIComponent(cleanPath)}`;
            }
            return `/api/view?filename=${encodeURIComponent(cleanPath)}&type=input`;
        }
        ...
        function refreshGallery(isRearranging = false) {
            ...
            paths.forEach((path, index) => {
                ...
                del.style.cssText = `
                    ...
                    background: ${CINE_FILM_LAB.danger}; color: ${CINE_FILM_LAB.text}; 
                ...
                del.onmouseenter = () => { del.style.background = CINE_FILM_LAB.dangerDark; };
                del.onmouseleave = () => { del.style.background = CINE_FILM_LAB.danger; };
        ...
        container.ondragover = (e) => { 
            ...
            container.style.borderColor = CINE_FILM_LAB.active; 
        };
        container.ondragleave = (e) => { 
            e.preventDefault();
            e.stopPropagation(); // Prevent ComfyUI from seeing the drag event leave
            container.style.borderColor = CINE_FILM_LAB.border; 
        };
        container.ondrop = (e) => {
            e.preventDefault();

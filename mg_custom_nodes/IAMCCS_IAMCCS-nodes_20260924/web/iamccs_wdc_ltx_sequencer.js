...
// Global registry to track all IAMCCS_WDC_LTXSequencer nodes across all subgraphs
window._IAMCCS_WDC_LTXSequencerGlobalNodes = window._IAMCCS_WDC_LTXSequencerGlobalNodes || new Set();
...
// Finds all other IAMCCS_WDC_LTXSequencer nodes globally and mirrors the value to them
function syncWidgetAcrossNodes(sourceNode, widgetName, value) {
    if (!window._IAMCCS_WDC_LTXSequencerGlobalNodes) return;
    
    for (const targetNode of window._IAMCCS_WDC_LTXSequencerGlobalNodes) {
        // Target all OTHER IAMCCS_WDC_LTXSequencer nodes by direct object reference
        if (targetNode !== sourceNode) {
            ...
            if (widgetName === "insert_mode" && typeof targetNode._updateVisibility === "function") {
...
app.registerExtension({
    name: "Comfy.IAMCCS_WDC_LTXSequencer.DynamicInputs",
    async nodeCreated(node) {
        const supportedSequencerClasses = new Set([
            "IAMCCS_WDC_LTXSequencer",
            "IAMCCS_CineLTXSequencerExact",
            "IAMCCS_WDC_LTXSequencerFixed5",
        ]);
        if (!supportedSequencerClasses.has(node.comfyClass)) return;

        // Register this node instance globally
        window._IAMCCS_WDC_LTXSequencerGlobalNodes.add(node);
        ...
        node._applyWidgetCount = function(count) {
            ...
            const safeCount = Math.max(0, Math.min(count || 0, 50));
            this.properties["num_images"] = safeCount;
            if (numWidget) {
                numWidget.label = "images_loaded";
                numWidget.value = safeCount;
            }
            ...
            // 2. Remove all existing dynamic insert_frame/insert_second/strength/header widgets
            if (this.widgets) {
            ...
            for (let i = 1; i <= safeCount; i++) {
                ...
                const insertSecondWidgetName = `insert_second_${i}`;
                ...
                // Add insert_second widget with Sync Callback
                const savedInsertSecondValue = this.properties[insertSecondWidgetName];
                this.addWidget("number", insertSecondWidgetName,
                    savedInsertSecondValue !== undefined ? savedInsertSecondValue : 0.0,
                    (value) => {
                        this.properties[insertSecondWidgetName] = value;
                        syncWidgetAcrossNodes(this, insertSecondWidgetName, value); // Sync out
                    }, { min: 0.0, max: 9999.0, step: 0.1, precision: 2 }
                );
        ...
        node._updateVisibility = function() {
            ...
            for (const w of this.widgets) {
                ...
                if (w.name?.startsWith("insert_frame_")) shouldBeVisible = mode === "frames";
                else if (w.name?.startsWith("insert_second_")) shouldBeVisible = mode === "seconds";

                const hidden = w.type === "hidden";
        ...
        node.configure = function(info) {
            ...
            // IAMCCS port: workflow JSON properties are authoritative on load.
            // Reading from visual widgets here can corrupt values when LiteGraph
            // has already shifted dynamic widgets during deserialization.
            if (info.properties) {
        ...
        node.onSerialize = function(info) {
            ...
            if (this.widgets) {
                this.widgets.forEach(w => {
                    if (w.name === "num_images" || w.name === "insert_mode" || w.name === "frame_rate" || w.name.startsWith("insert_") || w.name.startsWith("strength_")) {
                        this.properties[w.name] = w.value;
                    }
                });
            }
            ...
            for (let i = 1; i <= 50; i++) {
                ...
                const secVal = this.properties[`insert_second_${i}`];
                const sVal = this.properties[`strength_${i}`];
                strictArray.push(fVal !== undefined ? fVal : 0);
                strictArray.push(secVal !== undefined ? secVal : 0.0);
        ...
        setTimeout(() => {
            ...
            const modeWidget = node.widgets?.find(w => w.name === "insert_mode");
            if (modeWidget) {
                modeWidget.callback = (val) => {
                    node.properties["insert_mode"] = val;
                    syncWidgetAcrossNodes(node, "insert_mode", val);
                    node._updateVisibility?.();
                };
            }
            const frameRateWidget = node.widgets?.find(w => w.name === "frame_rate");
            if (frameRateWidget) {
                frameRateWidget.callback = (val) => {
                    node.properties["frame_rate"] = val;
                    syncWidgetAcrossNodes(node, "frame_rate", val);
                };
            }
        }, 100);
        ...
        function readSourceImageCount(self) {
            ...
            const imageSourceClasses = new Set([
            ...
            const isImageSourceNode = (node) => node && imageSourceClasses.has(node.comfyClass);

            // Helper to safely trace back through Reroutes, CineInfo, and ComfyUI Group Nodes/Subgraphs
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (isImageSourceNode(originNode)) {
                    if (originNode.comfyClass === "IAMCCS_CineInfo") {
                        const cineInput = originNode.inputs?.find(inp => inp.name === "cine_linx");
                        if (cineInput?.link) {
                            const tracedPlanner = traceUpstream(graph, cineInput.link, visited);
                            if (tracedPlanner) return tracedPlanner;
                        }
                    }
                    return originNode;
                }
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (isImageSourceNode(innerNode)) {
                            return innerNode;
                        }
                    } catch (e) {
            ...
            function getCountFromNode(n) {
                ...
                const propCount = Number(n.properties?.image_count ?? n.properties?.cine_image_count);
                if (Number.isFinite(propCount) && propCount > 0) return propCount;

                const pathsWidget = n.widgets?.find(w => w.name === "image_paths" || w.name === "reference_paths");
                if (pathsWidget) {
                    const value = pathsWidget.value;
            ...
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (isImageSourceNode(n) && n.comfyClass !== "IAMCCS_CineInfo") {
                        imageSources.push(n);
                    }
            ...
            if (imageSources.length === 1) {
                return getCountFromNode(imageSources[0]);
            }
        ...
        node.onRemoved = function() {
            ...
            window._IAMCCS_WDC_LTXSequencerGlobalNodes.delete(node);

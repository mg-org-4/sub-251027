    ...
    def __init__(self, model_cfg: DictConfig):
        ...
        if self.is_resnet:
            if model_cfg.pixel_encoder.type == 'resnet18':
                network = resnet18(pretrained=is_pretrained_resnet)
            elif model_cfg.pixel_encoder.type == 'resnet50':
                network = resnet50(pretrained=is_pretrained_resnet)
            else:
...
class MaskEncoder(nn.Module):
    def __init__(self, model_cfg: DictConfig, single_object=False):
        ...
        if model_cfg.mask_encoder.type == 'resnet18':
            network = resnet18(pretrained=is_pretrained_resnet, extra_dim=extra_dim)
        elif model_cfg.mask_encoder.type == 'resnet50':
            network = resnet50(pretrained=is_pretrained_resnet, extra_dim=extra_dim)
        else:
            raise NotImplementedError
        self.conv1 = network.conv1
        self.bn1 = network.bn1

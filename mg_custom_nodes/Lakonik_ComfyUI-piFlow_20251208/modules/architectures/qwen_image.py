...
class QwenImageTransformer2DModelMod(QwenImageTransformer2DModel):

    """Modified QwenImageTransformer2DModel to support expanded output channels.
    """
    ...
    def _forward(
        ...
        **kwargs
        ...
        hidden_states = hidden_states[:, :num_embeds].view(
            orig_shape[0], orig_shape[-2] // 2, orig_shape[-1] // 2, -1, orig_shape[1], 2, 2)
        hidden_states = hidden_states.permute(0, 3, 4, 1, 5, 2, 6)
        return hidden_states.reshape(orig_shape[0], -1, *orig_shape[1:])[..., :x.shape[-2], :x.shape[-1]]

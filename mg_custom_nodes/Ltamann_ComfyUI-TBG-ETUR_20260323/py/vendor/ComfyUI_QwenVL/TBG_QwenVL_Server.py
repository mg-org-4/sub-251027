...
def _safe_json_load(path):
    try:
        ...
        print(f"[OpenAI-VLM] Failed to load GGUF catalog: {exc}")
        return {}
...
class QwenVLServerClient:
    ...
    def _tensor_to_data_uri(tensor):
        if tensor is None:
            raise ValueError("[OpenAI-VLM] Missing image input.")
        if tensor.dim() == 4:
            tensor = tensor[0]
    ...
    def resolve_alias(self, alias):
        ...
        if cfg is None:
            ...
            raise ValueError(f"[OpenAI-VLM] Unknown GGUF alias '{alias}'. Supported: {supported}")

        repo_id = cfg.get("repo_id")
        model_file = cfg.get("model_file")
        mmproj_file = cfg.get("mmproj_file")
        if not repo_id or not model_file or not mmproj_file:
            raise ValueError(f"[OpenAI-VLM] Incomplete catalog entry for alias '{alias}'.")
        ...
        def _download_if_missing(file_path, filename):
            ...
            raise FileNotFoundError(
                f"[OpenAI-VLM] Could not download '{filename}' for alias '{alias}'. Attempted repos: {attempted}."
            ) from last_exc
        ...
        print(
            f"[OpenAI-VLM] Alias='{alias}' model='{model_file}' mmproj='{mmproj_file}' "
            f"model_repo='{model_repo}' mmproj_repo='{mmproj_repo}'"
        )
        print(f"[OpenAI-VLM] Download cache model={model_cache} mmproj={mmproj_cache}")
        ...
        resolved = {
            ...
            "repeat_penalty": float(cfg.get("repeat_penalty", 1.0)),
            "present_penalty": float(cfg.get("present_penalty", 1.5)),
    ...
    def list_models(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"[OpenAI-VLM] Failed to list models from '{self.health_url}': {exc}"
            ) from exc

        if status < 200 or status >= 300:
            raise RuntimeError(
                f"[OpenAI-VLM] Failed to list models from '{self.health_url}' status={status}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"[OpenAI-VLM] Failed to parse /v1/models response from '{self.health_url}'"
            ) from exc
    ...
    def run(self, alias, custom_prompt, image, seed=0):
        ...
        top_p = float(resolved.get("top_p", 0.8))
        top_k = int(resolved.get("top_k", 20))
        temperature = float(resolved.get("temperature", 0.7))
        repeat_penalty = float(resolved.get("repeat_penalty", 1.0))
        present_penalty = float(resolved.get("present_penalty", 1.5))
        max_tokens = int(resolved.get("max_tokens", 1024))
        greedy = bool(resolved.get("greedy", False))
        if greedy:
            temperature = 0.0
        elif temperature <= 0:
            temperature = 0.7
        ...
        try:
            ...
            raise RuntimeError(
                f"[OpenAI-VLM] Request failed stage=request alias='{alias}' model='{model_name}' "
        ...
        print(f"[OpenAI-VLM] Tile request done alias='{alias}' model='{model_name}' latency={elapsed:.2f}s")
        return (content,)
    ...
    def run_with_model(
        ...
        if not model_name:
            raise ValueError("[OpenAI-VLM] Missing model name for OpenAI-compatible request.")
        if greedy:
            temperature = 0.0
        elif temperature <= 0:
            temperature = 0.7
        ...
        payload = {
            ...
            "seed": int(seed or 0),
            "presence_penalty": float(present_penalty),
            "frequency_penalty": 0.0,
            "extra_body": {
                "top_k": int(top_k),
                "repetition_penalty": float(repeat_penalty),
                "repeat_penalty": float(repeat_penalty),
                "present_penalty": float(present_penalty),
                "greedy": bool(greedy),
            },
        }
        ...
        try:
            ...
            body = exc.read().decode("utf-8", errors="ignore")
            raise RuntimeError(
                f"[OpenAI-VLM] Request failed stage=request model='{model_name}' "
                f"url='{self.base_v1}' status={getattr(exc, 'code', 'unknown')} body='{body[:512]}'"
            ) from exc
        except Exception as exc:
            raise RuntimeError(
        ...
        elapsed = time.time() - started
        print(f"[OpenAI-VLM] Tile request done model='{model_name}' latency={elapsed:.2f}s")
        return (content,)

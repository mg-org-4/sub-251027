...
@staticmethod #TBG code
def combine_segs(SEGS1, SEGS2):
    """Combine two SEGS objects into one."""
    if SEGS1 is None:
        return SEGS2
    if SEGS2 is None:
        return SEGS1

    # Handle both (metadata, segments) and raw list formats
    meta1, segs1 = SEGS1 if isinstance(SEGS1, tuple) else ((0, 0), SEGS1)
    meta2, segs2 = SEGS2 if isinstance(SEGS2, tuple) else ((0, 0), SEGS2)

    # Combine segments (using extend is more memory efficient)
    combined_segments = list(segs1) + list(segs2)

    return (meta1, combined_segments)

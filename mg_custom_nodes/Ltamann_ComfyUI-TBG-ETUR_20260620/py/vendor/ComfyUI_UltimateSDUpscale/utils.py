...
def resize_and_pad_image(image, width, height, fill=False, blur=False):
    ...
    resized = image.resize((resize_width, resize_height), resample=Image.Resampling.BILINEAR)
    ...
    result = result.resize((width, height), resample=Image.Resampling.BILINEAR)
...
def crop_mask(cond_dict, region, init_size, canvas_size, tile_size, w_pad, h_pad):
    ...
    for i in range(mask_tensor.shape[0]):
        ...
        mask = mask.resize(canvas_size, Image.Resampling.BILINEAR)
...
# crop_reference_latents added by TBG Tobias Laarmann c 2025 GPL licence V2
def crop_reference_latents(cond_dict, region, init_size, canvas_size, tile_size, w_pad, h_pad):
        ...
        k = 8  # down-sample factor from pixel space → latent space

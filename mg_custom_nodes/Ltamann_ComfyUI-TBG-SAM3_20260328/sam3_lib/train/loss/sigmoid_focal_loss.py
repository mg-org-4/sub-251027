...
if HAS_TRITON:
    ...
    # Triton not available - provide stub functions that raise errors
    def triton_sigmoid_focal_loss(*args, **kwargs):
        raise RuntimeError(
            "Triton is not available (requires PyTorch with CUDA support). "
            "Sigmoid focal loss with Triton acceleration is not available."
        )

    def triton_sigmoid_focal_loss_reduce(*args, **kwargs):
        raise RuntimeError(
            "Triton is not available (requires PyTorch with CUDA support). "
            "Sigmoid focal loss with Triton acceleration is not available."
        )

...
function _createAlignmentBox(asset, options = {}) {
    ...
    enhancedActions.appendChild(regenerateCaptionBtn);
    enhancedActions.appendChild(copyCaptionBtn);
    enhancedCaptionHeader.appendChild(enhancedCaptionLabel);
    enhancedCaptionHeader.appendChild(enhancedActions);

    const enhancedCaptionBox = document.createElement("div");
    enhancedCaptionBox.style.cssText = `
        margin-top: 4px;
        padding: 8px;
        border-radius: 6px;
    ...
    const _normaliseCaptionDisplay = (value) => {
        ...
        let out = (lines.length ? lines.join(" ") : raw).replace(/\s+/g, " ").trim();
        out = out.replace(/:{2,}\s*$/, "").trim();
        return out;
    };
    ...
    if (!aiEnabled || !isImageAsset || !asset?.id) {
        ...
        regenerateCaptionBtn.addEventListener("click", async (e) => {
            e.stopPropagation();
            const prevText = regenerateCaptionBtn.textContent;
            regenerateCaptionBtn.disabled = true;
            ...
            try {
                ...
                regenerateCaptionBtn.textContent = prevText;
            }
        });
    }

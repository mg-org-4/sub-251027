...
function selectorForTabId(tabId) {
...
export function configureSidebarAssetBadge({ sidebarTabId } = {}) {
    const nextId = String(sidebarTabId || "").trim();
    if (nextId) _sidebarTabId = nextId;
    scheduleBadgeUpdate();
}

export function incrementSidebarAssetBadge(asset = null) {
...
export function resetSidebarAssetBadge() {
    _count = 0;
    _seenAssetIds.clear();
    scheduleBadgeUpdate();
}

export function getSidebarAssetBadgeCount() {
    return _count;
}

 ...
 */
export function registerScanningSettings(safeAddSetting: (def: any) => void, settings: Record<string, any>, notifyApplied: (..._args: any[]) => void): void {
    ...
    const _clampWatcherValue = (raw: any, fallback: any, min: any, max: any) => {
        const parsed = Math.round(_safeNum(raw, fallback));
        return Math.max(min, Math.min(max, parsed));
    };

    const applyWatcherSettingsFromBackend = (payload: Record<string, any> = {}) => {
        const changed = [];

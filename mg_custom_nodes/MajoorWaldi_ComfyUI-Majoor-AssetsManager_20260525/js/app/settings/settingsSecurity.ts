...
function shouldSkipBooleanSecurityUpdate(currentValue: any, nextValue: any) {
    return normalizeSecurityBoolean(currentValue) === normalizeSecurityBoolean(nextValue);
}

function normalizeSecurityToken(value: any) {
...
function getCryptoRandomValues(target: any) {
...
function buildRandomTokenSegment(byteCount: any) {
...
export function generateRecommendedApiToken(): string {
    return `mjr_${buildRandomTokenSegment(18)}`;
}

export function isRecommendedRemoteLanSecurity(security: Record<string, any> | null | undefined): boolean {
...
export function buildRecommendedRemoteLanSecuritySettings(security: Record<string, any> | null | undefined): Record<string, any> {
...
function buildBackendSecurityPayload(security: any) {
    const sec = security || {};
    return {
...
function pushSecuritySettings(security: any) {
    return setSecuritySettings(buildBackendSecurityPayload(security));
}

function tokenFieldPlaceholder(settings: any) {
...
export function registerSecuritySettings(safeAddSetting: (def: any) => void, settings: Record<string, any>, notifyApplied: (..._args: any[]) => void): void {
    ...
    const registerSecurityToggle = (key: any, nameKey: any, descKey: any, sectionKey: any = "cat.security") => {
        safeAddSetting({

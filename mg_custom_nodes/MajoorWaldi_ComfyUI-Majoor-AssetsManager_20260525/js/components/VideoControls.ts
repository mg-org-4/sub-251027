...
function createBtn(className: any, text: any, title: any) {
...
function createIconBtn(className: any, iconClass: any, title: any, ariaLabel: any) {
    const b = document.createElement("button");
    b.type = "button";
    b.className = `mjr-video-btn ${className || ""}`.trim();
    if (title) b.title = title;
...
function createNumberInput(className: string, { min, max, step, value, title, ariaLabel, widthPx }: Record<string, any> = {}) {
...
function _resolveVideoControlsVariant(opts: any) {
    try {
...
function _resolveInitialFps(opts: any) {
    try {
 ...
 */
export function mountVideoControls(video: any, opts: Record<string, any> = {}) {
    try {
        ...
        let seekZones: any = null;
        let leftTrimZone: any = null;
        let selectedZone: any = null;
        let rightTrimZone: any = null;
        if (advanced) {
        ...
        unsubs.push(
            safeAddListener(
                ...
                (e) => {
                    try {
                        if (controls.contains?.(e?.target as Node)) preventStop(e);
                    } catch (e: any) {
                        console.debug?.(e);
                    }
                },
        ...
        const setToggleBtn = (btn: any, on: any) => {
            try {
                if (!btn) return;
        ...
        const setPlaybackRate = (nextRate: any) => {
            try {
        ...
        const currentFrame = (timeOverride: number | null | undefined = null) => {
            try {
        ...
        const updateTimeUI = (timeOverride: number | null | undefined = null) => {
            try {
                const d = Number(video?.duration);
                const sourceTime = timeOverride == null ? video?.currentTime : timeOverride;
                const t = Number(sourceTime);
        ...
        const applyRangeChange = ({ prefer = null }: { prefer?: string | null } = {}) => {
            if (!advanced) return;
            try {
                normalizeRange();
        ...
        const stepFrames = (direction: any) => {
            tryAutoUnmute();
            try {
        ...
        unsubs.push(
            safeAddListener(video, "click", (e: any) => {
                try {
        ...
        unsubs.push(
            safeAddListener(playBtn, "click", (e: any) => {
                stop(e);
                togglePlay();
            }),
        );
        unsubs.push(
            safeAddListener(prevFrameBtn, "click", (e: any) => {
                stop(e);
                stepFrames(-1);
            }),
        );
        unsubs.push(
            safeAddListener(nextFrameBtn, "click", (e: any) => {
                stop(e);
                stepFrames(1);
            }),
        );
        unsubs.push(
            safeAddListener(toInBtn, "click", (e: any) => {
                stop(e);
                const { inF } = getEffectiveInOut();
                goToFrame(inF);
                flashFrameStep();
        ...
        unsubs.push(
            safeAddListener(toOutBtn, "click", (e: any) => {
                stop(e);
        ...
        const seekToClientX = (clientX: any) => {
            try {
                const d = Number(video?.duration);
        ...
        const seekDrag: Record<string, any> = { active: false, pointerId: null, ac: null };

        const endSeekDrag = (e: any = null) => {
        ...
        const moveSeekDrag = (e: any) => {
            if (!seekDrag.active) return;
            preventStop(e);
            seekToClientX(e.clientX);
        };

        const startSeekDrag = (e: any) => {
            try {
        ...
        unsubs.push(
            safeAddListener(seek, "input", (e: any) => {
                stop(e);
                tryAutoUnmute();
                try {
                    const d = Number(video?.duration);
        ...
        if (advanced) {
            unsubs.push(
                safeAddListener(setInBtn, "click", (e: any) => {
                    stop(e);
            ...
            unsubs.push(
                safeAddListener(setOutBtn, "click", (e: any) => {
                    stop(e);
            ...
            unsubs.push(
                safeAddListener(inInput, "change", (e: any) => {
                    stop(e);
                    try {
            ...
            unsubs.push(
                safeAddListener(outInput, "change", (e: any) => {
                    stop(e);
                    try {
            ...
            unsubs.push(
                safeAddListener(stepInput, "change", (e: any) => {
                    stop(e);
                    try {
            ...
            unsubs.push(
                safeAddListener(fpsInput, "change", (e: any) => {
                    stop(e);
                    try {
            ...
            unsubs.push(
                safeAddListener(inLabel, "click", (e: any) => {
                    stop(e);
                    resetInToStart();
                }),
            );
            unsubs.push(
                safeAddListener(outLabel, "click", (e: any) => {
                    stop(e);
                    resetOutToEnd();
                }),
            );
            unsubs.push(
                safeAddListener(rangeCountLabel, "click", (e: any) => {
                    stop(e);
                    resetPlayerAll();
                }),
            );
        ...
        unsubs.push(
            safeAddListener(muteBtn, "click", (e: any) => {
                stop(e);
                try {
        ...
        unsubs.push(
            safeAddListener(muteBtn, "contextmenu", (e: any) => {
                preventStop(e);
                try {
            ...
            safeAddListener(
                ...
                (e) => {
                    try {
                        ...
                        if (muteBtn.contains?.(e?.target as Node) || volumeWrap.contains?.(e?.target as Node))
                            return;
        ...
        if (volume) {
            unsubs.push(
                safeAddListener(volume, "input", (e: any) => {
                    stop(e);
                    try {
        ...
        unsubs.push(
            safeAddListener(speedSelect, "change", (e: any) => {
                stop(e);
                try {
        ...
        const playbackUiSync: Record<string, any> = {
            rafId: null,
            rvfcId: null,
        };
        ...
        const tickPlaybackUiSync = (_now = 0, meta: any = null) => {
            playbackUiSync.rafId = null;
            playbackUiSync.rvfcId = null;
            try {
        ...
        const setMediaInfo = (info: Record<string, any> = {}) => {
            let oldMax = 0;
        ...
        if (advanced) {
            const drag: Record<string, any> = { active: false, which: null, pointerId: null, ac: null, captureEl: null };

            const frameFromClientX = (clientX: any) => {
                try {
                    const rect = seekWrap.getBoundingClientRect();
            ...
            const startDrag = (e: any, which: any) => {
                preventStop(e);
                try {
                    drag.ac?.abort?.();
                } catch (e: any) {
            ...
            unsubs.push(
                safeAddListener(inHandle, "pointerdown", (e: any) => startDrag(e, "in"), {
                    passive: false,
                }),
            );
            unsubs.push(
                safeAddListener(outHandle, "pointerdown", (e: any) => startDrag(e, "out"), {
                    passive: false,
                }),
            );
            // Allow dragging the thin In/Out markers too (Nuke-like).
            unsubs.push(
                safeAddListener(inMark, "pointerdown", (e: any) => startDrag(e, "in"), {
                    passive: false,
                }),
            );
            unsubs.push(
                safeAddListener(outMark, "pointerdown", (e: any) => startDrag(e, "out"), {
                    passive: false,
                }),
            );

...
type WidgetChangeHandler = (newValue: unknown) => void;
 ...
 * @param {(newValue: any) => void} onChange  Called when the user edits the input
 * @param {object|null} [node]  Parent LGraphNode — passed to widget.callback for proper ComfyUI compat
 * @returns {HTMLElement}
 */
export function createWidgetInput(
    widget: WidgetRecord,
    onChange?: WidgetChangeHandler,
    node: LooseRecord | null = null,
): HTMLElement {
...
export function writeWidgetValue(
    widget: WidgetRecord | null | undefined,
    newValue: any,
    node: LooseRecord | null = null,
): boolean {
 ...
 */
function _syncWidgetDomElement(widget: WidgetRecord) {
    ...
    const el =
        ...
        null;
    const valueEl = el as { value?: unknown } | null;
    if (valueEl != null && "value" in valueEl && valueEl.value !== strVal) {
        valueEl.value = strVal;
    }
}
...
function _textInput(
    ...
    const wrapper: AutoFitElement<HTMLDivElement> = document.createElement("div");
    wrapper.className = "mjr-ws-text-wrapper";

    const textarea: AutoFitElement<HTMLTextAreaElement> = document.createElement("textarea");
...
function _readonlyInput(widget: WidgetRecord): HTMLInputElement {
    const input = document.createElement("input");
    input.type = "text";

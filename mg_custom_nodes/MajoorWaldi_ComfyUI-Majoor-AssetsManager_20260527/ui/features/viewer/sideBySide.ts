...
function _extractGridGenInfo(asset: any) {
 ...
 */
function _buildGridCompactOverlay(asset: any, label: any) {
 ...
 * the other.
 */
function _setupModel3DSync(panelA: any, panelB: any) {
    let attempts = 0;
 ...
 */
function _setupModel3DGridSync(sideView: any) {
    let attempts = 0;
    const MAX = 60;
    const tryConnect = () => {
        attempts++;
        const hosts = Array.from(sideView?.querySelectorAll?.(".mjr-model3d-host") || []) as any[];
        const ready = hosts.filter((h: any) => h._mjr3D?.controls);
        if (ready.length < 2) {
            if (attempts < MAX) setTimeout(tryConnect, 50);
            return;
        }
...
export function renderSideBySideView({
    ...
    const assignPlayableRole = (mediaEl: any, role: any) => {
        try {
            const video =
    ...
    if (count > 2 && !hasFilmstripCompare) {
        ...
        if (items.some((a: any) => a && isModel3DAsset(a))) {

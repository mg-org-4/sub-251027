 ...
 * @returns {HTMLElement}
 */
export function createRatingEditor(asset: Record<string, any>, onUpdate?: (...args: any[]) => void): HTMLElement {
    ...
    const setDisabled = (disabled: any) => {
        try {
            for (const btn of container.querySelectorAll?.("button.mjr-rating-star") || []) {
                try {
                    (btn as HTMLButtonElement).disabled = Boolean(disabled);
                } catch (e: any) {
                    console.debug?.(e);
                }
            }
    ...
    try {
        (container as any)._mjrSetRating = (rating: any) => {
 ...
 */
function updateStarColors(container: any, rating: any) {

 ...
 */
function matchesShortcut(e: any, shortcut: any) {
...
export function installGridKeyboard({
    ...
    onSelectionChanged = () => {},
    onAssetChanged = () => {},
}: { gridContainer?: any; getState?: () => Record<string, any>; getSelectedAssets?: () => any[]; getActiveAsset?: () => any; onOpenDetails?: (asset?: any) => void; onOpenTagsEditor?: (asset?: any) => void; onSelectionChanged?: () => void; onAssetChanged?: (asset?: any) => void } = {}): any {
    ...
    let activateGridHandler: any = null;
    let bound = false;
    ...
    const markGridActive = () => {
        ...
        try {
            (window as any).__MJR_LAST_SELECTION_GRID__ = gridContainer;
        } catch (e: any) {
            console.debug?.(e);
        }
    };
    ...
    const getGridColumns = () => {
        try {
            ...
            if (cards.length >= 2) {
                ...
                const sameRow = cards.filter((c: any) => {
                    try {
    ...
    const selectByIds = (ids: any, activeId: any) => {
    ...
    const scrollToAssetId = (assetId: any) => {
    ...
    const handleGridNavigation = (e: any) => {
        ...
        let currentIndex = assets.findIndex((a: any) => String(a?.id || "") === currentId);
        ...
        if (e.shiftKey) {
            ...
            const anchorIndex = assets.findIndex((a: any) => String(a?.id || "") === anchorId);
    ...
    const handleKeydown = async (e: any) => {
                ...
                try {
                    if (selected.length >= 2 && selected.length <= 4) {
                        ...
                        if (Array.isArray(state?.assets) && state.assets.length) {
                            ...
                            const cards = getRenderedCards();
                            allAssets = cards.map((card: any) => card._mjrAsset).filter(Boolean);
                        }
                        const idx = allAssets.findIndex((a: any) => String(a?.id) === String(asset?.id));
                ...
                try {
                    if (typeof gridContainer?._mjrSetSelection === "function") {
                        const remaining = selected
                            .filter((a: any) => !deletedIds.includes(String(a?.id || "")))
                            .map((a: any) => String(a?.id || ""))
                            .filter(Boolean);
        ...
        if (matchesShortcut(e, GRID_SHORTCUTS.SELECT_ALL)) {
            ...
            try {
                if (Array.isArray(state?.assets) && state.assets.length) {
                    ids = state.assets.map((a: any) => String(a?.id || "")).filter(Boolean);
                } else if (typeof gridContainer?._mjrGetAssets === "function") {
                    const list = gridContainer._mjrGetAssets();
                    if (Array.isArray(list))
                        ids = list.map((a: any) => String(a?.id || "")).filter(Boolean);
                }
            } catch (e: any) {
                console.debug?.(e);
            }
            if (!ids.length) {
                try {
                    ids = getRenderedCards()
                        .map((card: any) => String(card?._mjrAsset?.id || ""))
                        .filter(Boolean);
                } catch (e: any) {
                    console.debug?.(e);
                }
        ...
        if (matchesShortcut(e, GRID_SHORTCUTS.DESELECT_ALL)) {
            ...
            try {
                if (typeof gridContainer?._mjrSetSelection === "function") {
                    ...
                    const cards = getRenderedCards().filter((card: any) =>
    ...
    const unbind = () => {
        ...
        try {
            if ((window as any).__MJR_LAST_SELECTION_GRID__ === gridContainer) {
                delete (window as any).__MJR_LAST_SELECTION_GRID__;
            }
        } catch (e: any) {
            console.debug?.(e);
        }
 ...
 */
export function getShortcutDisplay(actionName: string): string {
    const shortcut = (GRID_SHORTCUTS as Record<string, any>)[actionName];

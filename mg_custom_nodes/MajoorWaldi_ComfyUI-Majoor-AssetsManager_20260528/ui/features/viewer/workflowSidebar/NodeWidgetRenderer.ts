...
type AutoFitElement<T extends HTMLElement> = T & { _mjrAutoFit?: () => void };

interface NodeWidgetRendererOptions {
    onLocate?: () => void;
    collapsible?: boolean;
    expanded?: boolean;
...
function _isNoteNode(node: LooseRecord | null | undefined): boolean {
    return NOTE_TYPE_RE.test(String(node?.type || ""));
}

function _getNoteText(node: LooseRecord | null | undefined): string {
...
function _setNoteText(node: LooseRecord | null | undefined, value: string) {
    const props = node?.properties;
    if (props) {
...
function _isOpaqueType(type: string): boolean {
    return UUID_RE.test(type) || HEX_HASH_RE.test(type);
}

function _getNodeTitle(node: LooseRecord | null | undefined): string {
    return _firstText(
        node?.title,
...
function _getNodeDisplayType(
    node: LooseRecord | null | undefined,
    { isSubgraph = false }: { isSubgraph?: boolean } = {},
): string {
...
function _getNodeSecondaryTitle(
    node: LooseRecord | null | undefined,
    displayType: string,
    { isSubgraph = false }: { isSubgraph?: boolean } = {},
): string {
...
export class NodeWidgetRenderer {
    ...
    _node: LooseRecord;
    _onLocate: (() => void) | null;
    _onToggle: ((expanded: boolean) => void) | null;
    ...
    _inputMap: Map<string, HTMLElement>;
    _autoFits: Array<() => void>;
    _noteTextarea: AutoFitElement<HTMLTextAreaElement> | null;
    _subgraphHeaderTitle: string;
    _mjrTreeItemEl?: HTMLElement | null;
     ...
     */
    constructor(node: LooseRecord, opts: NodeWidgetRendererOptions = {}) {
        this._node = node;
    ...
    syncFromGraph() {
        ...
        for (const w of this._node.widgets) {
            ...
            (storedInput as AutoFitElement<HTMLElement> | null)?._mjrAutoFit?.();
            (input as AutoFitElement<HTMLElement> | null)?._mjrAutoFit?.();
        }
    }
    ...
    _render() {
        ...
        locateBtn.addEventListener("click", (event: any) => {
            event.stopPropagation();
            this._locateNode();
        });
        ...
        for (const w of widgets) {
            ...
            const autoFit =
                (inputEl as AutoFitElement<HTMLElement>)._mjrAutoFit ??
                (inputEl.querySelector?.("textarea") as AutoFitElement<HTMLTextAreaElement> | null)
                    ?._mjrAutoFit;
...
function _resolveSyncInput(input: any) {

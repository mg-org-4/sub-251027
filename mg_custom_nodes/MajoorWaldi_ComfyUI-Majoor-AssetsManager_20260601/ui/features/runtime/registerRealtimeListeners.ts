...
function toNullableBool(value: any) {
    if (value === true || value === 1 || value === "1") return true;
    if (value === false || value === 0 || value === "0") return false;
    return null;
}

function hasVectorEmbedding(detail: any) {
    const flags = [
...
function shouldSkipAutoVectorIndex(assetId: any) {
    const now = Date.now();
...
function buildLivePlaceholderAsset(detail: any, { jobId = "" } = {}) {
...
function safeEscapeAssetId(value: any) {
...
function resolveMaintenanceTitle(op: any, t: any) {
...
function buildMaintenanceProgress(op: any, step: any) {
    const operation = String(op || "")
        .trim()
        .toLowerCase();
    ...
    const steps = (flows as Record<string, string[]>)[operation] || flows.db_restore;
...
export async function registerRealtimeListeners({
    ...
    api._mjrExecutedHandler = (event: any) => {
        try {
    ...
    api._mjrExecutionStartHandler = (event: any) => {
        try {
            executionRuntime.handleExecutionStart(event, {
                setCurrentJobId: (jobId: any) => liveStreamModule?.setCurrentJobId(jobId),
            });
    ...
    api._mjrExecutionEndHandler = (event: any) => {
        try {
    ...
    api._mjrStacksUpdatedHandler = (event: any) => {
        try {
    ...
    api._mjrRuntimeStatusHandler = (event: any) => {
        try {
    ...
    api._mjrExecutionCachedHandler = (event: any) => {
        try {
            const detail = event?.detail || {};
            emitRuntimeStatus({
                active_prompt_id:
    ...
    function markRecentAssetUpsert() {
        try {
            (window as any).__mjrLastAssetUpsert = Date.now();
            (window as any).__mjrLastAssetUpsertCount =
                (Number((window as any).__mjrLastAssetUpsertCount || 0) || 0) + 1;
        } catch (e) {
            console.debug?.(e);
        }
    }

    function upsertLiveAssetIntoGrid(grid: any, detail: any, { immediate = false, force = false } = {}) {
        const assetId = String(detail?.id || "").trim();
    ...
    api._mjrNewGenerationOutputHandler = (event: any) => {
        try {
    ...
    api._mjrAssetAddedHandler = (event: any) => {
        try {
    ...
    api._mjrAssetUpdatedHandler = (event: any) => {
        try {
            if (!event?.detail) return;
            const liveEvent = executionRuntime.prepareLiveAssetEvent(event.detail);
            const detail = liveEvent?.detail || event.detail;
    ...
    api._mjrCoreExecutionAssetsReadyHandler = (event: any) => {
        try {
            const detail = event?.detail || {};
    ...
    api._mjrScanProgressHandler = (event: any) => {
        try {
            const detail = event?.detail || {};
    ...
    api._mjrAssetIndexingHandler = (event: any) => {
        try {
            const detail = event?.detail || {};
    ...
    api._mjrAssetIndexedHandler = (event: any) => {
        try {
            const detail = event?.detail || {};
    ...
    api._mjrEnrichmentStatusHandler = (event: any) => {
        try {
            const detail = event?.detail || {};
    ...
    api._mjrDbRestoreStatusHandler = (event: any) => {
        try {
            const detail = event?.detail || {};
            ...
            const msg = (map as Record<string, any>)[step] || String(detail?.message || "");
            if (!msg) return;
            const tone =
    ...
    const assetsDeletedHandler = (event: any) => {
        try {
            const ids = Array.isArray(event?.detail?.ids)
                ? event.detail.ids.map((x: any) => String(x || "")).filter(Boolean)
                : [];

...
let _dbRestoreStatusHandler: any = null;
let _runtimeStatusHandler: any = null;
...
function setMaintenanceActive(active: any) {
    _maintenanceActive = !!active;
    try {
        (globalThis as any)._mjrMaintenanceActive = _maintenanceActive;
    } catch (e) {
        console.debug?.(e);
    }
}
...
function setStatusLines(statusText: any, lines: any, footerText: any = null) {
...
function appendStatusProgressBars(statusText: any, items: any[] = []) {
    if (!statusText) return;
...
function buildStatusProgressItems(counters: any, { totalAssets = 0, withWorkflows = 0, withGenerationData = 0 } = {}) {
    const total = Math.max(0, Number(totalAssets || counters?.total_assets || 0) || 0);
    if (total <= 0) return [];
    const pct = (value: any, denominator = total) => {
...
function setStatusWithHint(statusText: any, mainText: any, hintText: any) {
    if (!statusText) return;
    statusText.replaceChildren();
...
function applyStatusHighlight(section: any, tone = "neutral", options: Record<string, any> = {}) {
    if (!section) return;
                ...
                tone === "error"
                    ...
                    : tone === "warning"
                      ...
                      : tone === "success"
                        ...
                        : "info";
            try {
                comfyToast((messages as Record<string, any>)[tone] || messages.info, level, 1800, { noHistory: true });
            } catch (e) {
                console.debug?.(e);
            }
        }
...
function formatWatcherScopeLabel(scope: any) {
...
function formatWatcherLine(watcher: any, desiredScope = "") {
 ...
 */
export function createStatusIndicator(options: Record<string, any> = {}): any {
    ...
    let restoreDbBtn: any = null;
    const setSingleOption = (text: any) => {
    ...
    const refreshBackups = async (preferredName = "") => {
        try {
            ...
            if (_hasBackupChoices && !matched && backupSelect.childNodes[0]) {
                backupSelect.value = String((backupSelect.childNodes[0] as HTMLOptionElement)?.value || "");
            }
    ...
    restoreDbBtn.style.cssText = `
        ...
        cursor: pointer;
    `;
    restoreDbBtn.onclick = async (event: any) => {
        event.stopPropagation();
    ...
    const setActionLog = (message: any, tone = "neutral") => {
    ...
    backfillBtn.onclick = async (event) => {
        ...
        applyStatusHighlight(section, "info");
        try {
            const backfillOptions: Record<string, any> = {
                scope: backfillScope,
    ...
    deleteDbBtn.onclick = async (event) => {
        ...
        try {
            const res = preserveVectors
                ...
                : await forceDeleteDb();
            if (res?.ok) {
                (globalThis as any)._mjrCorruptToastShown = false;
                statusDot.style.background = "var(--mjr-status-success, #4CAF50)";
                applyStatusHighlight(section, "success");
            } else {
                statusDot.style.background = "var(--mjr-status-error, #f44336)";
    ...
    if (title) {
        title.addEventListener("click", (event) => {
            ...
            if (!body) return;
            const collapsed = (body as HTMLElement).style.display === "none";
    ...
    const onDbRestoreStatus = async (event: any) => {
        try {
    ...
    try {
        ...
        (section as any)._mjrDbRestoreStatusHandler = onDbRestoreStatus;
...
export async function triggerScan(
    ...
    try {
        const now = Date.now();
        const inflight = (globalThis as any)?._mjrScanInFlight;
        ...
        (globalThis as any)._mjrScanInFlight = {
            at: now,
            scope: desiredScope,
    ...
    const clearScanInFlight = () => {
        try {
            if ((globalThis as any)?._mjrScanInFlight) {
                (globalThis as any)._mjrScanInFlight = null;
            }
        } catch (e) {
            console.debug?.(e);
        }
    ...
    if (desiredScope === "input")
        ...
        const picked = desiredCustomRootId ? list.find((r: any) => r?.id === desiredCustomRootId) : null;
...
function createCapabilityBadge(label: any, available: any, hint: any) {
    ...
    `;
    badge.textContent = `${available ? "OK" : "ERR"} ${label}`;
...
function renderCapabilities(section: any, toolAvailability: Record<string, any> = {}, toolPaths: Record<string, any> = {}) {
    if (!section) return;
...
function renderToolsStatusLine(section: any, toolStatus: any = null, fallbackAvailability: Record<string, any> = {}) {
    if (!section) return;
...
function createCapabilityNode({ label, hint }: { label: any; hint: any }, available: any, path: any) {
...
function setBodyCollapsed(section: any, collapsed: any) {
    const body = section.querySelector("#mjr-status-body");
...
function buildDbHealthLine(healthData: any, dbDiagData: any) {
...
function buildIndexHealthLine(counters: any, desiredScope: any) {
    ...
    if (result.ok) {
        ...
        if (result?.code === "INVALID_RESPONSE" && result?.status === 404) {
            ...
            if (isCorrupt) {
                ...
                if (!(globalThis as any)._mjrCorruptToastShown) {
                    (globalThis as any)._mjrCorruptToastShown = true;
...
function createRetryButton(statusDot: any, statusText: any, capabilitiesSection = null, scanTarget = null) {
...
function _writeCachedStatusCounters(counters: any) {
    try {
...
function _collectProgressValues(status: any) {
    const values: any[] = [];
...
function _statusHasActiveWork(counters: any) {
...
function _buildKindCountsLine(counters: any) {
    const parts = [];
...
function _buildStatusSignature(counters: any) {
    if (!counters || typeof counters !== "object") return "";
...
function _hydrateStatusFromCache(statusDot: any, statusText: any, capabilitiesSection: any) {
...
function _advancePollState(currentState: any, counters: any, pollStateMeta: any) {
...
export function setupStatusPolling(
    ...
    section: any,
    capabilitiesSection: any = null,
    onCountersUpdate: ((data: any) => void) | null = null,
    getScanTarget: (() => any) | null = null,
) {
    ...
    try {
        const oldGlobalDispose = (window as any)?.[GLOBAL_POLL_KEY];
    ...
    const scheduleNext = () => {
        const baseMs =
            STATUS_POLL_INTERVALS_MS[pollStateMeta.mode as keyof typeof STATUS_POLL_INTERVALS_MS] ||
    ...
    try {
        window[GLOBAL_POLL_KEY as any] = section._mjrStatusPollDispose;
    } catch (e) {
        console.debug?.(e);
    }
    scheduleNext();

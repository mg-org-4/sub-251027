...
function makeCustomEvent(type: any, detail: any) {
    if (typeof CustomEvent === "function") {
        return new CustomEvent(type, { detail });
    }
    const event: any = typeof Event === "function" ? new Event(type) : { type };
    event.detail = detail;
    return event;
}
...
export class FloatingViewerPromptExecution {
    [key: string]: any;
    constructor(id: any, getApp = () => getRawHostApp()) {
        this.id = String(id || "");
        this.promptApi = null;
    ...
    executing(nodeId: any, step: any, maxSteps: any) {
        if (nodeId == null) {
            this.currentlyExecuting = null;
            return;
        }
...
export class FloatingViewerProgressService extends EventTarget {
    [key: string]: any;
    constructor({
        getApi = (app: any) => getRawHostApi(app),
        getApp = () => getRawHostApp(),
        waitForApi = (options: any) => waitForRawHostApi(options),
    } = {}) {
        super();
        this._getApi = getApi;
        this._getApp = getApp;
    ...
    _attachApiListeners(api: any) {
        ...
        this._attachListener(api, "executing", (event: any) => {
        ...
        this._attachListener(api, "progress", (event: any) => {
        ...
        this._attachListener(api, "execution_cached", (event: any) => {
            const detail = event?.detail || {};
            if (!this.currentExecution) {
                this.currentExecution = this.getOrMakePrompt(detail.prompt_id || detail.promptId);
            }
        ...
        this._attachListener(api, "executed", (event: any) => {
            const detail = event?.detail || {};
            if (!this.currentExecution) {
        ...
        this._attachListener(api, "execution_error", (event: any) => {
            const detail = event?.detail || {};
            if (!this.currentExecution) {
                this.currentExecution = this.getOrMakePrompt(detail.prompt_id || detail.promptId);
            }
    ...
    _attachListener(api: any, type: any, handler: any) {
...
const globalHost = getGlobalHost() as Record<string, any>;
...
export function ensureFloatingViewerProgressTracking(options: Record<string, any> = {}): Promise<any> {
    return floatingViewerProgressService.ensureInitialized(options);
}

function formatProgressText(detail: any) {
...
export function formatFloatingViewerMediaProgressText(detail: any): string {
    const prompt = detail?.prompt || null;
...
function applyFloatingViewerProgressState(viewer: any, detail: any) {
...
function centerFloatingViewerProgressNode(viewer: any, nodeId: any) {
    const safeNodeId = String(nodeId || "").trim();
...
export function buildFloatingViewerProgressBar(viewer: any): HTMLElement {
    ...
    root.addEventListener("pointerdown", (event: any) => {
    ...
    viewer._progressUpdateHandler = (event: any) => {
...
export function buildFloatingViewerMediaProgressOverlay(viewer: any) {
    const root = document.createElement("div");
...
export function disposeFloatingViewerProgressBar(viewer: any) {
    if (viewer?._progressUpdateHandler) {
        try {
            floatingViewerProgressService.removeEventListener(
                PROGRESS_UPDATE_EVENT,

...
function _isAbsoluteOrSpecialUrl(url: any) {
...
function _resolveLoaderFromAsset(asset: any) {
...
export function isModel3DInteractionTarget(target: any) {
    try {
 ...
 */
export function buildModel3DMouseButtons(eventLike: any, MOUSE_ENUM: any) {
 ...
 */
export function createModel3DMediaElement(asset: any, url: any, options: Record<string, any> = {}) {
    ...
    canvas.tabIndex = 0;
    (canvas as any)._mjrDisableViewerTransform = options.disableViewerTransform !== false;
    try {
    ...
    let lightObjects: any[] = []; // [mainDir, backDir, fillRight, fillLeft, bottomDir]
    let lightIntensity = MODEL3D_DEFAULT_LIGHT_INTENSITY;

    // Animation
    let animationMixer: any = null;
    let animationClock: any = null;
    let animationClips: any[] = [];
    let animationActions: any[] = [];
    ...
    const syncNaturalSize = () => {
        try {
            ...
            (canvas as any)._mjrNaturalW = Math.max(1, Math.round(rect.width || canvas.width || 1));
            (canvas as any)._mjrNaturalH = Math.max(1, Math.round(rect.height || canvas.height || 1));
        } catch (e: any) {
            console.debug?.(e);
        }
        try {
    ...
    const setViewportMode = (nextMode: any, { refit = true }: Record<string, any> = {}) => {
        viewportMode =
    ...
    const setupAnimations = (clips: any, object: any) => {
        ...
        animDom.playBtn.addEventListener("click", (e: any) => {
            _stopEvent(e);
            togglePlayPause();
        });
    ...
    const installInteractionShell = () => {
        ...
        const bind = (el: any, name: string, handler: any, opts?: any) => {
        ...
        bind(host, "wheel", (event: any) => _stopEvent(event));
        // Block context menu and drag
        bind(host, "contextmenu", (event: any) => _stopEvent(event, { preventDefault: true }));
        bind(host, "dragstart", (event: any) => _stopEvent(event, { preventDefault: true }));
        bind(host, "dragover", (event: any) => _stopEvent(event, { preventDefault: true }));
        bind(host, "dragleave", (event: any) => _stopEvent(event));
        bind(host, "drop", (event: any) => _stopEvent(event, { preventDefault: true }));
        return () => {
    ...
    const destroy = () => {
        ...
        try {
            if ((host as any)._mjrAxisScene) {
                (host as any)._mjrAxisScene.traverse?.((obj: any) => {
                    try {
                        obj.geometry?.dispose?.();
                    } catch (_: any) {}
                    try {
                ...
                (host as any)._mjrAxisScene = null;
            }
        } catch (e: any) {
            console.debug?.(e);
        }
    ...
    Promise.resolve()
        .then(async () => {
            ...
            if (!PREVIEWABLE_MODEL3D_LOADERS.has(loaderType as string)) {
                setStatus(
                    "3D preview unavailable",
                    `${(loaderType as string).toUpperCase()} is not supported in the embedded viewer.`,
                );
            ...
            try {
                ...
                manager.setURLModifier((requestedUrl: any) => {
                ...
                lightObjects.forEach((l: any) => scene.add(l));
                ...
                // Load model
                const loader = _createLoaderInstance(deps, loaderType as string, manager);
                if (!loader) {
                    setStatus(
                        "3D loader unavailable",
                        `${(loaderType as string).toUpperCase()} loader could not be created.`,
                    );
                    hint.style.display = "none";
                    return;
                }
                ...
                if (loaderType === "obj" && deps.MTLLoader) {
                    try {
                        const mtlUrl = url.replace(/\.obj(\?.*)?$/i, (m: any, q: any) => `.mtl${q || ""}`);
                        const mtlLoader = new deps.MTLLoader(manager);
                        const materials: any = await new Promise((res: any) => {
                ...
                const loaded = await _loadModelContent(loader, loaderType as string, url);
                if (destroyed) return;
                ...
                gridBtn.addEventListener("click", (e: any) => {
                    stopClick(e);
                    if (gridHelper) gridHelper.visible = !gridHelper.visible;
                    syncViewportButtons();
                    scheduleOverlayRedraw();
                });
                cameraModeBtn.addEventListener("click", (e: any) => {
                    stopClick(e);
                    setViewportMode(
                        viewportMode === MODEL3D_VIEW_MODES.PERSPECTIVE
                            ? MODEL3D_VIEW_MODES.ORTHOGRAPHIC

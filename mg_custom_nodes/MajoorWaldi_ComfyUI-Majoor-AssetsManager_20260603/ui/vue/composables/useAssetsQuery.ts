...
function isDefaultBrowseContext(state: Record<string, any> = {}, stableQuery = "*") {
    return (
        state.scope === "output" &&
        !state.collectionId &&
        stableQuery === "*" &&
...
export function createAssetsQueryController({
    ...
    const checkAndAutoLoad = async (initialLoadPromise: any) => {
        try {
            await initialLoadPromise;
        } catch (e) {
            console.debug?.(e);
    ...
    const scheduleAutoLoad = (initialLoadPromise: any, delayMs = 2000) => {
    ...
    const createCountersUpdateHandler = ({
        ...
        const runtimeWindow = (globalThis?.window ?? globalThis) as any;
        let lastKnownScan: any = null;
        let lastKnownIndexEnd: any = null;
        let lastKnownTotalAssets: any = null;
        let hasSeenFirstCounters = false;

        return async (counters: Record<string, any> = {}) => {

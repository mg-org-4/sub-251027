...
function _coerceBool(value: any, fallback = false) {
...
function _normalizeTagCacheKey(raw: any) {
...
function _dedupeTagList(tags: any) {
    const next = [];
    const seen = new Set();
...
try {
    ...
    if (w && !(w as any)[CLIENT_GLOBAL_KEY]) {
        (w as any)[CLIENT_GLOBAL_KEY] = { initialized: true };
...
const _apiFetchClient = createApiFetchClient({
    readObsEnabled: _readObsEnabled as () => boolean,
    readAuthToken: _readAuthToken as () => string,
    ensureWriteAuthToken: ensureWriteAuthToken as () => Promise<string>,
    normalizeWriteAuthFailure: _normalizeWriteAuthFailure,
});
const fetchAPI = _apiFetchClient.fetchAPI;

export async function get(url: string, options: Record<string, any> = {}) {
    return _apiFetchClient.get(url, options);
}

export async function post(url: string, body: unknown, options: Record<string, any> = {}) {
 ...
 */
export async function updateAssetRating(assetId: any, rating: any, options: Record<string, any> = {}) {
 ...
 * Update asset tags
 */
export async function updateAssetTags(assetId: any, tags: any, options: Record<string, any> = {}) {
    const enabled = _readRatingTagsSyncEnabled();
    const asset = assetId && typeof assetId === "object" ? assetId : null;
    const resolvedId = asset ? asset.id : assetId;
    const normalizedId = normalizeAssetId(resolvedId);
 ...
 */
export async function getAssetMetadata(assetId: any, options: Record<string, any> = {}) {
...
export async function getViewerInfo(assetId: any, options: Record<string, any> = {}) {
 ...
 */
export async function getAssetsBatch(assetIds: any, options: Record<string, any> = {}) {
...
export async function getFolderInfo(
    { filepath = "", root_id = "", subfolder = "" }: Record<string, any> = {},
    options: Record<string, any> = {},
) {
    try {
        if ((globalThis as any).__mjrFolderInfoSupported === false) {
            return {
                ok: false,
                data: null,
        ...
        if ((globalThis as any).__mjrFolderInfoSupported == null) {
            const rr = await get("/mjr/am/routes");
            if (rr?.ok && Array.isArray(rr.data)) {
                const hasRoute = rr.data.some(
                    (r: any) => String(r?.path || "").trim() === "/mjr/am/folder-info",
                );
                (globalThis as any).__mjrFolderInfoSupported = !!hasRoute;
                if (!hasRoute) {
                    return {
                        ok: false,
                        data: null,
                ...
                (globalThis as any).__mjrFolderInfoSupported = null;
            }
        }
    } catch (e) {
        console.debug?.(e);
    ...
    try {
        if (!res?.ok && Number(res?.status || 0) === 404) {
            (globalThis as any).__mjrFolderInfoSupported = false;
        }
    } catch (e) {
        console.debug?.(e);
    }

...
function _getRenderedCards(grid: any) {
    try {
...
function _applyFeedSelection(grid: any, ids: any, activeId = "") {
...
function _bindFeedSelection(host: any) {
    ...
    const onClick = (event: any) => {
        ...
        try {
            (window as any).__MJR_LAST_SELECTION_GRID__ = grid;
        } catch (e) {
            console.debug?.(e);
        }
...
function _getAssetGroupKey(asset: any) {
...
function _groupFeedAssets(assets: any) {
    ...
    for (const entry of groups.values()) {
        entry.members.sort((a: any, b: any) => {
...
function _getHostAssets(host: any) {
    return Array.from(host?.assetsByKey?.values?.() || []) as any[];
}

function _normalizeAssetIdentityPart(value: any) {
    return String(value ?? "")
        .trim()
        .toLowerCase();
}

function _isLivePlaceholderAsset(asset: any) {
    return (
...
function _clearLivePlaceholderState(asset: any) {
...
export function getFeedAssetIdentityKey(asset: Record<string, any> | null): string {
...
export function mergeFeedAssetIntoMap(assetsByKey: Map<string, any>, asset: Record<string, any> | null): Record<string, any> | null {
    ...
    const merged = existing ? { ...(existing as Record<string, any>), ...normalized } : normalized;
...
function _openGroupViewer(host: any, groupedAsset: any, startIndex = 0) {
    ...
    if (host?.grid && activeId) {
        try {
            (window as any).__MJR_LAST_SELECTION_GRID__ = host.grid;
        } catch (e) {
            console.debug?.(e);
        }
...
function _decorateFeedCard(host: any, card: any, groupedAsset: any) {
...
function _enhanceGroupedCards(host: any, groupedAssets: any) {
    const groupedById = new Map<string, any>(
        (Array.isArray(groupedAssets) ? groupedAssets : [])
            .map((asset): [string, any] => [String(asset?.id || "").trim(), asset])
            .filter(([id]) => !!id),
    );
...
async function _renderHostAssets(host: any) {
    if (!host?.grid) return;
    try {
...
function _detectKindFromExt(filename: any) {
...
function _normalizeAsset(asset: any) {
...
function _rememberPendingAsset(asset: any) {
    const normalized = _normalizeAsset(asset);
    if (!normalized) return null;
    const key = _assetKey(normalized);
    if (!key) return null;
    const identity = getFeedAssetIdentityKey(normalized);
    let identityExisting: any = null;
    if (identity) {
...
async function _applyPendingAssets(host: any) {
    if (!host?.grid) return;
...
async function _loadHistory(host: any) {
    if (!host?.grid) return;
    host.empty.style.display = "none";
...
async function _refreshAllHosts() {
    for (const host of Array.from(FEED_STATE.hosts) as Record<string, any>[]) {
        try {
...
export function refreshGeneratedFeedHosts(): void {
    void _refreshAllHosts();
}

function _buildEmpty(host: any) {
...
function _prepareBottomPanelContainer(container: any) {
    if (!container) return;
...
function _applyFeedSettingsClasses(grid: any) {
    if (!grid) return;
    const toggle = (cls: any, enabled: any) => {
...
function _buildGrid(host: any, externalGridWrapper = null) {
    const wrapper =
        externalGridWrapper ||
        (() => {
    ...
    const { app } = createMjrApp(VirtualAssetGridHost, {
        ...
        onCardRendered: (card: any, asset: any) => {
            try {
    ...
    const grid = (vm as any)?.gridContainer || mountTarget.querySelector?.(".mjr-grid") || null;
    host.gridApp = app;
    host.gridVm = vm;
    host.grid = grid;
    if (!grid) return;
    ...
    grid.addEventListener("keydown", (event: any) => {
...
function _getFeedSelectedAssets(host: any) {
    const grid = host?.grid;
    if (!grid) return [];
...
function _getFeedActiveAsset(host: any) {
    const grid = host?.grid;
    if (!grid) return null;
...
function _bindFeedKeyboard(host: any) {
    const grid = host?.grid;
    if (!grid) return;
...
function _applyFeedMinSize(host: any, rawSize: any) {
...
function _buildSizeControl(host: any) {
...
function _makeHost(container: any, external = {}) {
    ...
    _buildEmpty(host);
    _buildGrid(host, (external as any).gridWrapper ?? null);
...
export function pushGeneratedAsset(asset: Record<string, any> | null): void {
...
export function createGeneratedFeedHost(container: HTMLElement, external: Record<string, any> = {}): Record<string, any> {
    _prepareBottomPanelContainer(container);
    const host = _makeHost(container, external);
    try {
        (container as any)._mjrGeneratedFeedHost = host;
    } catch (e) {
        console.debug?.(e);
    }
...
export function disposeGeneratedFeedHost(host: Record<string, any> | null): void {
    const resolvedHost = host?._mjrGeneratedFeedHost || host || null;
    if (!resolvedHost) return;
    const container = resolvedHost.container || null;
    if (container) {
        try {
            delete (container as any)._mjrGeneratedFeedHost;
        } catch (e) {
            try {
                (container as any)._mjrGeneratedFeedHost = null;
            } catch (err) {
                console.debug?.(err);
            }
            console.debug?.(e);
...
export function getGeneratedFeedBottomPanelTab(): Record<string, any> {
    return {
        id: FEED_TAB_ID,
        title: t("bottomFeed.title", "Generated Feed"),
        icon: "pi pi-images",
        type: "custom",
        render: (el: any) => createGeneratedFeedHost(el),
        destroy: (hostOrEl: any) => disposeGeneratedFeedHost(hostOrEl),
    };
}

...
export const APP_DEFAULTS = Object.freeze({
    ...
    // DB tuning (user-settable via advanced settings)
    DB_TIMEOUT_MS: 5000,
    DB_MAX_CONNECTIONS: 10,
    DB_QUERY_TIMEOUT_MS: 1000,
    // Search limit (user-settable via settings)
...
type WideConfig<T> = { -readonly [K in keyof T]: T[K] extends number ? number : T[K] extends boolean ? boolean : T[K] extends string ? string : T[K] };

// Runtime config (some values are user-tunable via settings).
export const APP_CONFIG: WideConfig<typeof APP_DEFAULTS> & Record<string, any> = {
    ...APP_DEFAULTS,
};

export type AppConfig = WideConfig<typeof APP_DEFAULTS>;

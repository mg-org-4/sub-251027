...
export function getFilenameKey(filename: any): string {
    try {
...
export function getExtUpper(filename: any): string {
    try {
        return (
            String(filename || "")
                .split(".")
...
export function getStemLower(filename: any): string {
    try {
 ...
 */
function isVideoWithAudio(asset: any) {
    try {
 ...
 */
function getStackRepPriority(asset: any) {
    try {
        const kind = String(asset?.kind || "").toLowerCase();
        let kindPriority = 0;
 ...
 */
export function selectStackRepresentative(assets: unknown[]): any {
...
export function preserveRepresentativeGenerationTime(primary: any, members: any[]): any {
...
export function detectKind(asset: any, extUpper: string): string {
...
export function isHidePngSiblingsEnabled(loadMajoorSettings: () => Record<string, any>): boolean {
    try {
...
export function setCollisionTooltip(card: HTMLElement | null, filename: string, count: number | null): void {
    ...
    try {
        ...
        if (row) {
            (row as HTMLElement).title = `${String(filename || "")}\nName collision: ${n} items in this view`;
            return;
        }
    } catch (e: any) {
        console.debug?.(e);
...
export function buildCollisionPaths(bucket: any[] | null): string[] {
    const out = [];
    const seen = new Set();
...
function getSiblingContextKey(asset: any) {
...
function getFilenameCollisionKey(asset: any) {
    const filename = getFilenameKey(asset?.filename);
    if (!filename) return "";
    return `${getSiblingContextKey(asset)}|${filename}`;
}

function getSiblingMatchKey(asset: any, extUpper = getExtUpper(asset?.filename || "")) {
...
function ensureSiblingState(state: any) {
...
function removeExistingHiddenSiblings(state: any, matchKey: any, siblingMaps: any, predicate: any) {
...
export function shouldHideSiblingAsset(asset: any, state: any, loadMajoorSettings: () => Record<string, any>): { hidden: boolean; hideEnabled: boolean; removed: unknown[] } {
    ...
    if (kind === "video" || kind === "audio" || kind === "model3d" || extUpper === "WEBP") {
        ...
        const removed = removeExistingHiddenSiblings(
            ...
            matchKey,
            siblingMaps,
            (item: any) => getExtUpper(item?.filename || "") === "PNG",
        );
...
export function appendAssets(gridContainer: any, assets: any[], state: any, deps: any): any {
    ...
    const applyFilenameCollisions = () => {
        try {
            ...
            for (const [key, bucket] of filenameToAssets.entries()) {
                const visibleBucket = (Array.isArray(bucket) ? bucket : []).filter((asset: any) => {
                ...
                const secondaries = visibleBucket.filter((a: any) => a !== primary);
                ...
                const merged = [
                    ...prevMembers,
                    ...visibleBucket.filter((a: any) => !prevIds.has(String(a?.id || ""))),
                ];
                ...
                const secondaryIds = new Set(secondaries.map((a: any) => String(a?.id || "")));
                if (secondaryIds.size > 0) {
                    state.assets = state.assets.filter(
                        (a: any) => !secondaryIds.has(String(a?.id || "")),
                    );
                }
    ...
    if (assetsToRemoveFromState.size > 0) {
        ...
        state.assets = state.assets.filter((a: any) => !assetsToRemoveFromState.has(a));
...
export function updateGridSettingsClasses(container: any, deps: any): any {

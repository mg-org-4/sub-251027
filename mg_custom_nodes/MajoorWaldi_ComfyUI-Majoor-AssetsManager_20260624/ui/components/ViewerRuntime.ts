...
let _mod_sideBySide: any = null;
let _mod_model3d: any = null;
let _mod_scopes: any = null;
let _mod_genInfo: any = null;
let _mod_frameExport: any = null;

function _prewarmViewerModules() {
    if (!_mod_abCompare)
        import("../features/viewer/abCompare.js").then((m: any) => {
            _mod_abCompare = m;
        });
    if (!_mod_sideBySide)
        import("../features/viewer/sideBySide.js").then((m: any) => {
            _mod_sideBySide = m;
        });
    if (!_mod_model3d)
        import("../features/viewer/model3dRenderer.js").then((m: any) => {
            _mod_model3d = m;
        });
    if (!_mod_scopes)
        import("../features/viewer/scopes.js").then((m: any) => {
            _mod_scopes = m;
        });
    if (!_mod_genInfo)
        import("../features/viewer/genInfo.js").then((m: any) => {
            _mod_genInfo = m;
        });
    if (!_mod_frameExport)
        import("../features/viewer/frameExport.js").then((m: any) => {
            _mod_frameExport = m;
        });
}
// -----------------------------------------------------------------------------
...
function createViewer() {
    ...
    function setZoom(next: any, opts?: any) {
        try {
    ...
    function getMediaNaturalSize(mediaEl: any) {
        try {
    ...
    function updateVisibleAudioVisualizerMode() {
        ...
        try {
            ...
            for (const canvas of canvases) {
                try {
                    const proc = (canvas as any)?._mjrProc || null;
    ...
    let filenameRight: any = null;
    let badgesBarRight: any = null;
    let rightMeta: any = null;
    let rightArea: any = null;
    let leftArea: any = null;
    let leftMeta: any = null;
    let centerArea: any = null;
    let titleLine: any = null;
    let titleWrap: any = null;
    let modeButtonsEl: any = null;
    let headerTop: any = null;
    try {
    ...
    installViewerLightDismiss({
        overlay,
        requestClose: () => _requestCloseFromButton(),
    });
    ...
    const _viewerInfoCacheGet = (id: any) => {
        try {
            const key = String(id ?? "");
            if (!key) return null;
    ...
    const _viewerInfoCacheSet = (id: any, data: any) => {
        try {
            const key = String(id ?? "");
            if (!key || !data) return;
    ...
    const clearCanvas = (ctx: any, w: any, h: any) => {
        try {
    ...
    let _overlayRAF: any = null;
    function scheduleOverlayRedraw(immediate?: any) {
        try {
    ...
    const clearGenInfoBody = (target: any) => {
        if (!target) return;
        try {
            for (const child of Array.from(target.childNodes || [])) {
                try {
                    (child as any)?._mjrDispose?.();
                } catch (e: any) {
                    console.debug?.(e);
                }
            }
    ...
    const ensureGenInfoAsset = async (asset: any, { signal }: Record<string, any> = {}) => {
        try {
    ...
    const shouldShowGenInfoLoading = (asset: any) => {
        try {
    ...
    const renderGenInfoPanel = async () => {
        ...
        const renderNow = ({ left = null, leftExtra = null, right = null, rightExtra = null, single = null }: Record<string, any> = {}) => {
            ...
            const addBlockTo = (targetBody: any, title: any, assetObj: any, loading: any) => {
        ...
        try {
            ...
            let assetLeft: any = null;
            let assetRight: any = null;
            let assetC: any = null;
            let assetD: any = null;
            let assetSingle: any = null;
            ...
            const getCached = (a: any) => (a ? metadataHydrator?.getCached?.(a.id)?.data || a : null);
    ...
    let _frameExporterInst: any = null;
    function exportCurrentFrame(opts: any) {
        try {
    ...
    const renderBadges = () => {
        ...
        const makePill = (asset: any, { showName }: Record<string, any> = {}) => {
    ...
    const isDefaultGrade = (params: any) => {
        try {
    ...
    const navigateViewerAssets = (direction: any) => {
    ...
    const positionOverlayBox = (el: any, clientX: any, clientY: any, { offsetX = 16, offsetY = 16 } = {}) => {
        try {
    ...
    try {
        if (!content._mjrOverlayResizeBound && "ResizeObserver" in window) {
            try {
                (overlay as any)._mjrResizeObserver?.disconnect?.();
            } catch (e: any) {
                console.debug?.(e);
            }
            ...
            (overlay as any)._mjrResizeObserver = ro;
            lifecycleUnsubs.push(() => {
                try {
                    ro.disconnect();
                } catch (e: any) {
    ...
    const bindOpenListeners = () => {
        ...
        try {
            _openUnsubs.push(
                safeAddListener(overlay, "click", (e: any) => {
                    try {
                ...
                safeAddListener(
                    ...
                    (e) => {
                        try {
                            ...
                            if ((e as any).changedTouches?.length !== 1) {
                                _touchStart = null;
                                return;
                            }
        ...
        try {
            _openUnsubs.push(
                safeAddListener(
                    ...
                    (e) => {
                        try {
                            state._lastPointerX = (e as any).clientX;
                            state._lastPointerY = (e as any).clientY;
                        } catch (e: any) {
                            console.debug?.(e);
                        }
                    },
    ...
    try {
        if (!(overlay as any)._mjrBadgeSyncBound) {
            const onRatingSync = (e: any) => {
                try {
            ...
            (overlay as any)._mjrBadgeSyncBound = true;
        }
    } catch (e: any) {
        console.debug?.(e);
    }
    ...
    // Public API
    const api = {
        open(assets: any, startIndex = 0, compareAsset = null) {
            bindOpenListeners();
        ...
        dispose() {
            ...
            try {
                (overlay as any)._mjrResizeObserver = null;
            } catch (e: any) {
                console.debug?.(e);
            }
            try {
            ...
            try {
                for (const u of (overlay as any)._mjrViewerUnsubs || []) safeCall(u);
            } catch (e: any) {
                console.debug?.(e);
            }
            try {
                (overlay as any)._mjrViewerUnsubs = [];
            } catch (e: any) {
                console.debug?.(e);
            }
            try {
    ...
    (overlay as any)._mjrViewerAPI = api;
    ...
    try {
        bindViewerContextMenu({
            ...
            getCurrentViewUrl: (asset: any) => buildAssetViewURL(asset),
            onAssetChanged: () => {
                try {
                    renderBadges();
                } catch (e: any) {
 ...
 */
export function getViewerInstance(): any {

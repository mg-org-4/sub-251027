...
export async function mountAssetsManagerPanelRuntime(
    ...
    { useComfyThemeUI = true, external = {} }: { useComfyThemeUI?: boolean; external?: Record<string, any> } = {},
): Promise<unknown> {
    return _mountPanelRuntimeImpl(container, { useComfyThemeUI, external });
}

async function _mountPanelRuntimeImpl(container: any, { useComfyThemeUI = true, external = {} }: { useComfyThemeUI?: boolean; external?: Record<string, any> } = {}) {
    ...
    const readPanelValue = (key: string, fallback: any = "") =>
        panelStateBridge.read(key, (state as any)?.[key] ?? fallback);
    const writePanelValue = (key: any, value: any) => panelStateBridge.write(key, value);
    ...
    const summaryBarDisposers: any[] = [];
    const registerSummaryDispose = (fn: any) => {
    ...
    if (typeof external?.onGridContainerReady === "function") {
        ...
        container._mjrGridContextMenuUnbind = bindGridContextMenu({
            gridContainer,
            getState: () => state as Record<string, any>,
        });
    }
    ...
    let _dupPollTimer: any = null;
    let _autoLoadTimer: any = null;
    let browserNav: any = null;
    ...
    if (!hasVueGridHostState) {
        selectionState = bindGridSelectionState({
            ...
            readActiveAssetId: () => readActiveAssetId(),
            writeSelectedAssetIds: (ids: any) => writePanelValue("selectedAssetIds", ids),
            writeActiveAssetId: (id: any) => writePanelValue("activeAssetId", id),
            onSelectionChanged: () => {
                markUserInteraction();
    ...
    try {
        window.addEventListener(
            ...
            async (e: Event) => {
                const preferredId = String((e as CustomEvent)?.detail?.preferredId || "").trim() || null;
    ...
    const reconcileVisibleSelection = () => {
        try {
            ...
            const visibleIds = (Array.isArray(getAssets()) ? getAssets() : [])
                .map((asset: any) => String(asset?.id ?? "").trim())
                .filter(Boolean);
    ...
    Object.values(tabButtons as Record<string, any>).forEach((btn) => {
    ...
    const assetsQueryOptions = {
        ...
        restoreGridUiState: hasVueGridHostState
            ...
            : null,
        readScrollTop: () => panelStateBridge.read("scrollTop", (state as any).scrollTop || 0),
        restoreSelectionState: ({ scrollTop }: { scrollTop?: number } = {}) => {
        ...
        getQuery,
        getScope: () => panelStateBridge.read("scope", (state as any).scope),
        loadAssets: loadAssetsFn,
        lifecycleSignal: panelLifecycleAC?.signal || null,
    };
    ...
    let disposeSearchBinding: any = null;
    ...
    try {
        didHydrateFromSnapshot = await hydrateGridFromSnapshotFn(
            gridContainer,
            {
                ...
                workflowId: readPanelValue("workflowId", state.workflowId || "") || "",
                workflowModel: readPanelValue(
                    "workflowModelFilter",
                    state.workflowModelFilter || "",
                ) || "",
                workflowRunsOn: readPanelValue(
                    "workflowRunsOnFilter",
                    state.workflowRunsOnFilter || "",
                ) || "",
    ...
    try {
        (window as any).__mjrLastAssetUpsert = 0;
        (window as any).__mjrLastAssetUpsertCount = 0;
    } catch (e) {
        console.debug?.(e);
    }

    const renderDuration = endTimer("panelRender", "gridRender");
    trackGridRender(renderDuration ?? 0);

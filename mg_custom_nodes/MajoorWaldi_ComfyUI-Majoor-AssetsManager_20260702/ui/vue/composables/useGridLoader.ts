...
function getStableRealtimeAssetKey(asset: any) {
...
function coerceQueryText(query: any) {
...
function resolveElement(maybeRef: any) {
...
function formatLoadErrorMessage(prefix: any, err: any) {
...
function getAdaptivePageLimit(baseLimit: any, emptyAppendBatches: any) {
...
export function useGridLoader({
    ...
    let deferredExecutionReload: any = null;
    ...
    function recordOperation(name: any, detail: Record<string, any> = {}) {
    ...
    function classifyContextResetReason(nextContext: any) {
    ...
    function resolveResetReason(query: any, options: Record<string, any> = {}) {
    ...
    function canResetImmediately(reason: any) {
        return IMMEDIATE_RESET_REASONS.has(String(reason || "").toLowerCase());
    }

    function rememberCurrentGridContext(context: any = null) {
        try {
    ...
    function isExecutionBusy() {
        ...
        try {
            return !!String((window as any).__MJR_EXECUTION_RUNTIME__?.active_prompt_id || "").trim();
        } catch (e) {
            console.debug?.(e);
            return false;
        }
    ...
    function scheduleDeferredExecutionReload(query: any, options = {}) {
    ...
    function assetKey(asset: any, gridContainer = null) {
        if (!asset || typeof asset !== "object") return "";
        const fallback =
            asset.id != null
                ? `id:${asset.id}`
        ...
        try {
            const grid = gridContainer || getGridContainer() || (globalThis as any)?.__MJR_LAST_ASSETKEY_GRID__;
    ...
    function appendAssets(gridContainer: any, assets: any) {
        const startedAt = nowMs();
        const receivedCount = Array.isArray(assets) ? assets.length : 0;
        const addedCount: any = cardAppendAssets(gridContainer, assets, state, {
            loadMajoorSettings,
            clearGridMessage: () => {
                clearStatusMessage();
            },
            ...
            ensureDupStackCard: (gc: any, card: any, asset: any) => ensureDupStackCard(gc, card, asset),
        });
     ...
     */
    function buildEarlyFetchMatchKey(gridContainer: any, query: any) {
    ...
    function isDefaultOutputBrowseContext(query: any) {
    ...
    function normalizePageForPagination(page: any, { query = "*", limit = 0, offset = 0 } = {}) {
    ...
    async function fetchPage(query: any, limit: any, offset: any, { requestId = 0, signal = null as AbortSignal | null } = {}) {
        const gridContainer = getGridContainer();
        if (!gridContainer) {
        ...
        const page = await fetchGridPage(
            ...
            { requestId, signal: (signal ?? undefined) as undefined, cursor: state.cursor || null },
        );
        ...
        collection: {
            append(assets = []) {
                ...
                return gridContainer && assets.length
                    ? Number((appendAssets(gridContainer, assets) as any) || 0) || 0
                    : 0;
            },
            reset() {},
        },
        fetchPage: async ({ query, limit, offset, requestId }: { query: any; limit: any; offset: any; requestId: any }) => {
    ...
    function setLegacyPageState(nextState: Record<string, any> = {}, { syncPaged = true }: Record<string, any> = {}) {
    ...
    function finalizeLoad({ title = "", showEmptyMessage = true }: Record<string, any> = {}) {
        clearLoadingMessage();
     ...
     */
    let _headRefreshTimer: any = null;
    ...
    async function loadAssets(query = "*", options: Record<string, any> = {}) {
        ...
        try {
            (globalThis as any).__MJR_LAST_ASSETKEY_GRID__ = gridContainer;
        } catch (e) {
            console.debug?.(e);
        }
    ...
    async function loadAssetsFromList(assets: any, options: Record<string, any> = {}) {
        const {
            title = "Collection",
            reset = true,
            showLoading = true,
    ...
    function removeAssets(assetIds: any, { updateSelection = true } = {}) {
        ...
        if (updateSelection) {
            const visibleIds = state.assets.map((asset: any) => String(asset?.id || "")).filter(Boolean);
            reconcileSelection(visibleIds, { activeId: state.activeId });
        }
    ...
    function captureAnchor() {
        ...
        const isCardVisibleInHost = (card: any) => {
        ...
        const firstVisible = visibleCards.find((card: any) => isCardVisibleInHost(card)) || visibleCards[0] || null;
    ...
    async function restoreAnchor(anchor: any) {
        const gridContainer = getGridContainer();
        const scrollElement = readScrollElement();
    ...
    async function hydrateFromSnapshot(parts: Record<string, any> = {}, options: Record<string, any> = {}) {
        ...
        setLegacyPageState({
            ...
            cursor: (snapshot as any).cursor || null,
            total: Number(snapshot.total ?? snapshot.assets.length) || snapshot.assets.length,
            done: !!snapshot.done,
        });
    ...
    function _buildUpsertDeps(gridContainer: any) {
        return {
            ...
            assetKey: (asset: any) => getStableRealtimeAssetKey(asset) || assetKey(asset, gridContainer),
            loadMajoorSettings,
        };
    }

    function upsertAsset(asset: any) {
    ...
    function upsertAssetNow(asset: any) {

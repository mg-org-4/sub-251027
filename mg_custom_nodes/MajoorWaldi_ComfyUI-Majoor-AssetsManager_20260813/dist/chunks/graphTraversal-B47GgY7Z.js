...
function l(n) {
	let i = n?.graph || n?.canvas || n?.rootGraph ? e(n) : n, l = [], u = /* @__PURE__ */ new Set(), d = [];
	for (a(d, u, i, "Workflow"); d.length;) {
		let e = d.pop();
		if (e) {
			l.push(e);
			for (let t of o(e.graph)) a(d, u, t, `${e.label} / ${r(t, "Subgraph")}`);
			for (let n of t(e.graph)) for (let t of s(n)) a(d, u, t, `${e.label} / ${String(n?.title || n?.type || "Subgraph").trim()}`);
		}
	}
	for (let e of c(i)) a(l, u, e.graph, e.label);
	return l;
}
function u(e, n) {
	for (let r of l(e)) for (let [e, a] of t(r.graph).entries()) {
		let t = String(a?.id ?? a?.ID ?? e).trim() || String(e);
		n({
			node: a,
			graph: r.graph,
			label: r.label,
			locatorId: i(a, r.graph),
...
//#endregion
export { e as a, t as i, d as n, s as o, n as r, u as s, l as t };

...
export function getRtHydrateState(gridContainer: any, mapRef: Map<unknown, unknown>): any {
    try {
...
export function pruneRatingTagsHydrateSeen(st: any, max: number, ttlMs: number, budgetRef: any): void {
...
function normalizeHydratedTags(rawTags: any) {
    if (Array.isArray(rawTags)) return rawTags.filter((t: any) => String(t ?? "").trim());
...
function hasVueScopeAttr(el: any) {
    try {
...
export function updateCardRatingTagsBadges(card: any, rating: any, tags: any, deps: any): void {
    if (!card) return;
    try {
...
export function pumpRatingTagsHydration(gridContainer: any, deps: any): void {
...
export function enqueueRatingTagsHydration(gridContainer: any, card: any, asset: any, deps: any): void {

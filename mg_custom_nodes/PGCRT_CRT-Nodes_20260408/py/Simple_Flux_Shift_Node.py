...
class SimpleFluxShiftNode:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "intensity": (
                    intensity_options,
                    {"default": "0", "tooltip": "Distance from 0 determines effect strength"},
                ),
            ...
            "optional": {
                "latent": ("LATENT", {"tooltip": "Primary resolution source - overrides image input"}),
                "image": ("IMAGE", {"tooltip": "Fallback resolution source when latent not connected"}),
            },
        }
    ...
    def apply_shift_with_multiplier(self, model, intensity, resolution_multiplier, latent=None, image=None):
        ...
        colored_print(f"  📐 Resolution source: {source_type} ({width}x{height})", Colors.BLUE)
        colored_print(
            f"  📈 Effective resolution: {effective_width}x{effective_height} (x{resolution_multiplier})", Colors.BLUE
        )

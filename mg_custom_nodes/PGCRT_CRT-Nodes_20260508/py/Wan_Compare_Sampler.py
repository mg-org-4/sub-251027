...
class Log:
    ...
    @staticmethod
    def info(message):
        print(f"{Log.CYAN}{message}{Log.ENDC}")

    @staticmethod
    def success(message):
        print(f"{Log.GREEN}{message}{Log.ENDC}")

    @staticmethod
    def fail(message):
        print(f"{Log.FAIL}{message}{Log.ENDC}")
    ...
    @staticmethod
    def debug(message):
        print(f"{Log.CYAN}[DEBUG] {message}{Log.ENDC}")
...
def parse_lora_configs(config_string: str):
    ...
    for line in config_string.strip().split('\n'):
        ...
        parts = line.split('§')
        if len(parts) < 2:
            continue  # Needs at least rows and enabled status
...
def apply_lora_stack(model, lora_list):
    ...
    for lora in lora_list:
        if not lora.get('name') or not lora.get('enabled', True):
            continue
        lora_path = find_lora_path_by_name(lora['name'])
        if lora_path:
            lora_data = comfy.utils.load_torch_file(lora_path, safe_load=True)
            model_clone, _ = comfy.sd.load_lora_for_models(
                model_clone, None, lora_data, lora['strength'], lora['strength']
            )
...
class WAN2_2LoraCompareSampler:
    ...
    def sample(
        ...
        # 1. Handle I2V Image Overrides FIRST (Global Dimension Update)
        if i2v_image is not None:
            if vae is None:
                Log.fail("A VAE is required for I2V mode but was not provided.")
                return (
                    {"samples": torch.zeros([0, 16, 1, 1, 1])},
                    {"samples": torch.zeros([0, 16, 1, 1, 1])},
                    torch.zeros([0, 1, 1, 3]),
                    None,
                    "VAE required for I2V mode.",
                    model_high_noise,
                    model_low_noise,
                )
            Log.info("I2V Image provided. Image mode enabled.")
            _, img_height, img_width, _ = i2v_image.shape
            width, height = img_width, img_height
            Log.info(f"Overriding dimensions to {width}x{height} based on image.")

        # Determine latent channels and scale factor using updated dimensions
        latent_channels = 16
        if vae is not None:
        ...
        if not lora_groups:
            ...
            return (
                {"samples": torch.zeros([0, 4, 1, 1])},
                {"samples": torch.zeros([0, 4, 1, 1])},
                torch.zeros([0, 1, 1, 3]),
                None,
                "All LoRA groups disabled.",
                model_high_noise,
                model_low_noise,
            )
        ...
        custom_label_list = [
            label
            for i, label in enumerate(parse_label_configs(custom_labels))
            if i < len(all_lora_groups) and all_lora_groups[i].get("enabled", True)
        ]

        # Temp model for sigmas calculation
        ...
        del temp_model
        Log.info(
            f"Boundary {boundary} on scheduler '{scheduler}' corresponds to a switch AT step {switching_step}/{steps}."
        )
        ...
        for i, group in enumerate(lora_groups):
            ...
            # --- Per-Group Latent & Conditioning Setup ---
            base_latent = torch.zeros(
                [1, latent_channels, latent_t, latent_h, latent_w], device=comfy.model_management.intermediate_device()
            )
            noise_mask = None

            # Start with global conditioning
            group_positive = positive
            group_negative = negative

            if i2v_image is not None:
                # Upscale image to target resolution
                s_image_upscaled = comfy.utils.common_upscale(
                    i2v_image[:frame_count].movedim(-1, 1), width, height, "bilinear", "center"
                ).movedim(1, -1)

                # IMPORTANT: Pad the pixels to the full frame count first.
                pixel_video = (
                    torch.ones(
                        (frame_count, height, width, 3), device=s_image_upscaled.device, dtype=s_image_upscaled.dtype
                    )
                    * 0.5
                )
                input_frames = s_image_upscaled.shape[0]

                # Always start image
                pixel_video[:input_frames] = s_image_upscaled

                # Encode the FULL video tensor
                encoded_full = vae.encode(pixel_video[:, :, :, :3])

                if encoded_full.ndim == 4:
                    encoded_full = encoded_full.permute(1, 0, 2, 3).unsqueeze(0)

                # --- Wan 2.1 Concat Conditioning ---
                concat_latent_image = encoded_full
                mask_cond = torch.ones(
                    (1, 1, latent_t, latent_h, latent_w), device=base_latent.device, dtype=base_latent.dtype
                )

                # Mask out the frames corresponding to the image (Start)
                frames_in_latent = ((input_frames - 1) // 4) + 1
                mask_cond[:, :, :frames_in_latent] = 0.0

                cond_values = {"concat_latent_image": concat_latent_image, "concat_mask": mask_cond}
                group_positive = conditioning_set_values(group_positive, cond_values)
                group_negative = conditioning_set_values(group_negative, cond_values)

                # --- Wan 2.2 / Latent Inpainting Logic ---
                base_latent = encoded_full
                noise_mask = mask_cond  # Re-use the same mask (0=keep, 1=noise)

                latent_format = None
                if hasattr(model_high_noise, "model") and hasattr(model_high_noise.model, "latent_format"):
                    latent_format = model_high_noise.model.latent_format
                elif latent_channels == 48:
                    latent_format = comfy.latent_formats.Wan22()

                if latent_format is not None:
                    processed_latent = latent_format.process_out(base_latent)
                    base_latent = processed_latent * noise_mask + base_latent * (1.0 - noise_mask)
            ...
            if prompt_override.strip():
                if clip is not None:
                    ...
                    override_cond = [[cond, {"pooled_output": pooled}]]

                    if i2v_image is not None:
                        override_cond = conditioning_set_values(override_cond, cond_values)

                    group_positive = override_cond
                    group_positive_hash = get_conditioning_hash(group_positive)
                else:
            ...
            high_lora_stack = [
                {'name': r['high_name'], 'strength': r['high_strength'], 'enabled': r['enabled']}
                for r in group['rows']
                if r['high_name'] and r['enabled']
            ]
        ...
        for i, group in enumerate(lora_groups):
            ...
            if 'group_positive' in cached_data:
                ...
                group_negative = cached_data['group_negative']
            else:
                prompt_override = group.get("prompt_override", "")
                group_positive = positive
                group_negative = negative
                ...
                if i2v_image is not None:
                    ...
                    pixel_video = (
                        torch.ones(
                            ...
                            device=s_image_upscaled.device,
                            dtype=s_image_upscaled.dtype,
                        )
                        * 0.5
                    )
                    input_frames = s_image_upscaled.shape[0]
            ...
            if not is_bypassed and switching_step < steps:
                low_lora_stack = [
                    {'name': r['low_name'], 'strength': r['low_strength'], 'enabled': r['enabled']}
                    for r in group['rows']
                    if r['low_name'] and r['enabled']
                ]
                ...
                if in_mem_low_cache_key in low_noise_cache:
                    ...
                    low_full_config = {
                        ...
                        'scheduler': scheduler,
                        'sigma_shift': sigma_shift,
                        'positive_hash': get_conditioning_hash(group_positive),
                        'negative_hash': negative_hash,
                    ...
                    if os.path.exists(low_cache_file):
                        try:
                            cached_low_data = torch.load(
                                low_cache_file, map_location=comfy.model_management.intermediate_device()
                            )
                    ...
                    if final_latent is None:
                        ...
                        # Re-calculate noise mask for Low Noise pass
                        low_noise_mask = None
                        if i2v_image is not None:
                            frames_in_latent = ((i2v_image.shape[0] - 1) // 4) + 1
                            low_noise_mask = torch.ones(
                                (1, 1, latent_t, latent_h, latent_w),
                                device=comfy.model_management.intermediate_device(),
                                dtype=torch.float32,
                            )
                            low_noise_mask[:, :, :frames_in_latent] = 0.0
        ...
        final_images_batch = (
            torch.cat(decoded_images_for_output, dim=0)
            if decoded_images_for_output
            else torch.zeros([0, height, width, 3])
        )
        ...
        if final_images_batch.numel() > 0 and create_comparison_grid:
            ...
            if add_labels:
                ...
                _, _, _, bottom = dummy_draw.textbbox(
            ...
            for frame_idx in range(frame_count):
                ...
                for config_idx in range(len(lora_groups)):
                    ...
                    if img_idx < final_images_batch.shape[0]:
                        ...
                        if add_labels:
                            frame_img = add_label_to_image(
                                frame_img,
                                config_labels[config_idx],
                                label_font_size,
                                fixed_label_area_height=max_label_area_height,
                            ).squeeze(0)

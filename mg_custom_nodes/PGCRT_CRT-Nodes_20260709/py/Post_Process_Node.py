...
class CRTPostProcessNode:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "enable_upscale": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_levels": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_color_wheels": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_temp_tint": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_sharpen": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_small_glow": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                "small_glow_intensity": (
                    "FLOAT",
                    {"default": 0.015, "min": 0.0, "max": 2.0, "step": 0.01, "decimals": 3},
                ),
                "small_glow_radius": ("FLOAT", {"default": 0.1, "min": 0.0, "max": 10.0, "step": 0.1, "decimals": 3}),
                "small_glow_threshold": (
                    "FLOAT",
                    {"default": 0.25, "min": 0.0, "max": 1.0, "step": 0.01, "decimals": 3},
                ),
                "enable_large_glow": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                "large_glow_intensity": (
                    "FLOAT",
                    {"default": 0.25, "min": 0.0, "max": 2.0, "step": 0.01, "decimals": 3},
                ),
                "large_glow_radius": (
                    "FLOAT",
                    {"default": 50.0, "min": 30.0, "max": 100.0, "step": 0.5, "decimals": 3},
                ),
                "large_glow_threshold": (
                    "FLOAT",
                    {"default": 0.3, "min": 0.0, "max": 1.0, "step": 0.01, "decimals": 3},
                ),
                "enable_glare": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_chromatic_aberration": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                "ca_strength": ("FLOAT", {"default": 0.005, "min": 0.0, "max": 0.1, "step": 0.001, "decimals": 3}),
                "ca_edge_fallof": ("FLOAT", {"default": 2.0, "min": 0.0, "max": 2.0, "step": 0.01, "decimals": 3}),
                "enable_ca_hue_shift": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                "ca_hue_shift_degrees": (
                    "FLOAT",
                    {"default": 0.0, "min": -180.0, "max": 180.0, "step": 1.0, "decimals": 0},
                ),
                "enable_vignette": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_radial_blur": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                "radial_blur_type": (cls.radial_blur_types, {"default": "spin"}),
                "radial_blur_strength": (
                    "FLOAT",
                    {"default": 0.02, "min": 0.0, "max": 0.5, "step": 0.005, "decimals": 3},
                ),
                "radial_blur_center_x": (
                    "FLOAT",
                    {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.01, "decimals": 3},
                ),
                ...
                "radial_blur_falloff": (
                    "FLOAT",
                    {"default": 0.25, "min": 0.001, "max": 1.0, "step": 0.01, "decimals": 3},
                ),
                "radial_blur_samples": ("INT", {"default": 16, "min": 8, "max": 64, "step": 8, "decimals": 0}),
                "enable_film_grain": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
                ...
                "enable_lens_distortion": ("BOOLEAN", {"default": False, "label_on": "On", "label_of": "Off"}),
    ...
    def _hue_shift_rgb_color_vector(self, rgb_color_tensor, hue_shift_degrees):
        ...
        if 0 <= h_prime < 1:
            r_out, g_out, b_out = c, x, 0
        elif 1 <= h_prime < 2:
            r_out, g_out, b_out = x, c, 0
        elif 2 <= h_prime < 3:
            r_out, g_out, b_out = 0, c, x
        elif 3 <= h_prime < 4:
            r_out, g_out, b_out = 0, x, c
        elif 4 <= h_prime < 5:
            r_out, g_out, b_out = x, 0, c
        elif 5 <= h_prime < 6:
            r_out, g_out, b_out = c, 0, x
    ...
    def _apply_upscale(self, img, current_device, **kwargs):
        ...
        if precision_str == "auto":
            if current_device.type == 'cuda':
                processing_dtype = torch.float16
            ...
            try:
                ...
                def model_function_for_tiled_scale(tile_tensor_bchw):
                    ...
                    if autocast_enabled and upscale_model_execution_device.type == 'cuda':
                        with torch.autocast(
                            device_type=upscale_model_execution_device.type, dtype=processing_dtype, enabled=True
                        ):
    ...
    def _apply_color_wheels(self, img, kwargs):
        lift = torch.tensor(
            [kwargs.get('lift_r', 0.0), kwargs.get('lift_g', 0.0), kwargs.get('lift_b', 0.0)],
            device=img.device,
            dtype=img.dtype,
        )

        gamma_adj = torch.tensor(
            [kwargs.get('gamma_r', 1.0), kwargs.get('gamma_g', 1.0), kwargs.get('gamma_b', 1.0)],
            device=img.device,
            dtype=img.dtype,
        )

        gain = torch.tensor(
            [kwargs.get('gain_r', 1.0), kwargs.get('gain_g', 1.0), kwargs.get('gain_b', 1.0)],
            device=img.device,
            dtype=img.dtype,
        )
    ...
    def _create_star_glare(self, img_bchw, rays, length, angle, h, w, quality, ray_width):
        ...
        convolved_img = F.conv2d(
            img_bchw, kernel.expand(img_bchw.shape[1], 1, -1, -1), padding=kernel_size // 2, groups=img_bchw.shape[1]
        )
    ...
    def _apply_chromatic_aberration(self, img, kwargs):
        ...
        r_channel_shifted = F.grid_sample(
            img_bchw[:, 0:1, ...], grid_r, mode='bilinear', padding_mode='border', align_corners=True
        )
        g_channel_original = img_bchw[:, 1:2, ...]
        b_channel_shifted = F.grid_sample(
            img_bchw[:, 2:3, ...], grid_b, mode='bilinear', padding_mode='border', align_corners=True
        )
        ...
        if enable_hue_shift and c == 3 and hue_shift_degrees != 0.0:
            ...
            final_r_channel = (
                r_channel_shifted * shifted_r_fringe_color[0] + b_channel_shifted * shifted_b_fringe_color[0]
            )

            final_g_channel = (
                r_channel_shifted * shifted_r_fringe_color[1]
                + g_channel_original * 1.0
                + b_channel_shifted * shifted_b_fringe_color[1]
            )

            final_b_channel = (
                r_channel_shifted * shifted_r_fringe_color[2] + b_channel_shifted * shifted_b_fringe_color[2]
            )
    ...
    def _apply_zoom_blur(self, img, kwargs):
        ...
        falloff_param = kwargs.get('radial_blur_fallof', self.DEFAULTS['radial_blur_falloff'])
...
NODE_CLASS_MAPPINGS = {"CRTPostProcess": CRTPostProcessNode}

NODE_DISPLAY_NAME_MAPPINGS = {"CRTPostProcess": "CRT Post-Process Suite (CRT)"}

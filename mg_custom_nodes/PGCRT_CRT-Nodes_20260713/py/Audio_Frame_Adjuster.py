...
class AudioFrameAdjuster:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "audio": ("AUDIO",),
                "frame_count": ("INT", {"default": 120, "min": 1, "max": 999999, "step": 1, "display": "number"}),
                "fps": ("FLOAT", {"default": 30.0, "min": 1.0, "max": 120.0, "step": 0.01, "display": "number"}),
    ...
    def adjust_audio(self, audio, frame_count, fps, extension_mode, fade_duration):
        ...
        if current_samples == target_samples:
            print("[AudioFrameAdjuster] Audio duration matches target, no adjustment needed")
            ...
            if extension_mode == "silence":
                silence = torch.zeros(
                    waveform.shape[0], waveform.shape[1], samples_needed, dtype=waveform.dtype, device=waveform.device
                )
                ...
                while remaining > 0:
                    if remaining >= current_samples:
                        ...
                        if len(segments) > 0:
                            ...
                            segments[-1] = torch.cat([segments[-1][:, :, :-fade_samples], overlap], dim=2)
        ...
        # Final result audio
        result_audio = {'waveform': adjusted_waveform, 'sample_rate': sample_rate}

...
class LoadLastImage:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "folder_path": (
                    "STRING",
                    {"default": "", "placeholder": "X://insert/path/here"},
                ),
    ...
    def load_image(self, folder_path, sort_by, invert_order):
        ...
        if sort_by == "date":
            ...
            image_files.sort(key=natural_sort_key, reverse=not invert_order)
...
NODE_DISPLAY_NAME_MAPPINGS = {"LoadLastImage": "Load Last Image (CRT)"}

...
class FaceEnhancementWithInjectionSEGS:
    ...
    def execute(
        ...
        if not seg_entries:
            colored_print("[ERROR] No SEGS provided! Returning original image.", Colors.RED)
        ...
        if not valid_segments:
            colored_print("[ERROR] No valid SEGS crop regions available.", Colors.RED)
            fallback = torch.zeros_like(image)
            return (image, fallback, fallback, fallback, fallback)
        ...
        colored_print(f" Working canvas: {canvas_w}x{canvas_h}", Colors.GREEN)
        ...
        if enable_noise_injection == "enable":
            ...
            colored_print(
                f"    Split Sigmas Denoise: {split_ratio:.2f} | Stage1 sigmas: {len(stage1_sigmas)} | Stage2 sigmas: {len(stage2_sigmas)}",
                Colors.CYAN,
            )
            colored_print(
        ...
        colored_print(" Compositing final image...", Colors.HEADER)
        ...
        colored_print(
            "\n[OK] Face Enhancement with Injection SEGS completed successfully!",
            Colors.HEADER,
        )
        colored_print(

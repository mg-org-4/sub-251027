...
import os
import numpy as np
from .alignment_torch import robust_weighted_estimate_sim3_torch

try:
    from .alignment_triton import robust_weighted_estimate_sim3_triton
    HAS_TRITON = True
except ImportError:
    HAS_TRITON = False
...
def align_point_maps(point_map1, conf1, point_map2, conf2, conf_threshold):
    ...
    logger.info(f"The number of corresponding points matched: {all_pts1.shape[0]}")
...
def compute_alignment_error(point_map1, conf1, point_map2, conf2, conf_threshold, s, R, t):
    ...
    if len(target_points) == 0:
        logger.debug("No matching point pairs found for error calculation")
        return np.nan
...
def save_confident_pointcloud(
    ...
    logger.debug(f"shape of sampled point: {points.shape}")
    if HAS_TRIMESH:
...
def process_loop_list(chunk_index, loop_list, half_window=10):
    ...
    for idx1, idx2 in loop_list:
        try:
            ...
            logger.debug(f"Skipping pair ({idx1}, {idx2}): {e}")
    return results
...
def merge_ply_files(input_dir, output_path):
    ...
    if not input_files:
        logger.info("No PLY files found")
        return
    ...
    with open(output_path, "wb") as out_f:
        ...
        for file in input_files:
            logger.debug(f"Processing {idx_file}/{len(input_files)}: {file}")
            idx_file += 1
    ...
    logger.info(f"Merge completed! Total points: {total_vertices}")
...
def robust_weighted_estimate_sim3(
    ...
    for iter in range(max_iters):
        ...
        logger.debug(f"Residuals: {np.mean(residuals)}")
...
def compute_scale_ransac(
    ...
    """
    if not HAS_SKLEARN:
        logger.info("Warning: sklearn not available, returning default scale 1.0")
        return 1.0, 0.0
    ...
    if np.sum(valid_mask) < 100:
        logger.info(f"Warning: Only {np.sum(valid_mask)} valid points, using default scale 1.0")
        return 1.0, 0.0
    ...
    logger.info(f"RANSAC scale: {scale_factor:.6f}, inlier ratio: {inlier_ratio:.4f}")

    if 0.1 < scale_factor < 10.0:
        return scale_factor, inlier_ratio
    else:
        logger.info(f"Warning: Unreasonable scale {scale_factor}, using 1.0")
...
def compute_scale_weighted(
    ...
    logger.info(f"Weighted scale: {scale_quantile:.6f}, confidence: {confidence_score:.4f}")

    if 0.1 < scale_quantile < 10.0:
        return scale_quantile, confidence_score
    else:
        logger.info(f"Warning: Unreasonable scale {scale_quantile}, using 1.0")
...
def compute_chunk_scale_advanced(depth1, depth2, conf1, conf2, method="auto"):
    ...
    if method == "ransac":
        ...
        logger.info(f"RANSAC quality: {ransac_quality:.4f}, Weighted quality: {weighted_quality:.4f}")
...
def precompute_scale_chunks_with_depth(
    ...
    logger.info(f"Final scale: {scale_factor:.6f}, quality: {quality_score:.4f}, method: {method_used}")
...
def weighted_align_point_maps(
    ...
    if config["Model"]["align_lib"] == "numba":
        ...
        if not HAS_TRITON:
            raise ValueError("triton not available, please install triton or use a different align_lib")

...
from .processing.export import BpyPoseApplier
...
async def export_posed_fbx(request):
    ...
    try:
        ...
        try:
            # Use isolated bpy pose applier
            pose_applier = BpyPoseApplier()
            result = pose_applier.apply_pose(
            ...
            if not result.get("success"):
                error_msg = result.get("error", "Unknown error during pose export")
                return web.json_response({
                    'success': False,
                    'error': error_msg
                }, status=500)
...
async def get_mesh_files(request):
    ...
    """
    try:
        from .processing.load_mesh import SAM3DBodyLoadMesh

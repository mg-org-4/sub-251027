    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "RS_Bypass") {
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                self._rs_syncToggleAllState = function() {
                    const allItems = [...self.data.bypassedGroups, ...self.data.bypassedNodes];
                    if (allItems.length === 0) return;
                    
                    const allEnabled = allItems.every(item => item.enabled);
                    const allDisabled = allItems.every(item => !item.enabled);
                    
                    if (allEnabled) {
                        self.data.toggleAllOn = true;
                    } else if (allDisabled) {
                ...
                self._rs_syncFromExternal = function() {
                    ...
                    self.data.bypassedGroups.forEach(item => {
                        ...
                        if (allBypassed && !item.enabled) {
                            item.enabled = true;
                            changed = true;
                        } else if (!allBypassed && item.enabled) {
                            item.enabled = false;
                            changed = true;
                        }
                    });
                    ...
                    self.data.bypassedNodes.forEach(item => {
                        ...
                        if (!node) return;
                        const coveredByGroup = self.data.bypassedGroups.some(gItem => {
                            if (!gItem.enabled) return false;
                            const group = groups.find(g => g.id === gItem.id);
                            if (!group || !isNodeInGroup(node, group)) return false;
                    ...
                    const beforeCovered = self.data.bypassedNodes.length;
                    self.data.bypassedNodes = self.data.bypassedNodes.filter(item => {
                        const node = nodes.find(n => n.id === item.id);
                        if (!node) return true;
                        const coveredByGroup = self.data.bypassedGroups.some(gItem => {
                            if (!gItem.enabled) return false;
                    ...
                    if (self.data.bypassedNodes.length !== beforeCovered) changed = true;
                    ...
                    const beforeGroups = self.data.bypassedGroups.length;
                    const beforeNodes = self.data.bypassedNodes.length;

                    self.data.bypassedNodes = self.data.bypassedNodes.filter(item =>
                        nodes.some(n => n.id === item.id)
                    );
                    self.data.bypassedGroups = self.data.bypassedGroups.filter(item =>
                        groups.some(g => g.id === item.id)
                    );

                    if (self.data.bypassedNodes.length !== beforeNodes ||
                        self.data.bypassedGroups.length !== beforeGroups) {
                        changed = true;
                    }
                ...
                self.onDrawForeground = function(ctx, visibleRect) {
                    ...
                    self.data.bypassedNodes.forEach(item => {
                        ...
                        const belongsToCachedGroup = self.data.bypassedGroups.some(gItem => {
                            const group = groups.find(g => g.id === gItem.id);
                            return group && isNodeInGroup(targetNode, group);
                        });
                        if (belongsToCachedGroup) return;
                    ...
                    function renderContent() {
                        ...
                        groups.forEach(group => {
                            ...
                            groupText.onclick = (ev) => {
                                ...
                                if (entry.enabled) {
                                    self.data.bypassedNodes = self.data.bypassedNodes.filter(
                                        nItem => !groupNodes.some(n => n.id === nItem.id)
                                    );
                                }
                                self._rs_syncToggleAllState();
                                self._rs_syncData();
                ...
                self._rs_closeMenu = function() {
                    ...
                    if (self._rs_escHandler) {
                        document.removeEventListener("keydown", self._rs_escHandler);
                        self._rs_escHandler = null;
                    }
                    if (self._rs_outsideHandler) {
                        document.removeEventListener("pointerdown", self._rs_outsideHandler, true);
                        self._rs_outsideHandler = null;
                    }

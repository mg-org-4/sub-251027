"""
ComfyUI Qwen3-4B Text Encoder Inspector — Layer Analysis Tool

For Qwen3-4B text encoder.
...
class Qwen3_4BTextEncoderInspector:
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "clip": ("CLIP", {
                    "tooltip": "CLIP/Text Encoder to analyze (Qwen3-4B)"
                }),
                "prompt": ("STRING", {
                    "multiline": True,
    ...
    OUTPUT_NODE = True
    DESCRIPTION = """Analyzes Qwen3-4B text encoder layers.
...
Use QUICK mode first, then ABLATION if needed."""
    ...
    def inspect(self, clip, prompt, analysis_mode="quick", ablation_strength=0.5):
        print(f"[Qwen3-4B TE Inspector] Starting analysis...")
        print(f"[Qwen3-4B TE Inspector] Prompt: {prompt[:50]}...")
        ...
        print(f"[Qwen3-4B TE Inspector] Token count: {len(token_ids)}")
        ...
        for attr in ['qwen3_4b', 'qwen3_8b', 'transformer', 'model', 'text_model', 'encoder']:
        ...
        try:
            ...
            print(f"[Qwen3-4B TE Inspector] Forward pass error: {e}")
            capturer.clear()
            return self._error_report(str(e))
        
        print(f"[Qwen3-4B TE Inspector] Captured {len(capturer.activations)} activation points")
        ...
        if analysis_mode == "ablation_HEAVY":
            print(f"[Qwen3-4B TE Inspector] ⚠️ Running ablation analysis (HEAVY - 3x VRAM)...")
            ablation_results = self._run_ablation(clip, tokens_data, ablation_strength)
            results["ablation"] = ablation_results
        else:
            print(f"[Qwen3-4B TE Inspector] Quick mode - skipping ablation")
        ...
        print(f"[Qwen3-4B TE Inspector] Analysis complete.")
    ...
    def _run_ablation(self, clip, tokens_data, strength: float) -> Dict:
        ...
        try:
            ...
            for group_name, layer_indices in layer_groups.items():
                print(f"[Qwen3-4B TE Inspector] Ablating {group_name}...")
                ...
                if patches:
                    ...
                    print(f"[Qwen3-4B TE Inspector] Ablation {group_name}: {relative_change:.4f} relative change")
            ...
            print(f"[Qwen3-4B TE Inspector] Ablation error: {e}")
    ...
    def _format_report(self, results: Dict) -> str:
        lines = [
            "=" * 70,
            "QWEN3-4B TEXT ENCODER ANALYSIS REPORT",
            "=" * 70,
...
NODE_CLASS_MAPPINGS = {
    "Qwen3_4BTextEncoderInspector": Qwen3_4BTextEncoderInspector,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "Qwen3_4BTextEncoderInspector": "Text Encoder Inspector (Qwen3-4B)",
}

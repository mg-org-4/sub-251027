...
class StarwildcardsAdvancedSingle:
    BGCOLOR = "#3d124d"  # Background color
    COLOR = "#19124d"  # Title color
    
    RETURN_TYPES = ('STRING',)
    FUNCTION = 'star_wilds_adv_single'
    CATEGORY = '⭐StarNodes/Text And Data'
    ...
    def star_wilds_adv_single(self, seed, prompt, wildcard):
        # Process base prompt text using the same wildcard syntax logic
        processed = process_wildcard_syntax(prompt, seed)

        # Process optional external wildcard file selection
        if wildcard != "None":
            if wildcard == "Random":
                wildcard = self.get_random_wildcard(seed + 5)

...
app.registerExtension({
    name: "starnodes.panorama_viewer",
    
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "StarPanoramaViewer") {
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                const widget = this.addDOMWidget("panorama_viewer", "viewer", container, {
                    serialize: false,
                    hideOnZoom: false
                });
            ...
            nodeType.prototype.onExecuted = function(message) {
                ...
                if (panos && panos.length > 0) {
                    ...
                    if (this.panoramaWidget) {
                        initPanoramaViewer(this.panoramaWidget, imageUrl, panoData.layout, depthUrl);
                    }
                }
            };
...
function loadThreeJS(callback) {
    ...
    // Loaded from the official jsDelivr CDN (three.js r128) instead of a
    // bundled copy, to comply with the ComfyUI Registry policy on bundled
    // minified third-party libraries.
    script.src = "https://cdn.jsdelivr.net/npm/three@0.128.0/build/three.min.js";
    script.onload = callback;
    script.onerror = () => {
        console.error("Failed to load Three.js from CDN (internet connection required for the 3D panorama preview).");
    };
...
function initPanoramaViewer(widget, imageUrl, layout, depthUrl) {
    loadThreeJS(() => {
        ...
        textureLoader.load(imageUrl, (texture) => {
            ...
            speedSlider.addEventListener("input", () => {
                state.autoSpeed = parseFloat(speedSlider.value);
            }, { signal });
            ...
            function animate() {
                ...
                if (state.autoRotate) {
                    targetRotationY += state.autoSpeed * 0.003;
                }

    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            this._noteStorageKey = () => {
                try { return `zh_latentswitch_notes_${this.id || 'unknown'}`; } catch(_) { return 'zh_latentswitch_notes_unknown'; }
            };
            ...
            this.saveNoteValues = function () {
                try {
                    ...
                    for (let i = 1; i <= target; i++) {
                        const w = this.widgets?.find(w => w.name === `latent${i}_note`);
                        notes.push(w ? (w.value ?? "") : "");
                    }
                    this.properties.zh_latentswitch = { count: target, notes };
            ...
            this.updateInputs = function (n) {
                ...
                const isLatent = (name) => /^latent\d+$/.test(name);
                ...
                if (n < current) {
                    for (let i = this.inputs.length - 1; i >= 0; i--) {
                        ...
                        if (isLatent(input.name)) {
                            const num = parseInt(input.name.replace("latent", ""));
            ...
            this.getConnectedLatentInputs = function () {
                const connected = [];
                if (!this.inputs) return connected;
                for (const input of this.inputs) {
                    if (/^latent\d+$/.test(input.name) && input.link != null && input.link !== undefined) {
                        const num = parseInt(input.name.replace("latent", ""));
                        connected.push(num);
                    }
                }
            ...
            this.updateSelectOptions = function (n) {
                const w = this.widgets?.find(w => w.name === "select_latent");
                if (!w) return;
                const modeWidget = this.widgets?.find(w => w.name === "mode");
                const isManual = modeWidget?.value === "manual";
                let opts;
                if (isManual) {
                    const connected = this.getConnectedLatentInputs();
            ...
            this.syncCommentWidgets = function (n) {
                ...
                const isNote = (name) => /^latent\d+_note$/.test(name);
                for (let i = (this.widgets?.length || 0) - 1; i >= 0; i--) {
                    const w = this.widgets[i];
                    if (w && isNote(w.name)) {
                        const idx = parseInt(w.name.replace("latent", "").replace("_note", ""));
                        if (idx > target) {
                            w.onRemove?.();

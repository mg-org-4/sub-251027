...
rem 获取当前脚本所在目录，然后指向Chinese_Localization_Files子目录
set "SCRIPT_DIR=%~dp0"
...
rem 检查ComfyUI-DD-Translation扩展是否存在
echo 正在检查 ComfyUI-DD-Translation 扩展...

rem 向上查找custom_nodes目录
...
if exist "%DD_TRANSLATION_DIR%" (
    echo [OK] 找到 ComfyUI-DD-Translation 扩展
...
if errorlevel 1 (
    echo [ERROR] 复制 zhihui_nodes_comfyui.json 失败
    pause
    exit /b 1
) else (
    echo [OK] zhihui_nodes_comfyui.json 复制成功
)

echo   复制 zhihui_nodes_translation.json 到 Nodes 目录...
copy /Y "%SOURCE_DIR%\zhihui_nodes_translation.json" "%NODES_TARGET_DIR%\" >nul 2>&1
if errorlevel 1 (
    echo [ERROR] 复制 zhihui_nodes_translation.json 失败
    pause
    exit /b 1
) else (
    echo [OK] zhihui_nodes_translation.json 复制成功
)

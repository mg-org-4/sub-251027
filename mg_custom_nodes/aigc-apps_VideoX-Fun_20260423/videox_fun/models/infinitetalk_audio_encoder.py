...
class InfiniteTalkAudioEncoder(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    """
    ...
    def __init__(self, pretrained_model_path="facebook/wav2vec2-base-960h", device='cpu'):
        super(InfiniteTalkAudioEncoder, self).__init__()
    ...
    def extract_audio_feat_without_file_load(
        ...
        """
        Extract audio features from audio array (without loading from file).
        ...
        # Calculate video_length if not provided
        if video_length is None:
            audio_duration = len(audio_segment) / sample_rate
            video_length = int(audio_duration * self.video_rate)

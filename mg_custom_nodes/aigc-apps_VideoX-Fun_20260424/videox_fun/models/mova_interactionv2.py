# Modified from https://github.com/OpenMOSS/MOVA/blob/main/mova/diffusion/models/interactionv2.py
...
from .wan_transformer3d import WanRMSNorm as RMSNorm


class AttentionModule(nn.Module):
    """Attention module wrapper using attention_utils.attention."""
    def __init__(self, num_heads: int):
        super().__init__()
        self.num_heads = num_heads

    def forward(self, q, k, v):
        # q, k, v: [B, L, D] -> [B, L, N, D/N]
        q = rearrange(q, "b l (n d) -> b l n d", n=self.num_heads)
        k = rearrange(k, "b l (n d) -> b l n d", n=self.num_heads)
        v = rearrange(v, "b l (n d) -> b l n d", n=self.num_heads)
        x = attention(q, k, v)
        return rearrange(x, "b l n d -> b l (n d)")
...
class MOVADualTowerConditionalBridge(ModelMixin, ConfigMixin):
    """
    ...
    def __init__(self,
        ...
        apply_first_frame_bias_in_rope: bool = False,  # whether to account for 1/frame_rate bias for the first frame in RoPE alignment
    ...
    def build_aligned_freqs(self,
        ...
        if self.apply_first_frame_bias_in_rope:
            # Account for the "first frame lasts 1/frame_rate" bias.
            video_effective_fps = float(frame_rate) / 4.0
            if f_v > 0:
                t_starts = torch.zeros((f_v,), device=device, dtype=torch.float32)
                if f_v > 1:
                    t_starts[1:] = (1.0 / float(frame_rate)) + torch.arange(f_v - 1, device=device, dtype=torch.float32) * (1.0 / video_effective_fps)
            else:
            ...
            scale = float(self.audio_fps) / float(frame_rate / 4.0)
        ...
        # print(f"video fps: {frame_rate}, audio fps: {self.audio_fps}, scale: {scale}")
    ...
    def apply_conditional_control(self,
                                 ...
                                 video_grid_size: Optional[Tuple[int, int, int]] = None) -> torch.Tensor:
        """

# LongCat-Video 全参数训练指南

本文档提供 LongCat-Video Diffusion Transformer 全参数训练的完整流程，包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：LongCat-Video 是一个支持文生视频（T2V）和图生视频（I2V）的长视频生成模型。本文档涵盖普通视频生成任务的训练流程。
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_longcatvideo` |
...
| `--validation_prompts` | 验证视频生成的提示词 | `"一位年轻女子..."` |
...
| `model_name` | 模型路径 | `models/Diffusion_Transformer/LongCat-Video` |
...
| `save_path` | 生成视频保存路径 | `samples/longcat-videos-i2v` 或 `samples/longcat-videos-t2v` |
...
根据需求修改编辑 `examples/longcatvideo/predict_t2v.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。
...
# 训练好的权重路径，如 "output_dir_longcatvideo/checkpoint-xxx/diffusion_pytorch_model.safetensors"
...
根据需求修改编辑 `examples/longcatvideo/predict_i2v.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。
...
编辑 `examples/longcatvideo/predict_t2v.py` 或 `examples/longcatvideo/predict_i2v.py`：

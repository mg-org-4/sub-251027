# Wan2.2 Fun Full Parameter Training Guide

This document provides a complete workflow for full parameter training of Wan2.2 Fun (Inpainting), including environment setup, data preparation, distributed training, and inference testing.

> **Note**: Wan2.2 Fun is a video inpainting model based on the Wan2.2 architecture, supporting video inpainting tasks. Wan2.2 adopts a dual-Transformer architecture (high-noise/low-noise models), and the 5B version uses a single-Transformer architecture. This document covers the training workflow for Wan2.2 Fun, supporting both A14B and 5B model variants.
...
**Wan2.2 Fun 5B Training Example**:
...
- **Boundary Type (boundary_type)**:
  - `low`: Train low-noise model, high-noise model uses pretrained weights (recommended for T2V/I2V fine-tuning)
...
|-----------|-------------|---------------|
| `--pretrained_model_name_or_path` | Pretrained model path | `models/Diffusion_Transformer/Wan2.2-Fun-A14B-InP` |
...
| `--seed` | Random seed | 42 |
| `--output_dir` | Output directory | `output_dir_wan2.2` |
...
| `--boundary_type` | Wan2.2 dual-Transformer boundary type: `low` (train low-noise), `high` (train high-noise), `full` (single model like TI2V-5B) | `low` |
| `--train_mode` | Training mode: fixed to `inpaint` (video inpainting) | `inpaint` |
...
**Wan2.2 Training Strategy Recommendations**:
- **T2V Model (Dual-Transformer)**: Use `boundary_type="low"` and `train_mode="inpaint"` to train the low-noise model. This maintains the generality of the high-noise part while fine-tuning the low-noise part for your data.
- **I2V Model (Dual-Transformer)**: Use `boundary_type="low"` and `train_mode="inpaint"` to train the low-noise model. The dataset needs to include reference images.
- **TI2V Model (Single-Transformer)**: Use `boundary_type="full"` and `train_mode="inpaint"` for full training. The dataset needs to include reference images.
- **Memory Optimization**: Wan2.2 models are large (14B/5B parameters). It is strongly recommended to use `--low_vram` and `--gradient_checkpointing`.
- **Multi-GPU Training**: For 14B models, it is recommended to use FSDP or DeepSpeed-Zero-2/3 for multi-GPU training.
...
**A14B Mode Example** (dual-Transformer validation):
...
**5B Mode Example** (single-Transformer validation):
...
- Validation uses inpainting mode with reference image as the starting frame
...
accelerate launch --mixed_precision="bf16" --main_process_ip=$MASTER_ADDR --main_process_port=$MASTER_PORT --num_machines=$WORLD_SIZE --num_processes=$NUM_PROCESS --machine_rank=$RANK --use_deepspeed --deepspeed_config_file config/zero_stage2_config.json --deepspeed_multinode_launcher standard scripts/wan2.2_fun/train.py \
  --config_path="config/wan2.2/wan_civitai_i2v.yaml" \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --train_data_dir=$DATASET_NAME \
  --train_data_meta=$DATASET_META_NAME \
...
| `transformer_high_path` | Path to trained high-noise Transformer weights (dual-Transformer models only) | `None` |
...
| `validation_image_start` | Reference image path for I2V/TI2V mode | `"asset/1.png"` |
...
#### 4.2.1 A14B Dual-Transformer Model

Run the following command for single GPU inference:

```bash
python examples/wan2.2_fun/predict_t2v.py
```

Edit `examples/wan2.2_fun/predict_t2v.py` according to your needs. For initial inference, focus on the following parameters. If you're interested in other parameters, refer to the inference parameter reference above.
...
# Path to trained low-noise weights, e.g., "output_dir_wan2.2/checkpoint-xxx/diffusion_pytorch_model.safetensors"
...
#### 4.2.2 5B Single-Transformer Model

Run the following command for single GPU inference:

```bash
python examples/wan2.2_fun/predict_t2v_5b.py
```

Edit `examples/wan2.2_fun/predict_t2v_5b.py` according to your needs. For initial inference, focus on the following parameters.

```python
# Choose based on GPU memory
GPU_memory_mode = "sequential_cpu_offload"
# Based on actual model path (5B model)
model_name = "models/Diffusion_Transformer/Wan2.2-Fun-5B-InP"  
# Config path for 5B model
config_path = "config/wan2.2/wan_civitai_5b.yaml"
# Since 5B is a single-Transformer model, only transformer_path is used
transformer_path = None  
transformer_high_path = None  # Not used for 5B model
# Write based on the content you want to generate
prompt = "A brown dog shaking its head, sitting on a light-colored sofa in a cozy room. Behind the dog, there's a framed painting on a shelf, surrounded by pink flowers. The soft, warm lighting in the room creates a comfortable atmosphere."  
# 5B model typically uses higher resolution
...
**Key Differences Between A14B and 5B**:
- **Architecture**: A14B uses dual-Transformer (high/low noise), 5B uses single-Transformer
- **Config**: A14B uses `wan_civitai_i2v.yaml`, 5B uses `wan_civitai_5b.yaml`
- **Parameters**: For 5B, `transformer_high_path` and `lora_high_path` are not used
- **Resolution**: 5B model supports higher resolutions (e.g., 704x1280)
- **Frame Count**: 5B model can generate longer videos (e.g., 121 frames)

### 4.3 Image-to-Video (I2V) Inference

#### 4.3.1 A14B Dual-Transformer Model
...
Edit `examples/wan2.2_fun/predict_i2v.py` according to your needs. For initial inference, focus on the following parameters. If you're interested in other parameters, refer to the inference parameter reference above.
...
#### 4.3.2 5B Single-Transformer Model
...
Edit `examples/wan2.2_fun/predict_i2v_5b.py` according to your needs. For initial inference, focus on the following parameters.
...
# Starting image for image-to-video
validation_image_start = "asset/1.png"
validation_image_end = None  # Optional ending image
# Write based on the content you want to generate
...
Edit `examples/wan2.2_fun/predict_t2v.py` or `examples/wan2.2_fun/predict_i2v.py`:

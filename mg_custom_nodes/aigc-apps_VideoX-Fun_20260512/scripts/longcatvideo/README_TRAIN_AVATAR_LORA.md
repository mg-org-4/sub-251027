# LongCat-Video-Avatar LoRA Training Guide

This document provides a complete workflow for LoRA (Low-Rank Adaptation) training of LongCat-Video-Avatar (audio-driven long-form talking head video generation model), including environment configuration, data preparation, distributed training, and inference testing.
...
| **LongCat-Video-Avatar (Audio-Driven Generation)** | `file_path`, `audio_path`, `text`, `type` | ✅ **Required** |
...
| `--target_name` | Components/modules to apply LoRA (comma-separated) | `qkv,q_linear,kv_linear,ffn.w1,ffn.w2,ffn.w3` |
...
| `lora_path` | LoRA weights path | `output_dir_longcat_avatar_lora/pytorch_lora_weights.safetensors` |
...
Edit `examples/longcatvideo/predict_s2v_avatar.py` according to your needs. For LoRA inference, focus on these parameters:
...
# LoRA weight path, e.g., "output_dir_longcat_avatar_lora/checkpoint-xxx/lora_weights.safetensors"
lora_path = None

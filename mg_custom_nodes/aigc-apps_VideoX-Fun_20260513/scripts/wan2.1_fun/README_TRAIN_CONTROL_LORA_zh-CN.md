# Wan2.1 Fun Control LoRA 微调训练指南

本文档提供 Wan2.1 Fun Control LoRA 微调训练的完整流程,包括环境配置、数据准备、多种分布式训练策略和推理测试。

> **说明**：Wan2.1 Fun Control 是一个支持可控视频生成（如姿态控制 Pose）的视频生成模型。本指南涵盖 Control LoRA 微调训练流程。
...
| `--seed` | 随机种子（可复现训练） | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.1_fun_control_lora` |
...
# LoRA 权重路径，如 "output_dir_wan2.1_fun_control_lora/checkpoint-xxx/checkpoint-xxx.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55
# 控制信号视频路径（如姿态视频）

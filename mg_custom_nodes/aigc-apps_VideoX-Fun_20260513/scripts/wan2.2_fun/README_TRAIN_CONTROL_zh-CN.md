# Wan2.2 Fun Control 全参数训练指南

本文档提供 Wan2.2 Fun（可控视频生成）Control 全参数训练的完整流程，包括环境配置、数据准备、多种分布式训练策略和推理测试。

> **说明**：Wan2.2 Fun 是一个基于 Wan2.2 架构的可控视频生成模型，支持通过控制信号（如姿态视频、深度图等）引导视频生成。Wan2.2采用双Transformer架构（高噪声/低噪声模型），5B版本使用单Transformer架构。本指南涵盖 Wan2.2 Fun 的 Control 全参数训练流程，支持 A14B 和 5B 两种模型变体。
...
**Wan2.2 Fun Control 全参数训练示例（DeepSpeed-Zero-2）**：
...
> **说明**：本目录中的 `train_control.sh` 脚本提供了不带 DeepSpeed 的基础训练模板。对于更好的多GPU训练性能和显存效率，请使用上方的 DeepSpeed-Zero-2 命令。
...
- **边界类型（boundary_type）**：
  - `low`：训练低噪声模型，高噪声模型使用预训练权重（推荐用于T2V/I2V/Control全参数微调）
...
| `--learning_rate` | 初始学习率（全参数训练推荐值） | 2e-05 |
...
| `--seed` | 随机种子（可复现训练） | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.2_fun_control` |
...
| `--trainable_modules` | 可训练模块（`.` 表示训练所有模块） | `"."` |
...
> **说明**：这与 `train_control.sh` 脚本类似，但使用了正确的数据集路径。`train_control.sh` 脚本可以用作单GPU训练的起点。
...
# 训练好的低噪声权重路径，如 "output_dir_wan2.2_fun_control/checkpoint-xxx/diffusion_pytorch_model.safetensors"

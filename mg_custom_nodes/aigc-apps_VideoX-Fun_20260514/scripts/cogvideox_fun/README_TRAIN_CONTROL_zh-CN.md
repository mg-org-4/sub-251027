# CogVideoX-Fun Control 全参数训练指南

本文档提供 CogVideoX-Fun Control Diffusion Transformer 全参数训练的完整流程,包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：CogVideoX-Fun Control 是一个支持可控视频生成（如姿态控制 Pose）的视频生成模型。本文档涵盖 Control 模型的训练流程。
...
accelerate launch --use_deepspeed --deepspeed_config_file config/zero_stage2_config.json --deepspeed_multinode_launcher standard scripts/cogvideox_fun/train_control.py \
  ...
  --lr_warmup_steps=100 \
  --seed=43 \
  --output_dir="output_dir_cogvideox_fun_control" \
...
| `--lr_warmup_steps` | 学习率预热步数 | 50 |
| `--seed` | 随机种子 | 43 |
| `--output_dir` | 输出目录 | `output_dir_cogvideox_fun_control` |
...
| `model_name` | 模型路径 | `models/Diffusion_Transformer/CogVideoX-Fun-V1.1-2b-Pose` |
...
| `sample_size` | 生成视频分辨率 `[高度, 宽度]` | `[672, 384]` |
...
| `prompt` | 正向提示词，描述生成内容 | `"A young woman..."` |
...
| `save_path` | 生成视频保存路径 | `samples/cogvideox-fun-videos_control` |
...
根据需求修改编辑 `examples/cogvideox_fun/predict_v2v_control.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。
...
# 训练好的权重路径，如 "output_dir_cogvideox_fun_control/checkpoint-xxx/diffusion_pytorch_model.safetensors"
...
编辑 `examples/cogvideox_fun/predict_v2v_control.py`：

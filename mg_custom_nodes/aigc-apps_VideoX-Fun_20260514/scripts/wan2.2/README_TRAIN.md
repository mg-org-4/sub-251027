# Wan2.2 Full Parameter Training Guide

This document provides a complete workflow for full parameter training of Wan2.2 Diffusion Transformer, including environment setup, data preparation, distributed training, and inference testing.

> **Note**: Wan2.2 is a video generation model that supports Text-to-Video (T2V), Image-to-Video (I2V), and Text-Image-to-Video (TI2V). Wan2.2 adopts a dual-Transformer architecture (high-noise/low-noise models) for higher quality video generation. This document covers the training workflow for general video generation tasks.
...
- **T2V Model (Dual-Transformer)**: Use `boundary_type="low"` and `train_mode="normal"` to train the low-noise model. This maintains the generality of the high-noise part while fine-tuning the low-noise part for your data.
- **I2V Model (Dual-Transformer)**: Use `boundary_type="low"` and `train_mode="i2v"` to train the low-noise model. The dataset needs to include reference images.
- **TI2V Model (Single-Transformer)**: Use `boundary_type="full"` and `train_mode="ti2v"` for full training. The dataset needs to include reference images.
...
- `i2v` or `ti2v` mode requires the `--validation_paths` parameter
- Wan2.2 validation automatically selects single or dual-Transformer based on `boundary_type`
...
Edit `examples/wan2.2/predict_t2v.py` according to your needs. For initial inference, focus on the following parameters. If you're interested in other parameters, refer to the inference parameter reference above.
...
Edit `examples/wan2.2/predict_i2v.py` according to your needs. For initial inference, focus on the following parameters. If you're interested in other parameters, refer to the inference parameter reference above.
...
Edit `examples/wan2.2/predict_ti2v.py` according to your needs. For initial inference, focus on the following parameters. If you're interested in other parameters, refer to the inference parameter reference above.

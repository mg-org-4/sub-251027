# Wan2.2 Fun Control LoRA 微调训练指南

本文档提供 Wan2.2 Fun（可控视频生成）Control LoRA 微调训练的完整流程，包括环境配置、数据准备、多种分布式训练策略和推理测试。

> **说明**：Wan2.2 Fun 是一个基于 Wan2.2 架构的可控视频生成模型，支持通过控制信号（如姿态视频、深度图等）引导视频生成。Wan2.2采用双Transformer架构（高噪声/低噪声模型），5B版本使用单Transformer架构。本指南涵盖 Wan2.2 Fun 的 Control LoRA 微调训练流程，支持 A14B 和 5B 两种模型变体。
...
- [三、Control LoRA 训练](#三control-lora-训练)
  - [3.1 下载预训练模型](#31-下载预训练模型)
  - [3.2 快速开始（DeepSpeed-Zero-2）](#32-快速开始deepspeed-zero-2)
  - [3.3 Control + LoRA 专用参数解析](#33-control--lora-专用参数解析)
  - [3.4 训练验证](#34-训练验证)
...
- [四、推理测试](#四推理测试)
  ...
  - [4.2 Control 视频生成推理](#42-control-视频生成推理)
...
我们提供了一个包含控制信号的测试数据集，其中包含若干训练数据。
...
Control 训练数据集除了原始视频外，还需要提供一一对应的控制信号视频（如姿态视频、深度视频等）。
...
> **说明**：`train/` 目录存放原始视频，`control/` 目录存放与原始视频一一对应的控制信号视频。控制视频的文件名应与原始视频保持一致。
...
- `control_file_path`：控制信号视频路径（相对或绝对路径，**Control 训练必需**）
- `text`：视频描述（英文提示词）
- `type`：数据类型，固定为 `"video"`
...
**Wan2.2 Fun Control LoRA 训练示例（DeepSpeed-Zero-2）**：
...
> **说明**：本目录中的 `train_control_lora.sh` 脚本提供了不带 DeepSpeed 的基础训练模板。对于更好的多GPU训练性能和显存效率，请使用上方的 DeepSpeed-Zero-2 命令。

### 3.3 Control + LoRA 专用参数解析
...
- **边界类型（boundary_type）**：
  - `low`：训练低噪声模型，高噪声模型使用预训练权重（推荐用于T2V/I2V/Control LoRA微调）
...
| `--pretrained_model_name_or_path` | 预训练模型路径 | `models/Diffusion_Transformer/Wan2.2-Fun-A14B-Control` |
...
| `--seed` | 随机种子（可复现训练） | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.2_fun_control_lora` |
...
| `--train_mode` | 训练模式：`control`（纯Control）、`control_ref`（Control+参考图像）、`control_camera_ref`（Control+相机+参考图像） | `control_ref` |
| `--control_ref_image` | 参考图像来源：`first_frame`（首帧）、`random`（随机帧） | `random` |
| `--add_full_ref_image_in_self_attention` | 在自注意力中注入完整参考图像信息 | - |
| `--add_inpaint_info` | 在自注意力中注入修复（inpaint）信息 | - |
...
- 当 `train_mode="control_ref"` 时，验证同时使用控制视频和参考图像
...
> **说明**：这与 `train_control_lora.sh` 脚本类似，但使用了正确的数据集路径。`train_control_lora.sh` 脚本可以用作单GPU训练的起点。
...
| `model_name` | 模型路径 | `models/Diffusion_Transformer/Wan2.2-Fun-A14B-Control` |
...
| `sample_size` | 生成视频分辨率 `[高度, 宽度]` | `[832, 480]`（A14B）或 `[1280, 704]`（5B） |
| `video_length` | 生成视频帧数 | `81`（A14B）或 `121`（5B） |
| `fps` | 每秒帧数 | `16`（A14B）或 `24`（5B） |
| `weight_dtype` | 模型权重精度，不支持 bf16 的显卡使用 `torch.float16` | `torch.bfloat16` |
| `control_video` | 控制信号视频路径（如姿态视频） | `"asset/pose.mp4"` |
| `control_camera_txt` | 相机控制文本路径（可选，用于相机控制） | `None` |
| `ref_image` | 参考图像路径（control_ref 模式） | `"asset/8.png"` |
| `start_image` | 起始帧图像路径（可选，用于修复模式） | `None` |
...
Wan2.2 Fun Control 提供多种推理脚本，请根据您的模型版本和任务类型选择：

| 脚本 | 模型版本 | 架构 | 主要用途 |
|------|---------|------|---------|
| `predict_v2v_control_ref.py` | A14B | 双Transformer | Control + 参考图像（推荐） |
| `predict_v2v_control.py` | A14B | 双Transformer | 纯 Control（无参考图像） |
| `predict_v2v_control_ref_5b.py` | 5B | 单Transformer | Control + 参考图像（5B） |
| `predict_v2v_control_5b.py` | 5B | 单Transformer | 纯 Control（5B，无参考图像） |
...
#### 4.2.2 A14B 模型 Control + Ref 推理（双Transformer）
...
根据需求修改 `examples/wan2.2_fun/predict_v2v_control_ref.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。
...
# 训练好的 LoRA 权重路径，如 "output_dir_wan2.2_fun_control_lora/checkpoint-xxx/diffusion_pytorch_model.safetensors"
lora_path = None
lora_high_path = None
# 控制信号视频（如姿态视频）
control_video = "asset/pose.mp4"
# 参考图像路径（control_ref 模式）
...
> **说明**：Wan2.2 Fun Control 主要用于可控视频生成任务。提供 `control_video` 控制信号视频后，模型将根据控制信号引导视频生成。

#### 4.2.3 A14B 模型纯 Control 推理（双Transformer，无参考图像）
...
#### 4.2.4 5B 模型 Control + Ref 推理（单Transformer）
...
根据需求修改 `examples/wan2.2_fun/predict_v2v_control_ref_5b.py`，重点关注如下参数：
...
#### 4.2.5 5B 模型纯 Control 推理（单Transformer，无参考图像）
...
编辑 `examples/wan2.2_fun/predict_v2v_control_ref.py`：

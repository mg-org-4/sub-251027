# HunyuanVideo LoRA 微调训练指南

本文档提供 HunyuanVideo LoRA 微调训练的完整流程，包括环境配置、数据准备、多种分布式训练策略和推理测试。

> **说明**：HunyuanVideo 是支持文生视频（T2V）和图生视频（I2V）的视频生成模型。本指南涵盖 LoRA 微调训练流程，适用于自定义数据集的微调场景。
...
# 训练好的权重路径，如 "output_dir_hunyuanvideo_lora/checkpoint-xxx/lora_weights.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55
# 根据生成内容编写
...
# 根据实际模型路径
model_name = "models/Diffusion_Transformer/HunyuanVideo"  
# LoRA 权重路径，如 "output_dir_hunyuanvideo_lora/checkpoint-xxx/lora_weights.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55
# 图生视频的起始图像

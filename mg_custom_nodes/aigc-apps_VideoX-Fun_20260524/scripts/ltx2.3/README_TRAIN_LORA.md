# LTX-2.3 LoRA Training Guide

This document provides a complete workflow for LoRA (Low-Rank Adaptation) training of LTX-2.3 (Lightricks Text-to-Video), including environment configuration, data preparation, distributed training, and inference testing.

> **Note**: LTX-2.3 is an audio-visual generative video model that can simultaneously generate video and corresponding audio. The training data requires both video and audio files.
...
> ⚠️ **Important**: LTX-2.3 is an audio-visual generative model. Unlike regular video training, you **must provide the `audio_path` field in metadata.json**.
...
- `audio_path`: Audio file path (**LTX-2.3 specific and required**, main difference from regular video training)
...
**Dataset Comparison: LTX-2.3 vs Regular Video Training**:

| Model Type | Required Fields | Audio Field |
|---------|---------|---------|
| Regular Video (WAN, CogVideoX, etc.) | `file_path`, `text`, `type` | ❌ Not needed |
| **LTX-2.3 (Audio-Visual Generation)** | `file_path`, `audio_path`, `text`, `type` | ✅ **Required** |
...
# Download LTX-2.3 official weights
...
| `--target_name` | Components/modules to apply LoRA (comma-separated) | `to_q,to_k,to_v,ff.0,ff.2,audio_ff.0,audio_ff.2` |
| `--use_peft_lora` | Use PEFT module for adding LoRA (more memory-efficient) | - |
...
| `--video_sample_n_frames` | Number of frames to sample | 121 |
...
| `--learning_rate` | Initial learning rate (LoRA typically uses higher LR than full training) | 1e-04 |
| `--seed` | Random seed | 42 |
| `--output_dir` | Output directory | `output_dir_ltx2.3_lora` |
...
| `model_name` | Model path | `models/Diffusion_Transformer/LTX-2.3-Diffusers` |
...
| `lora_path` | LoRA weights path | `output_dir_ltx2.3_lora/pytorch_lora_weights.safetensors` |
...
| `guidance_scale` | Guidance strength | 3.0 |
| `audio_guidance_scale` | Audio guidance strength | 7.0 |
| `stg_scale` | Spatio-Temporal Guidance (STG) scale for video | 1.0 |
| `audio_stg_scale` | STG scale for audio | 1.0 |
| `modality_scale` | Modality isolation guidance scale for video | 3.0 |
| `audio_modality_scale` | Modality isolation guidance scale for audio | 3.0 |
| `guidance_rescale` | Guidance rescale factor for video | 0.7 |
| `audio_guidance_rescale` | Guidance rescale factor for audio | 0.7 |
| `spatio_temporal_guidance_blocks` | STG block indices | [28] |
...
Edit `examples/ltx2.3/predict_t2v.py` according to your needs. For LoRA inference, focus on these parameters:
...
Edit `examples/ltx2.3/predict_t2v.py`:

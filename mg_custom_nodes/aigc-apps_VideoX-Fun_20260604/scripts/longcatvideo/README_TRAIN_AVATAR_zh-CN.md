# LongCat-Video-Avatar 全参数训练指南

本文档提供 LongCat-Video-Avatar（音频驱动的长片段数字人视频生成模型）全参数训练的完整工作流，包括环境配置、数据准备、分布式训练和推理测试。

> **注意**：LongCat-Video-Avatar 是一个音频驱动的长片段数字人视频生成模型，需要同时提供参考图像和音频文件来生成说话视频。训练数据需要包含视频、音频和参考图像。
...
LongCat-Video-Avatar 的 `metadata.json` 与 VideoX-Fun 的普通 JSON 格式略有不同，**需要额外添加 `audio_path` 字段**。
...
- `audio_path`：音频文件的相对路径或绝对路径（**LongCat-Video-Avatar 必需字段**）
  - 音频文件通常为 `.wav` 格式
...
| `--seed` | 随机种子 | `42` |
| `--output_dir` | 输出目录 | `output_dir_longcat_avatar` |
...
| `prompt` | 生成提示词 | `"A young woman with long flowing purple hair stands by the seaside on a sunny day, singing..."` |
...
根据需求编辑 `examples/longcatvideo/predict_s2v_avatar.py`。首次推理请重点修改以下参数，其他参数见上方推理参数说明。
...
# 训练后的权重路径（可选），如 "output_dir_longcat_avatar/checkpoint-xxx/diffusion_pytorch_model.safetensors"

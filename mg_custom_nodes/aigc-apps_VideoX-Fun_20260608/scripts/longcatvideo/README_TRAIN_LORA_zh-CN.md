# LongCat-Video LoRA 微调训练指南

本文档提供 LongCat-Video LoRA 微调训练的完整流程，包括环境配置、数据准备、多种分布式训练策略和推理测试。

> **说明**：LongCat-Video 是支持文生视频（T2V）和图生视频（I2V）的视频生成模型。本指南涵盖 LoRA 微调训练流程，适用于自定义数据集的微调场景。
...
| `--seed` | 随机种子（可复现训练） | 42 |
| `--output_dir` | 输出目录 | `output_dir_longcatvideo_lora` |
...
| `--target_name` | 应用 LoRA 的组件/模块，用逗号分隔 | `qkv,q_linear,kv_linear,ffn.w1,ffn.w2,ffn.w3` |
...
# 训练好的权重路径，如 "output_dir_longcatvideo_lora/checkpoint-xxx/lora_weights.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55
# 根据生成内容编写
...
# 根据实际模型路径
model_name = "models/Diffusion_Transformer/LongCat-Video"  
# LoRA 权重路径，如 "output_dir_longcatvideo_lora/checkpoint-xxx/lora_weights.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55
# 图生视频的起始图像

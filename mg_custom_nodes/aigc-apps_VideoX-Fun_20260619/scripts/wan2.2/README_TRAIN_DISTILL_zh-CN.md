...
本文档提供了将 Wan2.2 蒸馏的完整工作流,包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：Wan2.2 是一个支持文生视频（T2V）、图生视频（I2V）和文本图生视频（TI2V）的视频生成模型。Wan2.2采用双Transformer架构（高噪声/低噪声模型），该训练代码可以将推理步数从 25-50 步减少到 4-8 步，同时保持视频生成质量。
...
- [四、推理测试](#四推理测试)
  ...
  - [4.3.1 文本图生视频（TI2V）推理](#431-文本图生视频ti2v推理)
...
- **边界类型（boundary_type）**：
  - `low`：训练低噪声模型，高噪声模型使用预训练权重（推荐用于T2V/I2V蒸馏）
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.2_distill` |
...
**Wan2.2 蒸馏训练策略建议**：
- **T2V模型（双Transformer）**：使用 `boundary_type="low"` 和 `train_mode="normal"` 蒸馏低噪声模型，这样可以保持高噪声部分的通用性，同时微调低噪声部分以实现快速推理。
- **I2V模型（双Transformer）**：使用 `boundary_type="low"` 和 `train_mode="i2v"` 蒸馏低噪声模型，数据集需要包含参考图像。
- **TI2V模型（单Transformer）**：使用 `boundary_type="full"` 和 `train_mode="ti2v"` 蒸馏单模型，数据集需要包含参考图像。TI2V模型支持在推理时动态切换T2V和I2V模式。
- **显存优化**：Wan2.2模型较大（14B/5B参数），强烈建议使用 `--low_vram` 和 `--gradient_checkpointing`。
- **多卡训练**：对于14B模型，推荐使用FSDP或DeepSpeed-Zero-2/3进行多卡训练；5B模型可以使用单卡或少数卡训练。
- **蒸馏步数**：默认的 `--denoising_step_indices_list=1000 750 500 250` 对应4步蒸馏，可以根据需要调整为8步或其他配置。
...
| `transformer_path` | 加载训练好的低噪声 Transformer 权重路径 | `None` 或 `output_dir_wan2.2_distill/checkpoint-xxx/diffusion_pytorch_model.safetensors` |
...
# 训练好的低噪声权重路径，如 "output_dir_wan2.2_distill/checkpoint-xxx/diffusion_pytorch_model.safetensors"
...
# 训练好的权重路径，如 "output_dir_wan2.2_distill/checkpoint-xxx/diffusion_pytorch_model.safetensors"
transformer_path = None  

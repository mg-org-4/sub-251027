# CogVideoX-Fun 全参数训练指南

本文档提供 CogVideoX-Fun Diffusion Transformer 全参数训练的完整流程,包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：CogVideoX-Fun 是一个支持文生视频（T2V）、图生视频（I2V）和视频生视频（V2V）的视频生成模型。本文档涵盖普通视频生成任务的训练流程。
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_cogvideox_fun` |
...
- `train_mode="normal"`：标准文生视频模式。如果只需要文生视频功能，可以移除此参数或设置为 `normal`。
...
| `save_path` | 生成视频保存路径 | `samples/cogvideox-fun-videos-i2v` |
...
# 训练好的权重路径，如 "output_dir_cogvideox_fun/checkpoint-xxx/diffusion_pytorch_model.safetensors"

...
This document provides a complete workflow for LoRA (Low-Rank Adaptation) training of LTX-2 (Lightricks Text-to-Video), including environment configuration, data preparation, distributed training, and inference testing.

> **Note**: LTX-2 is an audio-visual generative video model that can simultaneously generate video and corresponding audio. The training data requires both video and audio files.
...
| `--seed` | Random seed | 42 |
| `--output_dir` | Output directory | `output_dir_ltx2_lora` |
...
| `lora_path` | LoRA weights path | `output_dir_ltx2_lora/pytorch_lora_weights.safetensors` |
...
Edit `examples/ltx2/predict_t2v.py` according to your needs. For LoRA inference, focus on these parameters:

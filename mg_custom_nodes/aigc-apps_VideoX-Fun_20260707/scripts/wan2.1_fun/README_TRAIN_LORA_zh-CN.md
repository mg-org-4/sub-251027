# Wan2.1 Fun LoRA 微调训练指南

本文档提供 Wan2.1 Fun LoRA 微调训练的完整流程,包括环境配置、数据准备、多种分布式训练策略和推理测试。

> **说明**：Wan2.1 Fun 是一个支持文生视频（T2V）、图生视频（I2V）和视频修复（Inpainting）的视频生成模型。本指南涵盖 LoRA 微调训练流程，适用于普通视频生成和视频修复任务的微调场景。
...
| `--seed` | 随机种子（可复现训练） | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.1_fun_lora` |
...
| `--low_vram` | 低显存模式 | - |
| `--train_mode` | 训练模式：`normal`（文生视频）或 `inpaint`（视频修复） | `inpaint` |
...
| `--validation_paths` | 验证 inpaint 的参考图像路径（仅 inpaint 模式） | `"asset/1.png"` |
...
- `inpaint` 模式必须提供 `--validation_paths` 参数
...
| `model_name` | 模型路径 | `models/Diffusion_Transformer/Wan2.1-Fun-V1.1-1.3B-InP` |
...
| `validation_image_start` | inpaint 模式的参考图像路径 | `"asset/1.png"` |
| `validation_image_end` | inpaint 模式的目标图像路径（纯 I2V 时设为 `None`） | `None` |
...
| `save_path` | 生成视频保存路径 | `samples/wan-videos-fun-t2v` 或 `samples/wan-videos-fun-i2v` |
...
根据需求修改编辑 `examples/wan2.1_fun/predict_t2v.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。
...
# 训练好的权重路径，如 "output_dir_wan2.1_fun_lora/checkpoint-xxx/checkpoint-xxx.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55
# 根据生成内容编写
...
根据需求修改编辑 `examples/wan2.1_fun/predict_i2v.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。
...
# 根据实际模型路径
model_name = "models/Diffusion_Transformer/Wan2.1-Fun-V1.1-1.3B-InP"  
# LoRA 权重路径，如 "output_dir_wan2.1_fun_lora/checkpoint-xxx/checkpoint-xxx.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55
# 图生视频/修复的起始图像
validation_image_start = "asset/1.png"
# inpaint 的目标图像（纯图生视频时设为 None）
...
编辑 `examples/wan2.1_fun/predict_t2v.py` 或 `examples/wan2.1_fun/predict_i2v.py`：

# Wan2.2-S2V Full Parameter Training Guide

This document provides a complete workflow for full parameter training of Wan2.2-S2V (audio-driven talking head video generation model), including environment configuration, data preparation, distributed training, and inference testing.

> **Note**: Wan2.2-S2V is an audio-driven talking head video generation model that requires both a reference image and an audio file to generate talking videos. The training data needs to include video, audio, and reference images.
...
> ⚠️ **Important**: Wan2.2-S2V is an audio-driven talking head video generation model. Unlike regular video training, you **must provide the `audio_path` field in metadata.json**.
...
- `audio_path`: Audio file path (**Wan2.2-S2V specific and required**, main difference from regular video training)
...
**Dataset Comparison: Wan2.2-S2V vs Regular Video Training**:

| Model Type | Required Fields | Audio Field |
|---------|---------|---------|
| Regular Video (WAN, CogVideoX, etc.) | `file_path`, `text`, `type` | ❌ Not needed |
| **Wan2.2-S2V (Audio-Driven Generation)** | `file_path`, `audio_path`, `type` | ✅ **Required** |
...
|-----|------|-------|
| `--config_path` | Model configuration file path | `config/wan2.2/wan_civitai_s2v.yaml` |
| `--pretrained_model_name_or_path` | Path to pretrained model | `models/Diffusion_Transformer/Wan2.2-S2V-14B` |
...
| `--video_sample_n_frames` | Number of frames to sample | 80 |
...
| `--seed` | Random seed | 42 |
| `--output_dir` | Output directory | `output_dir_wan2.2_s2v` |
...
| `--boundary_type` | Model type: `"full"` for single model, `"low"`/`"high"` for dual model | `"full"` |
| `--control_ref_image` | Reference image strategy: `"first_frame"` or `"random"` | `"random"` |
...
| `config_path` | Model configuration file path | `config/wan2.2/wan_civitai_s2v.yaml` |
| `model_name` | Model path | `models/Diffusion_Transformer/Wan2.2-S2V-14B` |
| `sampler_name` | Sampler type: `Flow`, `Flow_Unipc`, `Flow_DPM++` | `Flow` |
| `shift` | Sampler shift parameter | 3 |
...
| `segment_frame_length` | Number of frames per segment | 80 |
...
| `save_path` | Generated video save path | `samples/wan-videos-speech2v` |
...
Edit `examples/wan2.2/predict_s2v.py` according to your needs. For first-time inference, focus on these parameters. For other parameters, see the Inference Parameters section above.
...
# Trained weights path, e.g. "output_dir_wan2.2_s2v/checkpoint-xxx/diffusion_pytorch_model.safetensors"
...
Edit `examples/wan2.2/predict_s2v.py`:

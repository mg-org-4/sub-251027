...
本文档提供 LTX-2.3（Lightricks 文生视频模型）LoRA（Low-Rank Adaptation）训练的完整工作流，包括环境配置、数据准备、分布式训练和推理测试。
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_ltx2.3_lora` |
...
| `lora_path` | LoRA 权重路径 | `output_dir_ltx2.3_lora/pytorch_lora_weights.safetensors` |
...
根据需求编辑 `examples/ltx2.3/predict_t2v.py`。LoRA 推理请重点关注以下参数：

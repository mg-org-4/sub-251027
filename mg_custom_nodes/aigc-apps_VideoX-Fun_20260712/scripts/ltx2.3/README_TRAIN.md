# LTX-2.3 Full Parameter Training Guide

This document provides a complete workflow for full parameter training of LTX-2.3 (Lightricks Text-to-Video), including environment configuration, data preparation, distributed training, and inference testing.
...
| `--seed` | Random seed | 42 |
| `--output_dir` | Output directory | `output_dir_ltx2.3` |
...
Edit `examples/ltx2.3/predict_t2v.py` according to your needs. For first-time inference, focus on these parameters. For other parameters, see the Inference Parameters section above.
...
# Trained weights path, e.g. "output_dir_ltx2.3/checkpoint-xxx/diffusion_pytorch_model.safetensors"

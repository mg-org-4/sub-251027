# HunyuanVideo 全参数训练指南

本文档提供 HunyuanVideo Diffusion Transformer 全参数训练的完整流程，包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：HunyuanVideo 是一个支持文生视频（T2V）和图生视频（I2V）的视频生成模型。本文档涵盖普通视频生成任务的训练流程。
...
| `--low_vram` | 低显存模式 | - |
| `--train_mode` | 训练模式：`normal`（普通）或 `i2v`（图生视频，注：验证阶段尚未实现） | `normal` |
...
| `model_name` | 模型路径 | `models/Diffusion_Transformer/HunyuanVideo` |
...
根据需求修改编辑 `examples/hunyuanvideo/predict_t2v.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。
...
# 训练好的权重路径，如 "output_dir_hunyuanvideo/checkpoint-xxx/diffusion_pytorch_model.safetensors"
...
根据需求修改编辑 `examples/hunyuanvideo/predict_i2v.py`，初次推理重点关注如下参数，如果对其他参数感兴趣，请查看上方的推理参数解析。

```python
# 根据显卡显存选择
GPU_memory_mode = "model_group_offload"
# 根据实际模型路径（I2V 使用 HunyuanVideo-I2V 模型）
...
编辑 `examples/hunyuanvideo/predict_t2v.py` 或 `examples/hunyuanvideo/predict_i2v.py`：

# LongCat-Video LoRA Fine-tuning Training Guide

This document provides a complete guide for LongCat-Video LoRA fine-tuning training, including environment configuration, data preparation, multiple distributed training strategies, and inference testing.

> **Note**: LongCat-Video is a video generation model that supports Text-to-Video (T2V) and Image-to-Video (I2V). This guide covers the LoRA fine-tuning training process, suitable for custom dataset fine-tuning scenarios.
...
# Download LongCat-Video official weights
...
| `--output_dir` | Output directory | `output_dir_longcatvideo_lora` |
...
| `--train_mode` | Training mode: `normal` (regular) or `i2v` (image-to-video) | `normal` |
| `--resume_from_checkpoint` | Resume training from checkpoint path; use `"latest"` to auto-select the latest checkpoint | None |
| `--rank` | Dimension of LoRA update matrices (higher rank = stronger表达能力, but more VRAM) | 64 |
| `--network_alpha` | Scaling factor for LoRA update matrices (usually set to half of rank or same) | 32 |
| `--target_name` | Components/modules to apply LoRA, separated by commas | `qkv,q_linear,kv_linear,ffn.w1,ffn.w2,ffn.w3` |
...
```bash
  ...
  --validation_prompts="A young woman stands on a sunlit coastline, her white dress gently fluttering in the sea breeze."
```
...
| `model_name` | Model path | `models/Diffusion_Transformer/LongCat-Video` |
...
| `save_path` | Path to save generated video | `samples/longcat-videos-i2v` or `samples/longcat-videos-t2v` |
...
```bash
python examples/longcatvideo/predict_t2v.py
```

Modify `examples/longcatvideo/predict_t2v.py` according to your needs. For initial inference, focus on the following parameters. If you're interested in other parameters, please refer to the inference parameter explanation above.
...
# Path to trained weights, e.g., "output_dir_longcatvideo_lora/checkpoint-xxx/lora_weights.safetensors"
lora_path = None
...
```bash
python examples/longcatvideo/predict_i2v.py
```

Modify `examples/longcatvideo/predict_i2v.py` according to your needs. For initial inference, focus on the following parameters. If you're interested in other parameters, please refer to the inference parameter explanation above.
...
# Based on actual model path
model_name = "models/Diffusion_Transformer/LongCat-Video"  
# LoRA weights path, e.g., "output_dir_longcatvideo_lora/checkpoint-xxx/lora_weights.safetensors"
lora_path = None
# LoRA weight strength
lora_weight = 0.55
...
Edit `examples/longcatvideo/predict_t2v.py` or `examples/longcatvideo/predict_i2v.py`:
...
```bash
torchrun --nproc-per-node=2 examples/longcatvideo/predict_t2v.py
```

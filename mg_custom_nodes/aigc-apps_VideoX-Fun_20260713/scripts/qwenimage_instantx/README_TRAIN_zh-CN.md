# Qwen-Image InstantX ControlNet 全量参数训练指南

本文档提供 Qwen-Image InstantX ControlNet 模型训练的完整流程,包括环境配置、数据准备、分布式训练和推理测试。

在 Qwen-Image 训练中,可以选择使用 DeepSpeed 或 FSDP 来节省大量显存。

> **重要提示**: InstantX ControlNet 架构与 Fun Control 架构不同。InstantX 使用独立的 ControlNet 模型,而非在原 Transformer 中添加 control 模块。
...
| `--cn_pretrained_model_name_or_path` | ControlNet 预训练模型路径 | `models/Diffusion_Transformer/Qwen-Image-ControlNet-Union` |
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_qwen_image_instantx_control` |
...
| `--controlnet_path` | 加载预训练的 ControlNet 权重(可选) | `None` |
| `--trainable_modules` | 可训练模块(`"."` 表示训练所有模块,或指定具体模块名) | `"."` |
...
| `--enable_text_encoder_in_dataloader` | 在dataloader中编码文本,节省显存 | - |
...
| `config_path` | 配置文件路径 | 无(InstantX 不使用此参数) |
| `model_name` | Transformer 模型路径 | `models/Diffusion_Transformer/Qwen-Image` |
| `model_name_controlnet` | ControlNet 模型路径 | `models/Diffusion_Transformer/Qwen-Image-ControlNet-Union` |
| `sampler_name` | 采样器类型:`Flow`、`Flow_Unipc`、`Flow_DPM++` | `Flow` |
| `transformer_path` | 加载训练好的 Transformer 权重路径(可选) | `None` |
| `controlnet_path` | 加载训练好的 ControlNet 权重路径(可选) | `output_dir/checkpoint-xxx/diffusion_pytorch_model.safetensors` |
...
| `controlnet_conditioning_scale` | ControlNet 条件权重,推荐值 0.80 | 0.80 |
...
根据需求修改编辑 `examples/qwenimage_instantx/predict_t2i_control.py`,初次推理重点关注如下参数,如果对其他参数感兴趣,请查看上方的推理参数解析。
...
# InstantX ControlNet 模型路径
model_name_controlnet = "models/Diffusion_Transformer/Qwen-Image-ControlNet-Union"  
# 训练好的 Transformer 权重路径(可选),如 "output_dir_qwen_image_instantx_control/checkpoint-xxx/"
transformer_path = None  
# 训练好的 ControlNet 权重路径(可选),如 "output_dir_qwen_image_instantx_control/checkpoint-xxx/"
...
生成结果会保存在 `samples/qwenimage-t2i-instantx-control` 目录下。
...
编辑 `examples/qwenimage_instantx/predict_t2i_control.py`:

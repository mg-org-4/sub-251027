...
本文档提供 Wan2.1 LoRA 微调训练的完整流程,包括环境配置、数据准备、多种分布式训练策略和推理测试。

> **说明**：Wan2.1 是一个支持文生视频（T2V）和图生视频（I2V）的视频生成模型。本指南涵盖 LoRA 微调训练流程，适用于自定义数据集的微调场景。
...
| `--seed` | 随机种子（可复现训练） | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.1_lora` |

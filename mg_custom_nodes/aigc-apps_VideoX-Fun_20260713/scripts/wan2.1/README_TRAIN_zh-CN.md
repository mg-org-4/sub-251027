...
本文档提供 Wan2.1 Diffusion Transformer 全参数训练的完整流程，包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：Wan2.1 是一个支持文生视频（T2V）和图生视频（I2V）的视频生成模型。本文档涵盖普通视频生成任务的训练流程。
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.1` |
...
# 训练好的权重路径，如 "output_dir_wan2.1/checkpoint-xxx/diffusion_pytorch_model.safetensors"

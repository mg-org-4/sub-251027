...
本文档提供 Wan2.1 Fun Diffusion Transformer 全参数训练的完整流程，包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：Wan2.1 Fun 是一个支持文生视频（T2V）、图生视频（I2V）和视频修复（Inpaint）的视频生成模型。本文档涵盖普通视频生成和视频修复任务的训练流程。
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.1_fun` |
...
| `--low_vram` | 低显存模式 | - |
| `--train_mode` | 训练模式：`inpaint`（视频修复） | `inpaint` |
...
| `validation_image_start` | 图生视频/视频修复的参考图像路径（Inpaint 模式） | `"asset/1.png"` |
| `validation_image_end` | 视频修复的目标图像路径（仅 Inpaint 模式） | `None` |
...
# 训练好的权重路径，如 "output_dir_wan2.1_fun/checkpoint-xxx/diffusion_pytorch_model.safetensors"

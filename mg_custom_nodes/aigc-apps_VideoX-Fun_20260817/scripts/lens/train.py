...
def _patchify_latents(latents):
    """2x2 patchify: [B, C, H, W] -> [B, 4*C, H/2, W/2]"""
...
# Lens chat-template constants (mirror videox_fun.pipeline.pipeline_lens).
_LENS_CHAT_SYSTEM = (
...
def encode_prompt(
    ...
    """Encode prompts with the Lens GPT-OSS encoder.

    Mirrors :meth:`LensPipeline._build_chat_inputs` /
    :meth:`LensPipeline._get_text_embeddings`. Returns a list of per-layer
    feature tensors (length == ``len(text_encoder._lens_selected_layers)``)
    and a bool attention mask of shape ``[B, S_txt]`` (True = valid).
    """
    ...
    for p in prompts:
        conversation = [
            {"role": "system", "content": _LENS_CHAT_SYSTEM, "thinking": None},
            {"role": "user", "content": p, "thinking": None},
            {"role": "assistant", "thinking": _LENS_CHAT_ASSISTANT_THINKING, "content": ""},
        ]
    ...
    # NOTE: Call text_encoder(...) directly instead of text_encoder.encode_layers(...).
    # When text_encoder is wrapped by FSDP, attribute access via .encode_layers
    # returns the bound method on the *inner* (unwrapped) module, so calling
    # ``self(...)`` inside encode_layers bypasses FSDP's pre-forward hook and
    # the root flat_param (containing embed_tokens.weight) is never unsharded,
    # causing "tensor data is not allocated yet" at embed_tokens.
    layer_outputs = text_encoder.encode_layers(input_ids=input_ids, attention_mask=attn_mask)

    offset = _LENS_TXT_OFFSET
    if input_ids.shape[1] > offset:
        features = [
            feat[:, offset:, :].contiguous().to(dtype) for feat in layer_outputs
        ]
        mask = attn_mask[:, offset:].bool()
    else:
        zero_shape = (input_ids.shape[0], 0, layer_outputs[0].shape[-1])
        features = [
            layer_outputs[0].new_zeros(zero_shape).to(dtype) for _ in layer_outputs
        ]
        mask = torch.zeros(
...
def main():
    ...
    # `from_pretrained` So Mistral3Model and AutoencoderKLFlux2 will not enjoy the parameter sharding
    # across multiple gpus and only transformer3d will get ZeRO sharded.
    ...
    # Get Transformer
    transformer3d = LensTransformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, 
    ...
    # the Lens DiT (multi-layer feature extraction).
    text_encoder.set_selected_layers(transformer3d.config.selected_layer_index)
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            # NOTE: enable_text_encoder_in_dataloader is intentionally ignored
            # for Lens. The text encoder produces multi-layer feature lists +
            # masks, which are awkward to ship across dataloader workers; we
            # always encode in the main process below.
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                with torch.no_grad():
                    ...
                    if vae_stream_1 is not None:
                        ...
                        with torch.cuda.stream(vae_stream_1):
                            latents = _batch_encode_vae(pixel_values)  # [B, C, H, W] - NO unsqueeze
                    else:
                        latents = _batch_encode_vae(pixel_values)  # [B, C, H, W] - NO unsqueeze
                ...
                # Spatial token count after 2x2 patchify, matching the inference pipeline
                # (pipeline_lens.LensPipeline.__call__: seq_len = latent_h * latent_w).
                image_seq_len = latents.shape[-2] * latents.shape[-1]

                # Setup scheduler with linear sigmas (matching inference pipeline).
                # FlowMatchEulerDiscreteScheduler with use_dynamic_shifting=True
                # requires `mu`; reuse the empirical curve used at inference time.
                mu = compute_empirical_mu(image_seq_len, args.train_sampling_steps)
                sigmas_init = torch.linspace(1.0, 0.0, args.train_sampling_steps + 1)
                noise_scheduler.set_timesteps(
                    sigmas=sigmas_init[:-1], device=latents.device, mu=mu
                )
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                    # Lens DiT consumes [B, S_img, C] tokens with img_shapes.
                    lat_h, lat_w = noisy_latents.shape[-2], noisy_latents.shape[-1]
                    noisy_seq = rearrange(noisy_latents, "b c h w -> b (h w) c")
                    img_shapes = [(1, lat_h, lat_w)]
                    noise_pred_seq = transformer3d(
                    ...
                    # proj_out width is patch_size**2 * out_channels = 4 * 32 = 128,
                    # which already matches the BN-normalized patchified latent space.
                    noise_pred = rearrange(
                        noise_pred_seq, "b (h w) c -> b c h w", h=lat_h, w=lat_w
                    )

# Modified from https://github.com/huggingface/diffusers/blob/main/src/diffusers/models/autoencoders/autoencoder_kl_minimax_h3.py
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        for resnet in self.resnets:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(resnet, hidden_states, **ckpt_kwargs)
            else:
...
class MiniMaxH3VideoRotaryPosEmbed(nn.Module):
    ...
    def _compute_inv_freq(self, device: Optional[torch.device] = None) -> torch.Tensor:
        return 1.0 / self.theta ** torch.arange(
            0, 1, 2 * self.num_axes / self.dim, dtype=torch.float32, device=device
        )
...
class MiniMaxH3VideoAttnProcessor:
    def __call__(
        ...
        # The decode recipe is float16 autocast over float32 weights, so the half-precision fast path is taken under
        # autocast and a float32 decode stays in float32 rather than being rounded down by the kernel.
...
class MiniMaxH3VideoAttention(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, rotary_emb: Optional[Tuple[torch.Tensor, torch.Tensor]] = None
    ) -> torch.Tensor:
        ...
        for block in self.transformer_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(
                    block, hidden_states, rotary_emb, **ckpt_kwargs
                )
            else:
...
class AutoencoderKLMiniMaxH3(MiniMaxH3MixedPrecisionLoaderMixin, ModelMixin, ConfigMixin):
    r"""
    ...
    def from_pretrained_original(cls, checkpoint_path, torch_dtype=None, device=None):
        r"""
        Load the video VAE of an *original* MiniMax-H3 checkpoint partition (e.g. `MiniMax-H3/FL2VA`) without
        converting it on disk.

        The module tree is built empty on the meta device — with `latents_mean` / `latents_std` read from the
        original `video_vae/config.json` wrapper — and the original weights, which live one level deeper
        (`video_vae/source/...`), are streamed through the shared key / tensor mapping of `minimax_h3_conversion`
        and assigned in place. The released weights are float32 and the decode recipe is float16 autocast over
        float32 weights, so the dtype stays float32 whatever the rest of the pipeline uses; ``torch_dtype`` is
        accepted for interface compatibility with the transformer and silently ignored.

        Args:
            checkpoint_path (`str` or `os.PathLike`):
                An original MiniMax-H3 partition folder holding `video_vae/`.
            torch_dtype (`torch.dtype`, *optional*):
                Ignored; the VAE always stays float32 as released.
        ...
        from .minimax_h3_conversion import (
            ...
            iter_original_video_vae_tensors,
            read_original_video_vae_wrapper_config,
        )
        ...
        with torch.device("meta"):
            model = cls(
                **MINIMAX_H3_VIDEO_VAE_CONFIG,
                latents_mean=tuple(wrapper_config["latents_mean"]),
                latents_std=tuple(wrapper_config["latents_std"]),
            )
        # The decoder's rotary frequencies are a float32 constant of the config rather than a checkpoint weight, and
        # the meta build left the buffer as a meta tensor: recompute it on the CPU.
        model.decoder.rope.inv_freq = model.decoder.rope._compute_inv_freq()
        assign_original_tensors(model, iter_original_video_vae_tensors(checkpoint_path))
    ...
    def _set_gradient_checkpointing(self, *args, **kwargs):
        ...
        for down_block in self.encoder.down_blocks:
            down_block.gradient_checkpointing = value

    def enable_slicing(self) -> None:
        r"""Decode one video of a batch at a time, which lowers the memory requirement."""
        self.use_slicing = True

    def disable_slicing(self) -> None:
        r"""Decode a whole batch at once again."""
        self.use_slicing = False
    ...
    def enable_tiling(
        ...
        tile_sample_min_overlap_height: Optional[int] = None,
        tile_sample_min_overlap_width: Optional[int] = None,
    ) -> None:
        r"""
    ...
    def disable_tiling(self) -> None:
        r"""
        Turn tiling off. MiniMax-H3 was released with tiling enabled and the released frames are the blended-tile
        ones, so this changes the output.
        """
    ...
    def encode(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether to return an [`~models.autoencoders.autoencoder_kl.AutoencoderKLOutput`] instead of a plain
                tuple.

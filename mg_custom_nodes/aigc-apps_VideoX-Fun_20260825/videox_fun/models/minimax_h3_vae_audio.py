# Modified from https://github.com/huggingface/diffusers/blob/main/src/diffusers/models/autoencoders/autoencoder_kl_minimax_h3_audio.py
...
That includes `torch.nn.utils.weight_norm` (the `weight_g` / `weight_v` spelling) and the registered
Kaiser-window resampling `filter` buffers of the anti-aliased activations.
"""
...
class MiniMaxH3AudioAttnProcessor:
    r"""Processor of [`MiniMaxH3AudioCausalAttention`]. The causal mask is expressed as a flag rather
    than as a materialized mask."""
    ...
    def __call__(self, attn: "MiniMaxH3AudioCausalAttention", hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # The released checkpoint is float32; the half-precision fast path is only taken when a caller runs the
        # autoencoder under autocast or in half precision.
        if query.dtype in (torch.float16, torch.bfloat16):
            hidden_states = attention(query, key, value, causal=True)
        else:
...
class AutoencoderKLMiniMaxH3Audio(MiniMaxH3MixedPrecisionLoaderMixin, ModelMixin, ConfigMixin):
    r"""
    ...
    def from_pretrained_original(cls, checkpoint_path, torch_dtype=None, device=None):
        r"""
        Load the audio VAE of an *original* MiniMax-H3 checkpoint partition (e.g. `MiniMax-H3/FL2VA`) without
        converting it on disk.

        The module tree is built empty on the meta device with the constructor kwargs of `audio_vae/metadata.json`
        (cross-checked against `audio_vae/config.json`, which also carries the per-channel `latents_mean` /
        `latents_std`), and the original weights — whose naming already matches this module tree one for one — are
        assigned in place. The released weights are float32, and so is the decode recipe, whatever the rest of the
        pipeline uses; ``torch_dtype`` is accepted for interface compatibility with the transformer and silently
        ignored.

        Args:
            checkpoint_path (`str` or `os.PathLike`):
                An original MiniMax-H3 partition folder holding `audio_vae/`.
            torch_dtype (`torch.dtype`, *optional*):
                Ignored; the audio VAE always stays float32 as released.
        ...
        from .minimax_h3_conversion import (
            ...
            load_original_audio_vae_state_dict,
        )

        with torch.device("meta"):
            model = cls(**get_audio_vae_config(checkpoint_path))
        assign_original_tensors(
            model, load_original_audio_vae_state_dict(checkpoint_path).items()
        )
    ...
    def encode(
    ...
    ) -> Union[MiniMaxH3AudioEncoderOutput, Tuple[MiniMaxH3AudioDiagonalGaussianDistribution]]:
        r"""
        ...
        always consumes the posterior **mean** (`latent_dist.mode()`).
    ...
    @apply_forward_hook
    def decode(self, latents: torch.Tensor, return_dict: bool = True) -> Union[DecoderOutput, Tuple[torch.Tensor]]:
        r"""

...
# The legacy `torch.nn.utils.weight_norm` spelling (`weight_g` / `weight_v`), kept by some audio
# autoencoders (e.g. the MiniMax-H3 / DAC-lineage ones) for checkpoint-name compatibility.
try:
    from torch.nn.utils.weight_norm import WeightNorm as _LegacyWeightNorm
except ImportError:
    _LegacyWeightNorm = None

...
本文档提供了将 Wan2.1 蒸馏的完整工作流，包括环境配置、数据准备、分布式训练和推理测试。

> **说明**：Wan2.1 是一个支持文生视频（T2V）和图生视频（I2V）的视频生成模型。该训练代码可以将推理步数从 25-50 步减少到 4-8 步，同时保持视频生成质量。
...
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_wan2.1_distill` |
...
| `--generator_transformer_path` | 从 DMD 权重热启动 generator 与 fake score | `output_dir_wan2.1_distill/checkpoint-xxx/diffusion_pytorch_model.safetensors` |
...
| `transformer_path` | 加载训练好的 Transformer 权重路径 | `None` 或 `output_dir_wan2.1_distill/checkpoint-xxx/diffusion_pytorch_model.safetensors` |
...
# 训练好的权重路径，如 "output_dir_wan2.1_distill/checkpoint-xxx/diffusion_pytorch_model.safetensors"

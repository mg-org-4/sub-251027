# Modified from https://github.com/huggingface/diffusers/blob/main/src/diffusers/schedulers/scheduling_minimax_h3.py
...
class MiniMaxH3Scheduler(SchedulerMixin, ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path=None, **kwargs):
        r"""
        Load a scheduler from either an original MiniMax-H3 checkpoint or a diffusers layout.

        An original MiniMax-H3 partition carries the two sigma shifts (``video`` and ``audio``) in its root
        ``model_index.json`` instead of a scheduler subfolder. This override reads them from there and builds
        the scheduler directly, so the caller never has to branch on the checkpoint format.

        The modality is inferred from ``subfolder``: ``"scheduler"`` → video, ``"audio_scheduler"`` →
        audio.
        """
        import os
        ...
        if subfolder in ("scheduler", "audio_scheduler") and pretrained_model_name_or_path is not None:
            ...
            if os.path.isdir(pretrained_model_name_or_path) and is_raw_minimax_h3_format(pretrained_model_name_or_path):
                ...
                modality = "video" if subfolder == "scheduler" else "audio"
                return cls(shift=float(shifts[modality]))
    ...
    def set_shift(self, shift: float) -> None:
        ...
        if shift <= 0:
            raise ValueError(f"`shift` must be positive, got {shift}.")
        self._shift = float(shift)
        # Keep the frozen config in step with the override, so a `save_pretrained` after a per-request
        # `flow_shift` / `audio_flow_shift` override stores the shift the scheduler actually applies.
        self._internal_dict["shift"] = self._shift
        if "shift" in self.config:
            self.config["shift"] = self._shift
    ...
    ) -> Union[MiniMaxH3SchedulerOutput, Tuple]:
        r"""

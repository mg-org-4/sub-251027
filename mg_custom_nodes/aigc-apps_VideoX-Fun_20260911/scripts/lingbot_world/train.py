...
from videox_fun.data import (ASPECT_RATIO_512, ASPECT_RATIO_RANDOM_CROP_512,
                             ...
                             LingbotImageVideoDataset,
                             RandomSampler, get_closest_ratio, get_random_mask,
                             prepare_lingbot_dit_cond_dict_from_c2ws)
    ...
    try:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
            ...
            if args.boundary_type == "full":
                ...
                # Reuse the same class as the trainable model so the paired
                # transformer keeps the camera-injection layers when enabled.
                _CompanionCls = WanTransformer3DModel_LingbotWorld if args.enable_camera_control else Wan2_2Transformer3DModel
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Enable LingBot-World camera-pose (plücker) conditioning during "
            "training. When set, the training script uses "
            "``WanTransformer3DModel_LingbotWorld`` and "
            "``LingbotImageVideoDataset``, and expects each video entry in the "
            "annotation file to include an ``action_path`` field pointing to a "
            "directory containing ``poses.npy`` and ``intrinsics.npy``."
        ),
    )
    parser.add_argument(
        "--action_data_root",
        ...
        default=None,
        help=(
            "Root directory used to resolve relative ``action_path`` entries. "
            "Defaults to ``--train_data_dir`` when unset."
        ),
    )
    parser.add_argument(
        "--control_type",
        ...
        default="cam",
        choices=["cam"],
        help="LingBot-World control type. Only ``cam`` (6-dim plücker) is supported.",
    )
    parser.add_argument(
...
def main():
    ...
    if args.boundary_type == "low" or args.boundary_type == "full":
        ...
        sub_path = config['transformer_additional_kwargs'].get('transformer_high_noise_model_subpath', 'transformer')
    # LingBot-World reuses the Wan2.2 I2V backbone with extra camera-injection
    # layers when ``--enable_camera_control`` is set; otherwise fall back to the
    # stock Wan2.2 transformer.
    TransformerCls = WanTransformer3DModel_LingbotWorld if args.enable_camera_control else Wan2_2Transformer3DModel
    ...
    # Get the dataset
    if args.enable_camera_control:
        # LingBot-World variant: additionally load per-video ``poses.npy`` /
        # ``intrinsics.npy`` for camera-pose conditioning.
        train_dataset = LingbotImageVideoDataset(
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            # LingBot-World camera-pose control (variable length per sample).
            if args.enable_camera_control:
                new_examples["action_c2ws"] = []
                new_examples["action_intrinsics"] = []
                new_examples["action_intrinsics_org_hw"] = []
            ...
            for example in examples:
                ...
                if args.enable_camera_control:
                    # Camera trajectories are variable-length across samples,
                    # so keep them as a python list; batch_size is typically 1.
                    new_examples["action_c2ws"].append(example.get("action_c2ws", None))
                    new_examples["action_intrinsics"].append(example.get("action_intrinsics", None))
                    new_examples["action_intrinsics_org_hw"].append(example.get("action_intrinsics_org_hw", None))
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                    # LingBot-World: build the per-token camera condition from
                    # the (per-sample) trajectory that the dataset loaded, and
                    # hand it to the transformer via ``dit_cond_dict``. When no
                    # trajectory is available (image samples, missing files)
                    # the branch degenerates to the plain Wan2.2 forward.
                    lingbot_kwargs = {}
                    if args.enable_camera_control:
                        action_c2ws_batch = batch.get("action_c2ws", None)
                        action_intrinsics_batch = batch.get("action_intrinsics", None)
                        action_intrinsics_org_hw_batch = batch.get("action_intrinsics_org_hw", None)
                        sample_c2ws = action_c2ws_batch[0] if action_c2ws_batch else None
                        sample_intrinsics = action_intrinsics_batch[0] if action_intrinsics_batch else None
                        # The calibration resolution is guaranteed by the dataset
                        # (samples missing it are filtered out during loading), so
                        # a present trajectory always comes with a valid value.
                        sample_intrinsics_org_hw = action_intrinsics_org_hw_batch[0] if action_intrinsics_org_hw_batch else None
                        if sample_c2ws is not None and sample_intrinsics is not None and sample_intrinsics_org_hw is not None:
                            cur_intrinsics_org_height, cur_intrinsics_org_width = sample_intrinsics_org_hw
                            _, _, cur_num_frames, cur_h_lat, cur_w_lat = latents.size()
                            cur_h = cur_h_lat * vae.config.spatial_compression_ratio
                            cur_w = cur_w_lat * vae.config.spatial_compression_ratio
                            cur_frame_num = (cur_num_frames - 1) * vae.config.temporal_compression_ratio + 1
                            dit_cond_dict, _ = prepare_lingbot_dit_cond_dict_from_c2ws(
                                c2ws=sample_c2ws,
                                ...
                                intrinsics_org_height=cur_intrinsics_org_height,
                                intrinsics_org_width=cur_intrinsics_org_width,
                            )
                            lingbot_kwargs["dit_cond_dict"] = dit_cond_dict

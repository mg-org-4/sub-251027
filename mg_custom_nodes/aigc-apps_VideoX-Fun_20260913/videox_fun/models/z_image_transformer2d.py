...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def from_pretrained(
                ...
                def initialize_missing_parameters(missing_keys, model_state_dict, torch_dtype=None):
                    ...
                    with torch.no_grad():
                        for key in missing_keys:
                            ...
                            if "control" in key and key.replace("control_", "") in filtered_state_dict.keys() and model_state_dict[key].size() == filtered_state_dict[key.replace("control_", "")].size():
        ...
        for key in model_state_dict:
            if "control" in key and key.replace("control_", "") in state_dict.keys() and model_state_dict[key].size() == state_dict[key.replace("control_", "")].size():

...
def encode_prompt(
    ...
    text_in_dim: int = None,
) -> Tuple[torch.FloatTensor, torch.LongTensor]:
    """Encode text prompts to embeddings (Ernie-Image style)."""
    ...
    # Pad text embeddings to batch tensor
    B = len(text_hiddens)
    if B == 0:
        text_bth = torch.zeros((0, 0, text_in_dim), device=device, dtype=dtype)
        text_lens = torch.zeros((0,), device=device, dtype=torch.long)
    else:
        normalized = [
            th.squeeze(1).to(device).to(dtype) if th.dim() == 3 else th.to(device).to(dtype) for th in text_hiddens
        ]
        text_lens = torch.tensor([t.shape[0] for t in normalized], device=device, dtype=torch.long)
...
def main():
    ...
    # Get Transformer
    transformer3d = ErnieImageTransformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, 
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            if args.enable_text_encoder_in_dataloader:
                prompt_embeds, text_lens = encode_prompt(
                    batch['text'], device="cpu",
                    text_encoder=text_encoder, 
                    tokenizer=tokenizer,
                    dtype=weight_dtype,
                    text_in_dim=transformer3d.config.text_in_dim,
                )

                new_examples['prompt_embeds'] = prompt_embeds
                new_examples['text_lens'] = text_lens
    ...
    if fsdp_stage != 0 or zero_stage != 0:
        ...
        shard_fn = partial(shard_model, device_id=accelerator.device, param_dtype=weight_dtype, module_to_wrapper=text_encoder.vision_tower.transformer.layers)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.enable_text_encoder_in_dataloader:
                    prompt_embeds, text_lens = batch['prompt_embeds'].to(dtype=latents.dtype, device=accelerator.device), batch['text_lens'].to(accelerator.device)
                else:
                    with torch.no_grad():
                        prompt_embeds, text_lens = encode_prompt(
                ...
                noise_scheduler.set_timesteps(sigmas=sigmas_init[:-1], device=latents.device)

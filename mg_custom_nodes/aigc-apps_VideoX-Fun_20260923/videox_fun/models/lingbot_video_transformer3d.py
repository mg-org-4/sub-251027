# Modified from https://github.com/Robbyant/lingbot-video/blob/main/lingbot_video/transformer_lingbot_video.py
# Copyright 2024-2025 The Alibaba Wan Team Authors. All rights reserved.
#
# LingBot-Video transformer for first-frame conditioned video generation.
# Reference: https://github.com/Robbyant/lingbot-video
#
# Unlike the Wan2.2 backbone, this is a single-stream joint DiT: the video
# patch tokens and the Qwen3-VL encoded text tokens (including the first-frame
# visual tokens) are concatenated and attend to each other in one sequence,
# so it is implemented as a standalone HF-style model.
import math
import os
import types
...
from ..dist import (get_sequence_parallel_rank,
                    ...
                    usp_attn_lingbot_video_forward)
...
_FLOAT8_DTYPES = tuple(
    dtype for dtype in (getattr(torch, name, None) for name in ("float8_e4m3fn", "float8_e5m2"))
...
def make_joint_position_ids(
    ...
    grid_w: int,
    device: torch.device,
    padded_text_len: Optional[int] = None,
) -> torch.Tensor:
    """3D positions in [video; text] order. Text t-axis is 1..text_len; video t-axis starts at text_len+1.

    `padded_text_len` extends the text axis for right-padded prompts (padded tokens
    are masked in attention, so their positions never influence valid tokens).
    """
...
class LingBotVideoAttention(nn.Module):
    ...
    def forward(self, x, rotary_emb, attention_mask=None, num_video_tokens=None):
        # num_video_tokens is only meaningful for the sequence-parallel replacement
        # in dist/lingbot_video_xfuser.py; a single rank attends over the whole
        # joint sequence at once.
        B, S, _ = x.shape
        ...
        # videox_fun attention expects and returns (B, S, H, D)
        out = attention(q, k, v, attn_mask=attention_mask)
    ...
    def _run_experts_for_loop(self, tokens: torch.Tensor, counts: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx, expert_tokens in enumerate(splits):
            if expert_tokens.numel() == 0:
                continue
            # Expert weights are raw Parameters, so the fp8 Linear wrapper cannot
            # dequantize them; cast per expert to the token dtype instead.
            w1 = self.experts.w1[expert_idx].to(expert_tokens.dtype)
            w2 = self.experts.w2[expert_idx].to(expert_tokens.dtype)
            w3 = self.experts.w3[expert_idx].to(expert_tokens.dtype)
            h = F.silu(expert_tokens @ w1.transpose(-2, -1))
            h = h * (expert_tokens @ w3.transpose(-2, -1))
            h = h @ w2.transpose(-2, -1)
            outputs.append(h)
        if not outputs:
...
class LingBotVideoBlock(nn.Module):
    ...
    def forward(
        ...
        # AdaLN mod: dense and MoE both keep scale_shift_table fp32
        ...
        if bulk_dtype in _FLOAT8_DTYPES:
            # fp8-quantized weights are dequantized inside the Linear wrapper, so the
            # bulk compute dtype is the one the residual stream already carries.
            bulk_dtype = x.dtype
        attn_in = (self.norm1(x) * scale_msa + shift_msa).to(bulk_dtype)
        attn_out = self.attn(attn_in, rotary_emb, attention_mask, num_video_tokens)
...
class LingBotVideoTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    r"""
    LingBot-Video diffusion backbone for first-frame conditioned video generation.

    A single-stream joint DiT: the patchified video latent tokens and the
    Qwen3-VL encoded text tokens (including the first-frame visual tokens) are
    concatenated into one sequence and processed with full self-attention, so
    no cross-attention branch exists. The diffusion timestep is passed as
    ``sigma * 1000``. When ``num_experts > 0`` every block routes its FFN
    through a sparse MoE layer, turning the model into the MoE 30B-A3B variant.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            in_channels (`int`, *optional*, defaults to 16):
                Input video latent channels (C_in)
            out_channels (`int`, *optional*, defaults to 16):
                Output video latent channels (C_out)
            ...
            intermediate_size (`int`, *optional*, defaults to 6144):
                Intermediate dimension of the dense feed-forward network
            text_dim (`int`, *optional*, defaults to 2560):
                Input dimension of the Qwen3-VL encoded text embeddings
            freq_dim (`int`, *optional*, defaults to 256):
                Dimension for sinusoidal time embeddings
            norm_eps (`float`, *optional*, defaults to 1e-6):
                Epsilon for the RMSNorm / LayerNorm layers
            rope_theta (`float`, *optional*, defaults to 256.0):
                Base period of the rotary position embedding
            axes_dims (`tuple`, *optional*, defaults to (32, 48, 48)):
                Per-axis RoPE dimensions (t, h, w), must sum to the head dim
            axes_lens (`tuple`, *optional*, defaults to (8192, 1024, 1024)):
                Maximum per-axis RoPE positions (t, h, w)
            qkv_bias (`bool`, *optional*, defaults to False):
                Enable bias in the attention qkv projection
            out_bias (`bool`, *optional*, defaults to True):
                Enable bias in the attention output projection
            patch_embed_bias (`bool`, *optional*, defaults to True):
                Enable bias in the patch embedding linear layer
            timestep_mlp_bias (`bool`, *optional*, defaults to True):
                Enable bias in the timestep embedding MLP
            num_experts (`int`, *optional*, defaults to 0):
                Number of MoE experts per block, 0 keeps a dense FFN
            num_experts_per_tok (`int`, *optional*, defaults to 8):
                Number of experts activated per token
            moe_intermediate_size (`int`, *optional*, defaults to 512):
                Intermediate dimension of the routed experts
            decoder_sparse_step (`int`, *optional*, defaults to 1):
                Every Nth block uses MoE, the others stay dense
            mlp_only_layers (`tuple`, *optional*, defaults to ()):
                Block indices forced to use the dense FFN
            n_shared_experts (`int`, *optional*, defaults to None):
                Number of always-on shared experts per block
            score_func (`str`, *optional*, defaults to "sigmoid"):
                Router scoring function ("sigmoid" or "softmax")
            ...
            topk_group (`int`, *optional*, defaults to None):
                Number of expert groups selected per token
            routed_scaling_factor (`float`, *optional*, defaults to 1.0):
                Scaling factor applied to the routed expert outputs
        """
        super().__init__()
    ...
    def enable_multi_gpus_inference(self):
        ...
        self.all_gather = self.sp_group.all_gather

        for block in self.blocks:
            block.attn.forward = types.MethodType(
                usp_attn_lingbot_video_forward, block.attn)
    ...
    def forward(
        ...
        The video patch tokens and the (padded) text tokens are concatenated
        into one joint sequence with a per-sample RoPE layout: the video
        t-axis positions start right after the real text length of the sample.

        Args:
            hidden_states (`torch.Tensor`):
                Noisy video latent, shape [B, C, T, H, W]
            timestep (`torch.Tensor`):
                Diffusion sigma scaled to [0, 1000] (= sigma * 1000), shape [B]
            encoder_hidden_states (`torch.Tensor`):
                Qwen3-VL encoded text (and first-frame visual) embeddings,
                shape [B, L, text_dim]
            encoder_attention_mask (`torch.Tensor`, *optional*):
                Text token validity mask, shape [B, L] with 1 for valid tokens
            return_dict (`bool`, *optional*, defaults to True):
                Whether to return a [`Transformer2DModelOutput`] instead of a
                plain tuple

        Returns:
            `Transformer2DModelOutput` or tuple:
                Predicted velocity (noise - x0) with shape [B, C, T, H, W]
        """
        ...
        # Right-padded text tokens receive positions beyond the real length; they are
        # masked in attention so this never affects valid tokens.
        rotary_parts = [
            self.rope(
                make_joint_position_ids(
                    text_lens_list[i], gt, gh, gw, device, padded_text_len=L
                )
            )
            for i in range(B)
        ]
        ...
        # Sequence parallel: shard the video tokens only and keep the text tokens
        # replicated on every rank (they are passed to attention as joint tensors).
        # Positions are sliced from the full RoPE table, so each rank keeps its
        # global positions and no padding token is introduced.
        sp_world_size = self.sp_world_size
        num_video_tokens = n_video
        if sp_world_size > 1:
            if encoder_attention_mask is not None and bool((text_lens < L).any()):
                raise ValueError(
                    "Sequence-parallel inference cannot mask padded text tokens; "
                    "pass unpadded text embeddings (batch size 1)."
                )
            if n_video % sp_world_size != 0:
                raise ValueError(
                    f"Video token count {n_video} (= {gt}x{gh}x{gw}) is not divisible by "
                    f"the sequence-parallel world size {sp_world_size}."
                )
            rotary = torch.cat(
                [
                    sequence_parallel_chunk(rotary[:, :n_video], dim=1),
                    rotary[:, n_video:],
                ],
                dim=1,
            )
            x = sequence_parallel_chunk(x, dim=1)
            num_video_tokens = n_video // sp_world_size
        ...
        temb6 = self.time_modulation(temb_input.reshape(B * joint_seq_len, -1))  # (B*S, 6D)

        sync_text = sp_world_size > 1 and os.environ.get("LINGBOT_SP_SYNC_TEXT", "1") == "1"
        for block in self.blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                joint = torch.utils.checkpoint.checkpoint(
                    block,
                    joint,
                    temb6,
                    rotary,
            ...
            if sync_text:
                # The text tail is recomputed redundantly on every rank, so its
                # rounding can drift apart; re-broadcasting keeps every rank's text
                # keys identical for the next block.
                text_tail = joint[:, num_video_tokens:].contiguous()
                self.sp_group.broadcast(text_tail, src=self.sp_group.first_rank)
                joint = torch.cat([joint[:, :num_video_tokens], text_tail], dim=1)

        if sp_world_size > 1:
            # Re-assemble the full video sequence; the text tail is already identical
            # on every rank. temb is uniform across tokens, so it is simply re-expanded.
            joint = torch.cat(
                [
                    sequence_parallel_all_gather(
                        joint[:, :num_video_tokens].contiguous(), dim=1
                    ),
                    joint[:, num_video_tokens:],
                ],
                dim=1,
            )
            temb_input = t_emb.unsqueeze(1).expand(B, joint.shape[1], -1)

"""Training script for Wan Latent Upsampler.

Trains the WanLatentUpsamplerModel to spatially upsample Wan VAE latents.
    ...
    try:
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=weight_dtype):
            ...
            for i, video_path in enumerate(args.validation_paths):
                ...
                # Encode high-res
                gt_latents = vae.encode(pixel_values).latent_dist.mode()
                ...
                low_h = int(h_target / scale / spatial_ratio) * spatial_ratio
                low_w = int(w_target / scale / spatial_ratio) * spatial_ratio
                ...
                input_latents = vae.encode(pixel_values_low).latent_dist.mode()
                ...
                # Also decode low-res for comparison
...
def parse_args():
    parser = argparse.ArgumentParser(description="Training script for Wan Latent Upsampler.")
    parser.add_argument(
        "--config_path",
        type=str,
        default="config/wan2.2/wan_civitai_t2v_2.2vae.yaml",
        help="Path to the model config yaml file (must contain latent_upsampler_kwargs).",
    )
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to pretrained model directory (contains vae weights).",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_dir_wan_upsampler",
    ...
    parser.add_argument(
        "--enable_degradation", action="store_true",
        help="Apply random degradation to HR frames before spatial downsample, "
             "so the upsampler learns restoration + super-resolution.",
    )
    parser.add_argument(
        "--degradation_ops_range", type=int, nargs=2, default=[1, 4],
        metavar=("MIN_OPS", "MAX_OPS"),
        help="Range of degradation operations count per frame.",
    )
    # ==================== Noisy Training Args ====================
    parser.add_argument(
        "--enable_noisy_training", action="store_true",
        help="Add flow-matching noise to latents during training, "
             "so the upsampler learns to work at arbitrary denoising steps.",
    )
    parser.add_argument(
        "--noise_sigma_max", type=float, default=0.25,
        help="Maximum sigma (noise level) for noisy training (only used when "
             "--noise_sample_mode=uniform). Ignored in 'scheduler' mode.",
    )
    parser.add_argument(
        "--noise_sigma_min", type=float, default=0.0,
        help="Minimum sigma (noise level) for noisy training (only used when "
             "--noise_sample_mode=uniform). Ignored in 'scheduler' mode.",
    )
    parser.add_argument(
        "--noise_sample_mode", type=str, default="scheduler",
        choices=["uniform", "scheduler"],
        help="How to sample sigma for noisy training. "
             "'uniform': sample sigma uniformly in [sigma_min, sigma_max]. "
             "'scheduler': sample a random timestep from the scheduler's actual schedule "
             "(respects shift), so training distribution matches inference.",
    )
    parser.add_argument(
        "--noise_num_inference_steps", type=int, default=50,
        help="Number of inference steps to simulate when sampling sigma from scheduler. "
             "Only used when --noise_sample_mode=scheduler.",
    )
    parser.add_argument(
        "--noise_step_range", type=int, nargs=2, default=None,
        metavar=("MIN_STEP", "MAX_STEP"),
        help="Range of step indices [min, max) to sample from when using scheduler mode. "
             "E.g. '--noise_step_range 30 50' to only sample sigmas from the later half. "
             "If not specified, samples from all steps (full range).",
    )
    # ==================== Pixel Loss Args ====================
    parser.add_argument(
        "--enable_pixel_loss", action="store_true",
        help="Enable pixel-space MSE loss by decoding predicted latents through frozen VAE decoder.",
    )
    parser.add_argument(
        "--pixel_loss_weight", type=float, default=1.0,
        help="Weight for pixel-space MSE loss relative to latent MSE loss.",
    )
...
def main():
    ...
    # ==================== Load Noise Scheduler (for noisy training) ====================
    noise_scheduler = None
    noise_sigmas_pool = None  # Pre-computed sigma values for scheduler mode
    if args.enable_noisy_training:
        scheduler_kwargs_raw = OmegaConf.to_container(config.get('scheduler_kwargs', {}), resolve=True)
        noise_scheduler = FlowMatchEulerDiscreteScheduler(
            **filter_kwargs(FlowMatchEulerDiscreteScheduler, scheduler_kwargs_raw)
        )

        if args.noise_sample_mode == "scheduler":
            # Pre-compute the sigma schedule so training samples from realistic values
            noise_scheduler.set_timesteps(args.noise_num_inference_steps)
            all_timesteps = noise_scheduler.timesteps  # descending order (high noise -> low noise)
            all_sigmas = all_timesteps.float() / noise_scheduler.config.num_train_timesteps
            # Optionally restrict to a sub-range of steps
            if args.noise_step_range is not None:
                step_min, step_max = args.noise_step_range
                all_sigmas = all_sigmas[step_min:step_max]
            noise_sigmas_pool = all_sigmas.cpu()  # [num_valid_steps]
            logger.info(
                f"Noisy training enabled (scheduler mode). "
                f"num_inference_steps={args.noise_num_inference_steps}, "
                f"step_range={args.noise_step_range}, "
                f"sigma_range=[{noise_sigmas_pool.min().item():.4f}, {noise_sigmas_pool.max().item():.4f}], "
                f"num_sigma_candidates={len(noise_sigmas_pool)}."
            )
        else:
            logger.info(
                f"Noisy training enabled (uniform mode). "
                f"sigma_range=[{args.noise_sigma_min}, {args.noise_sigma_max}]."
            )
    ...
    lu_cfg_raw = config.get('latent_upsampler_kwargs', {}) or {}
    lu_kwargs = OmegaConf.to_container(lu_cfg_raw, resolve=True) if hasattr(lu_cfg_raw, 'keys') and not isinstance(lu_cfg_raw, dict) else dict(lu_cfg_raw)
    ...
    if args.latent_upsampler_path is not None:
        ...
        state_dict = state_dict["state_dict"] if "state_dict" in state_dict else state_dict
        # Filter out keys whose shape mismatches the current model (e.g. when
        # rational_spatial_scale changes, `upsampler.conv.*` channel count differs).
        # Such keys will be re-initialized while all other weights are reused.
        model_sd = latent_upsampler.state_dict()
        filtered_state_dict = {
        ...
        if skipped_shape_mismatch:
            print(f"Upsampler shape-mismatch skipped keys ({len(skipped_shape_mismatch)}): {skipped_shape_mismatch}")
        m, u = latent_upsampler.load_state_dict(filtered_state_dict, strict=False)
        print(f"Upsampler reused: {len(filtered_state_dict)}/{len(state_dict)}, missing keys: {len(m)}, unexpected keys: {len(u)}")
            ...
            def load_model_hook(models, input_dir):
                if args.use_ema:
                    ...
                    if os.path.exists(ema_path):
                        load_model = WanLatentUpsamplerModel.from_pretrained(input_dir, subfolder="latent_upsampler_ema")
                        load_ema = EMAModel(load_model.parameters(), model_cls=WanLatentUpsamplerModel, model_config=load_model.config)
                ...
                for i in range(len(models)):
                    model = models.pop()
                    load_model = WanLatentUpsamplerModel.from_pretrained(input_dir, subfolder="latent_upsampler")
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            spatial_ratio = vae.config.spatial_compression_ratio
            # Align to lcm(spatial_ratio, spatial_ratio * scale) so that both high_res
            # and low_res (= high_res / scale) are divisible by spatial_ratio for VAE encoding.
            align_to = math.lcm(int(spatial_ratio), int(spatial_ratio * spatial_scale))
            closest_size = [int(x / align_to) * align_to for x in closest_size]
            ...
            for example in examples:
                ...
                # --- Optional degradation before spatial downsample ---
                # Follow ImageVideoTileDataset pattern: one n_ops per clip for
                # temporal consistency, applied on the normalized tensor.
                if args.enable_degradation:
                    n_ops = random.randint(*args.degradation_ops_range)
                    # Denormalize [-1,1] -> uint8 [0,255] for degradation ops
                    frames_uint8 = ((pixel_values_hr * 0.5 + 0.5) * 255).clamp(0, 255)
                    # Convert to list of [H, W, C] numpy uint8 frames
                    frame_list = [frames_uint8[i].permute(1, 2, 0).byte().numpy()
                                  for i in range(frames_uint8.shape[0])]
                    # Video-level degradation: same ops + params for all frames
                    degraded_stack = random_degradation_video(frame_list, n_operations=n_ops)
                    pixel_values_hr = (
                        torch.from_numpy(degraded_stack).permute(0, 3, 1, 2).float() / 255.0
                    )
                    pixel_values_hr = (pixel_values_hr - 0.5) / 0.5  # re-normalize to [-1, 1]

                # Low-res: spatially downsample (degraded) HR frames
    ...
    vae.requires_grad_(False)  # Freeze VAE params but allow gradient flow for pixel loss
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #         save_videos_grid(pixel_value[None, ...], f"{args.output_dir}/sanity_check/rank_{accelerator.process_index}_sample_{idx}_hr.mp4", rescale=True)

            #     pixel_values_low_check = batch['pixel_values_low'].cpu()
            #     pixel_values_low_check = rearrange(pixel_values_low_check, "b f c h w -> b c f h w")
            #     for idx, pixel_value in enumerate(pixel_values_low_check):
            #         save_videos_grid(pixel_value[None, ...], f"{args.output_dir}/sanity_check/rank_{accelerator.process_index}_sample_{idx}_low.mp4", rescale=True)
            ...
            with accelerator.accumulate(latent_upsampler):
                ...
                with torch.no_grad():
                    # 1. VAE encode high-res -> GT latents
                ...
                if args.low_vram and not args.enable_pixel_loss:
                    vae.to('cpu')
                    torch.cuda.empty_cache()

                # 2.5 Add flow-matching noise to simulate intermediate denoising latents
                if noise_scheduler is not None:
                    if noise_sigmas_pool is not None:
                        # Scheduler mode: sample sigma from the pre-computed schedule
                        idx = torch.randint(0, len(noise_sigmas_pool), (bsz,))
                        sigma = noise_sigmas_pool[idx].to(device=gt_latents.device, dtype=gt_latents.dtype)
                    else:
                        # Uniform mode: sample sigma uniformly in [sigma_min, sigma_max]
                        sigma = torch.rand(
                            (bsz,), device=gt_latents.device, dtype=gt_latents.dtype
                        ) * (args.noise_sigma_max - args.noise_sigma_min) + args.noise_sigma_min

                    # Expand sigma for broadcasting: [B, 1, 1, 1, 1]
                    sigma_expand = sigma
                    while sigma_expand.ndim < gt_latents.ndim:
                        sigma_expand = sigma_expand.unsqueeze(-1)

                    # Generate independent noise for HR and LR
                    noise_gt = torch.randn_like(gt_latents)
                    noise_lr = torch.randn_like(input_latents)

                    # Flow matching: x_t = (1 - sigma) * x_0 + sigma * noise
                    gt_latents_noisy = (1.0 - sigma_expand) * gt_latents + sigma_expand * noise_gt
                    input_latents_noisy = (1.0 - sigma_expand) * input_latents + sigma_expand * noise_lr

                    # Target: upsampler should predict the noisy GT (same noise level)
                    gt_latents = gt_latents_noisy
                    input_latents = input_latents_noisy
                ...
                # 4. Compute loss
                if args.enable_pixel_loss:
                    # Pixel-space MSE: decode predicted latents through frozen VAE decoder
                    # Gradient flows through frozen decoder back to upsampler
                    decoded_pred_list = []
                    for i in range(0, bsz, bs):
                        pred_bs = predicted_latents[i:i + bs].to(weight_dtype)
                        decoded_pred_list.append(vae.decode(pred_bs, return_dict=False)[0])
                    decoded_pred = torch.cat(decoded_pred_list, dim=0)

                    loss = F.mse_loss(decoded_pred.float(), pixel_values.float())

                    if args.low_vram:
                        vae.to('cpu')
                        torch.cuda.empty_cache()
                else:
                    # Latent-space MSE (original behavior)

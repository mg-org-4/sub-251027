...
def main():
    ...
    if args.use_ema:
        ...
        ema_module = VaceWanTransformer3DModel.from_pretrained(
            os.path.join(args.pretrained_model_name_or_path, sub_path),
            transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs']),
            low_cpu_mem_usage=True,
        )
        if args.use_fsdp:
            ...
            ema_transformer3d = EMAModel(ema_module.parameters(), model_cls=VaceWanTransformer3DModel, model_config=ema_module.config)

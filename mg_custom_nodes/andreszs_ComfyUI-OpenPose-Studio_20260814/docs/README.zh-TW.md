﻿<h4 align="center">
  <a href="./README.md">English</a> | <a href="./README.de.md">Deutsch</a> | <a href="./README.es.md">Español</a> | <a href="./README.fr.md">Français</a> | <a href="./README.hi.md">हिन्दी</a> | <a href="./README.pt.md">Português</a> | <a href="./README.ru.md">Русский</a> | <a href="./README.ja.md">日本語</a> | <a href="./README.ko.md">한국어</a> | <a href="./README.zh.md">中文</a> | 繁體中文
</h4>
...
匯入的 OpenPose 手部可以作為一個群組進行變換，也可以逐一調整關鍵點。在 Canvas 上選取一隻手即可顯示其變換框，並使用周圍的控制柄調整大小、旋轉、鏡像或開啟聚焦手部編輯器。

![顯示變換與編輯控制項的已選手部](../locales/zh-TW/openpose-studio-hand-tools-2.png)

你也可以直接點擊側邊欄中 **Left hand** 或 **Right hand** 旁的鉛筆圖示開啟聚焦編輯器。在此檢視中，拖曳關鍵點 1–20 可調整手指；關鍵點 0 會保持鎖定，作為手部錨點。將滑鼠游標移到側邊欄項目上時，對應的點會醒目顯示。使用確認按鈕可將整個手部編輯工作階段套用為一個可復原的變更；使用關閉按鈕或 **Escape** 可捨棄變更。

![聚焦手部關鍵點編輯器](../locales/zh-TW/openpose-studio-hand-editor-2.png)
...
此編輯器完整支援 **OpenPose COCO-18 (body)** 編輯，以及個別編輯 **OpenPose 手部關鍵點**。Face 關鍵點會被保留並渲染，但仍作為 *pass-through* 資料處理，目前無法個別編輯。
...
**Render** 模組可自訂 workflow 執行時 OpenPose stickman 的渲染方式。它提供 body、hands 與 face 的樣式控制，例如線寬、keypoint 半徑，以及 hands/face 的 keypoint 顏色。

Render 設定會本機儲存在此瀏覽器的 local storage 中，而不是儲存在 workflow 中。變更這些設定會影響後續 workflow 執行。
...
1. **Face 編輯**
  - Face 關鍵點會被保留並渲染，但目前無法在 Canvas 上個別編輯。

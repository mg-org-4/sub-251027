<h4 align="center">
  <a href="./README.md">English</a> | <a href="./README.de.md">Deutsch</a> | <a href="./README.es.md">Español</a> | <a href="./README.fr.md">Français</a> | <a href="./README.hi.md">हिन्दी</a> | <a href="./README.pt.md">Português</a> | Русский | <a href="./README.ja.md">日本語</a> | <a href="./README.ko.md">한국어</a> | <a href="./README.zh.md">中文</a> | <a href="./README.zh-TW.md">繁體中文</a>
</h4>
...
- 📱 [Адаптивный мобильный интерфейс](#адаптивный-мобильный-интерфейс)
- 📦 [Установка](#установка)
- 🎯 [Использование](#использование)
- 🖐️ [Редактирование рук](#редактирование-рук)
- 🔧 [Nodes](#nodes)
...
- Индивидуальное редактирование keypoints рук в отдельном увеличенном редакторе
...
OpenPose Studio полностью адаптирован для мобильных браузеров и сенсорного управления. Editor сохраняет удобство работы с canvas на узких экранах, выводит инструменты Preset и COCO Keypoints в отдельных представлениях и предлагает компактную Gallery с несколькими вариантами плотности миниатюр.

<p align="center">
  <img src="../assets/openpose-studio-mobile-responsive.png" alt="Адаптивный мобильный интерфейс OpenPose Studio с представлениями Canvas, Preset, COCO Keypoints и Gallery" width="100%" />
</p>

<p align="center"><sub>Редактирование canvas, элементы управления Preset, управление отсутствующими keypoints и режим Small Icons в Gallery на Android.</sub></p>
...
### Вариант 1: нативный Extension Manager (рекомендуется)

1. Откройте нативный **Extension Manager** в ComfyUI, затем **Nodes Manager**.
2. Найдите `openpose-studio` и выберите **OpenPose Studio**.
3. Нажмите **Install** и перезапустите ComfyUI после завершения установки.

<p align="center">
  <img src="../assets/openpose-studio-extension-manager-install.png" alt="Установка OpenPose Studio через нативный Extension Manager в ComfyUI" width="100%" />
</p>

### Вариант 2: ручная установка

Откройте терминал в `ComfyUI/custom_nodes/` и клонируйте репозиторий:

```bash
git clone https://github.com/andreszs/comfyui-openpose-studio.git
```

После клонирования репозитория перезапустите ComfyUI.

### Проверка установки

Убедитесь, что **OpenPose Studio** появился в меню нод в разделе `image > OpenPose Studio`.
...
Импортированные руки OpenPose можно трансформировать целиком или настраивать по одному keypoint. Выберите руку на canvas, чтобы отобразить рамку трансформации, и используйте расположенные вокруг элементы управления для изменения размера, поворота, отражения или открытия отдельного редактора руки.

![Выбранная рука с элементами управления трансформацией и редактированием](../locales/ru/openpose-studio-hand-tools-2.png)

Отдельный редактор также можно открыть с помощью значка карандаша рядом с **Left hand** или **Right hand** на боковой панели. В этом режиме перетаскивайте keypoints 1–20 для настройки пальцев; keypoint 0 остаётся заблокированным якорем руки. При наведении на строку боковой панели подсвечивается соответствующая точка. Кнопка подтверждения применяет всю сессию как одно отменяемое изменение, а кнопка закрытия или **Escape** отменяет её.

![Отдельный редактор keypoints руки](../locales/ru/openpose-studio-hand-editor-2.png)
...
Этот редактор полностью поддерживает редактирование **OpenPose COCO-18 (body)** и индивидуальное редактирование **keypoints рук OpenPose**. Keypoints лица сохраняются и рендерятся, но остаются данными *pass-through* и пока не могут редактироваться по отдельности.
...
Gallery поддерживает четыре режима отображения:
- **Large** — крупные миниатюры для быстрого визуального выбора
- **Medium** — сбалансированный размер и плотность миниатюр
- **Small** — плотная сетка значков, оптимизированная для узких и мобильных интерфейсов
...
> [!NOTE]
> Nodes 2.0 поддерживается. Если отсутствует canvas предпросмотра или кнопка редактора, проверьте кеш браузера, загрузку frontend и логи установки.

### Текущие ограничения и обходные пути

1. **Редактирование лица**
  - Keypoints лица сохраняются и рендерятся, но пока не могут редактироваться по отдельности на canvas.
...
3. **Совместимость с Nodes 2.0**
  - Статус: Поддерживается в текущих версиях.
  - Примечание: Если UI редактора не появляется, наиболее вероятные причины — устаревший кеш браузера, ошибка загрузки frontend-модулей или неполная установка.
  - Диагностика: Полностью перезапустите ComfyUI, выполните жёсткое обновление браузера и проверьте консоль браузера вместе с логом запуска ComfyUI.

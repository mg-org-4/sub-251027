...
class Coordinate_SplitIndex:
    ...
    def get_coordinates(self, coordinates, index, normalized, image=None):
        try:
            points = json.loads(coordinates)
            ...
            if not normalized and image is not None:
                if len(image.shape) != 4:
                    raise ValueError(f"Invalid image shape: {image.shape}, expected [batch, H, W, C]")

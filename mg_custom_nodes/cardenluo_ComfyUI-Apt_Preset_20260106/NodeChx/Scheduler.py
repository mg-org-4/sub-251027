...
class XXXscheduler_sigmas2Graph:
    def __init__(self):
        pass
...
class scheduler_sigmas2Graph:
    ...
    def simple_output(self, sigmas, color, print_as_list, scale):
        ...
        if sigmas is None or len(sigmas) == 0:
            ...
            # 调试信息：打印归一化前后的值
            print(f"[scheduler_sigmas2Graph] Processing {len(sigmas_cpu)} points")
            print(f"[scheduler_sigmas2Graph] Original range: {sigmas_cpu.min():.6f} - {sigmas_cpu.max():.6f}")
            print(f"[scheduler_sigmas2Graph] Original values (first 5): {sigmas_cpu.tolist()[:5]}")
            print(f"[scheduler_sigmas2Graph] Original values (last 5): {sigmas_cpu.tolist()[-5:]}")
            print(f"[scheduler_sigmas2Graph] Normalized values (first 5): {normalized_sigmas.tolist()[:5]}")
            print(f"[scheduler_sigmas2Graph] Normalized values (last 5): {normalized_sigmas.tolist()[-5:]}")

...
class pre_Unit_PromptWeight:
    ...
    def process(self, clip, prompt, main_prompt_ratio=0.5):
        ...
        fused_individual, pooled_individual, norm_individual = self.fuse_conditions_equal(cond_features, weights)
        ...
        w_individual = 1.0 - main_prompt_ratio
        
        fused_feat = feat_whole.unsqueeze(0) * w_overall + fused_individual * w_individual
        fused_pooled = p_whole * w_overall + pooled_individual * w_individual

        final_parts = []
        final_parts.append(f"main_prompt({whole_content}){int(round(w_overall * 100))}%")
        for seg, nw in zip(segments, norm_individual):
            pct = int(round(nw * w_individual * 100))

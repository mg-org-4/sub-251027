...
class type_AnyIndex:
    ...
    def getIndex(self, any, index, prompt=None, my_unique_id=None):
        ...
        if output_is_list or len(any) > 1:
            safe_index = min(index, len(any) - 1) if len(any) > 0 else 0
            return (any[safe_index],)
        elif isinstance(any[0], torch.Tensor):
            batch_max_index = any[0].shape[0] - 1 if any[0].shape[0] > 0 else 0
            batch_index = min(batch_max_index, index)
            s = any[0][batch_index:batch_index + 1].clone()
            return (s,)
        else:
            safe_index = min(index, len(any[0]) - 1) if len(any[0]) > 0 else 0

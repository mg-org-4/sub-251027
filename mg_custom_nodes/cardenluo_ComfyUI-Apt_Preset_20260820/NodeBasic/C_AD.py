...
from ..NodeChx.main_nodes import basic_Ksampler_simple, chx_Ksampler_refine, AD_CreateVideo
...
class AD_MiniMax_guide(AD_MiniMax_Ref2V):
    ...
    def INPUT_TYPES(cls):
        ...
        media_input = ("IMAGE,VIDEO,AUDIO,LATENT",)
    ...
    def execute(self, clip, vae, audio_vae, prompt, width, height, length, ref_image_size="match", _context_latent=None, **kwargs):
...
class _AD_MinMaxBase:
    ...
    def _sample_video(self, context, model, positive, latent, seed, fps,
                      has_context_latent, text, enable_refine=False,
                      refine_model="None", refine_denoise=0.3, exact_audio=None):
        guide_context = new_context(
            context, model=model, positive=positive, latent=latent
        )
        sample_result = basic_Ksampler_simple().run(
            guide_context,
            seed,
            1.0,
            image_output="Hide",
        )
        sampled_context, _images = sample_result["result"]
        if enable_refine:
            if refine_model != "None" and refine_model not in folder_paths.get_filename_list("upscale_models"):
                raise ValueError(f"AD_MinMax_Ref2_mul: invalid refine_model: {refine_model}")
            refine_result = chx_Ksampler_refine().run(
                sampled_context,
                seed,
                refine_denoise,
                refine_model,
            ...
            sampled_context, _images = refine_result["result"]
        ...
        output_context.update({
            "positive": sampled_context["positive"],
            "latent": sampled_context["latent"],
            "images": sampled_context["images"],
            "pos": text,
        })
        return output_context, video, text


class AD_MinMax_Ref2(_AD_MinMaxBase, AD_MiniMax_guide):
    """Ref2VA generation with context-latent chaining."""
    ...
    def execute(self, context, prompt, width, height, length, seed, ref_image_size="match", model=None, fps=24.0, **kwargs):
        clip = context.get("clip")
        vae = context.get("vae")
        audio_vae = context.get("audio_vae")
        missing = [name for name, value in (("clip", clip), ("vae", vae), ("audio_vae", audio_vae)) if value is None]
        if missing:
            raise ValueError(f"AD_MinMax_Ref2 context is missing: {', '.join(missing)}")
...
class AD_MinMax_Ref2_mul(_AD_MinMaxBase, AD_MiniMax_guide):
    ...
    @classmethod
    def INPUT_TYPES(cls):
        inherited = AD_MinMax_Ref2.INPUT_TYPES()
        required = dict(inherited["required"])
        ...
        required["enable_refine"] = (
            "BOOLEAN",
            {"default": False, "tooltip": "Run a second sampling pass before saving this stage."},
        )
    ...
    def execute(self, context, prompt, width, height, length, seed, stage_prompts,
                single_long_video_split, single_long_audio_split,
                enable_refine, refine_model, refine_denoise,
                ref_image_size="match", model=None, fps=24.0, stage_info=None, stage_data=None,
        ...
        if stage_data is not None:
            selected_kwargs["media"] = stage_data
        global_to_local = {}
        for local_index, global_index in enumerate(references, start=1):
            value = kwargs.get(f"media_{global_index}")
            if value is None:
                raise ValueError(f"AD_MinMax_Ref2_mul: material {global_index} is not connected")
            media_type = _ad_guide_media_type(value)
            if not media_type:
                raise ValueError(f"AD_MinMax_Ref2_mul: material {global_index} has an unsupported type")
            selected_kwargs[f"media_{local_index}"] = value
            selected_kwargs[f"media_type_{local_index}"] = media_type
        ...
        output_context, video, text = self._sample_video(
            context, active_model, positive, latent, seed, fps, has_context_latent, text,
            enable_refine, refine_model, refine_denoise, exact_audio,
        )

        merged_video = ExecutionBlocker(None)
        if run_id is not None:
            _ad_stage_save_video(video, run_id, stage_index)
            if stage_index == total - 1 and _ad_output_is_connected(workflow_prompt, unique_id, 2):
                output_dir = _ad_stage_output_dir(run_id)
                ...
                if missing_paths:
                    raise FileNotFoundError(f"AD_MinMax_Ref2_mul: missing segment {os.path.basename(missing_paths[0])}")
        ...
        return output_context, video, merged_video, text


class AD_MinMax_FL2(_AD_MinMaxBase):
    """FL2VA generation: ordered Picture 1/2 are first/last frames."""
    ...
    def _collect_media(kwargs):
        ...
        if unsupported:
            raise ValueError("AD_MinMax_FL2 Media only accepts image, latent or text")
        images = [item for item in items if item[1] == "image"]
        latents = [item for item in items if item[1] == "latent"]
        if len(images) > 2:
            raise ValueError("AD_MinMax_FL2 accepts at most two ordered images")
        if len(latents) > 1:
            raise ValueError("AD_MinMax_FL2 accepts only one context latent")
        return items

    def execute(self, context, prompt, width, height, length, seed,
                single_image_position="auto", model=None, fps=24.0, **kwargs):
        if _h3_empty_av_latent is None or _h3_resize is None or _node_helpers is None:
            raise RuntimeError("This ComfyUI build does not provide MiniMax H3 support")
        clip = context.get("clip")
        vae = context.get("vae")
        ...
        if missing:
            raise ValueError(f"AD_MinMax_FL2 context is missing: {', '.join(missing)}")
        ...
        return self._sample_video(
            context, model, positive, latent, seed, fps,
            has_context_latent, str(prompt or ""),
        )

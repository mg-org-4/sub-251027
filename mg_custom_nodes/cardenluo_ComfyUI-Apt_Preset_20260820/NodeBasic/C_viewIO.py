...
def _io_latent_streams(latent):
    ...
    if hasattr(samples, "unbind"):
        ...
        raise ValueError(f"IO_SaveLatent: expected an H3 video/audio latent, got {type(samples)!r}")
    if len(streams) < 2:
        raise ValueError("IO_SaveLatent: latent has no audio stream")
    return streams
...
class IO_loadLatent:
    ...
    FUNCTION = "load"
    CATEGORY = "Apt_Preset/IO_Port"
    DESCRIPTION = "Load an H3 video/audio latent saved by IO_SaveLatent."
    ...
    def load(self, latent_path, clip_index=0):
        ...
        if "video" not in data or "audio" not in data:
            raise ValueError(f"IO_loadLatent: {path} is not an H3 video/audio latent")
...
class IO_SaveLatent:
    ...
    OUTPUT_NODE = True
    CATEGORY = "Apt_Preset/IO_Port"
    DESCRIPTION = "Save an H3 video/audio latent for IO_loadLatent."

    def save(self, latent, filename_prefix, clip_index=0):
        video, audio = _io_latent_streams(latent)[:2]

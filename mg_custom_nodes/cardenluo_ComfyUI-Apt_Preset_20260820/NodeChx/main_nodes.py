...
class sum_load_MiniMaxH3:
    ...
    RETURN_TYPES = ("RUN_CONTEXT", "MODEL", "CLIP", "VAE", "VAE")
    RETURN_NAMES = ("context", "model", "clip", "vae", "audio_vae")
    FUNCTION = "process_settings"
    CATEGORY = "Apt_Preset/chx_load"
    NAME = "sum_load_adv"

    @classmethod
    def _compute_cache_key(cls, unet_name, unet_Weight_Dtype, clip_type, clip1):
        key_data = f"{unet_name}|{unet_Weight_Dtype}|{clip_type}|{clip1}"
        return hashlib.md5(key_data.encode()).hexdigest()
    ...
    def _load_model_cached(cls, unet_name, unet_Weight_Dtype, clip_type, clip1):
        ...
        if clip1 != "None" and clip_type != "None":
            if clip1.endswith(".gguf"):
                if not GGUF_AVAILABLE:
                    raise RuntimeError("GGUF not installed. Please install gguf and protobuf using: pip install gguf protobuf")
                from .load_GGUF.nodes import CLIPLoaderGGUF2
    ...
    def process_settings(self, steps, cfg, sampler, scheduler,
                         ...
                         unet_name=None, clip1=None):

        model, clip = self._load_model_cached(unet_name, unet_Weight_Dtype, clip_type, clip1)
        ...
        return (context, model, clip, vae_obj, audio_vae_obj)
...
class Stack_Ksampler_highAndLow:
    ...
    def encode(self, add_noise, noise_seed, start_step, mid_step, steps, return_with_leftover_noise, model2=None,
               positive2=None, sampler=None, scheduler=None):
        data = (add_noise, noise_seed, start_step, mid_step, steps, return_with_leftover_noise, model2, positive2, sampler, scheduler)
        return (data,)
...
#-------------------ksample_type------------------------------------------------------------------
        ...
        if ksample_type is not None:
            ...
            if len(funtion) == 10:
                (add_noise, noise_seed, start_step, mid_step, steps, return_with_leftover_noise, model2, positive2, sampler2, scheduler2)=funtion 
                disable_noise = False 
                if add_noise == "disable":
                    disable_noise = True 
                force_full_denoise = True
                ...
                latent = common_ksampler(model2, noise_seed, steps, cfg, sampler2, scheduler2, positive2, negative, latent1, denoise=denoise, 
...
#region-----------AD_CreateVideo 合并 CreateVideo + VAEDecodeAudio------------------#
...
class AD_CreateVideo(comfy_io.ComfyNode):
    ...
    def define_schema(cls):
        return comfy_io.Schema(
            node_id="AD_CreateVideo",
            search_aliases=["images to video", "latent to video"],

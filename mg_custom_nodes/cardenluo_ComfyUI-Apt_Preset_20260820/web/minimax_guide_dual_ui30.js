...
const AD_GUIDE_UI_VERSION = "2026.08.19-guide-v52-ui30";
globalThis.__AD_MINIMAX_GUIDE_UI30_MODULE__ = true;
console.info(`[ADMiniMaxGuide] frontend ${AD_GUIDE_UI_VERSION} loaded`);
const NODE_CLASS = "AD_MiniMax_guide";
const REF2_NODE_CLASS = "AD_MinMax_Ref2";
const REF2_MUL_NODE_CLASS = "AD_MinMax_Ref2_mul";
const FL2_NODE_CLASS = "AD_MinMax_FL2";
...
const TEXT = {
    ...
    fl2MaterialEmpty: ZH_BROWSER ? "\u4ece\u7d20\u6750\u8282\u70b9\u7aef\u53e3\u8fde\u5165\u6700\u591a\u4e24\u5f20\u56fe\u7247" : "Connect up to two images from node outputs",
...
const SIX_FRAMEWORK_SNIPPET = `subject_definitions:
...
const GUIDE_TEMPLATE_SNIPPET = SIX_FRAMEWORK_SNIPPET;
const PROMPT_SNIPPETS = [
    { label: "\u4e3b\u4f53", text: "<Subject X>" },
    { label: "\u955c\u5934", text: "[Shot X] At 00:00.000" },
    { label: "\u6846\u67b6", text: SIX_FRAMEWORK_SNIPPET },
    { label: "\u6a21\u677f", text: GUIDE_TEMPLATE_SNIPPET },
    { label: "\u53e3\u97f3", text: "(S1)" },
    { label: "\u5408\u97f3", text: "(S1,S2)" },
    { label: "\u4e2d\u6587", text: "<d>[Chinese] \u5199\u5165\u53f0\u8bcd</d>" },
    { label: "\u82f1\u6587", text: "<d>[English] write dialogue</d>" },
...
function isTarget(node) {
    return [NODE_CLASS, REF2_NODE_CLASS, REF2_MUL_NODE_CLASS, FL2_NODE_CLASS].includes(String(node?.comfyClass || node?.type || node?.constructor?.nodeData?.name || ""));
}

function isChxTarget(node) {
    return [REF2_NODE_CLASS, REF2_MUL_NODE_CLASS, FL2_NODE_CLASS].includes(String(node?.comfyClass || node?.type || node?.constructor?.nodeData?.name || ""));
}
...
function isFl2Target(node) {
    return String(node?.comfyClass || node?.type || node?.constructor?.nodeData?.name || "") === FL2_NODE_CLASS;
}
...
function localizeNodeDefinition(nodeData) {
    if (!nodeData || ![NODE_CLASS, REF2_NODE_CLASS, REF2_MUL_NODE_CLASS, FL2_NODE_CLASS, LOADER_CLASS, OUTPUT_CLASS].includes(nodeData.name)) return;
    nodeData.display_name = nodeData.name === LOADER_CLASS
        ? TEXT.loaderTitle
        : nodeData.name === OUTPUT_CLASS
            ? TEXT.outputTitle
            : [REF2_NODE_CLASS, REF2_MUL_NODE_CLASS, FL2_NODE_CLASS].includes(nodeData.name)
                ? nodeData.name
                : TEXT.mainTitle;
}
...
function mediaLimits(node) {
    if (isFl2Target(node)) {
        return { image: 2, video: 0, audio: 0, latent: 1, text: 1, total: 2 };
    }
    if (isMulTarget(node)) {
...
function buildRuntimePrompt(node, runtimeLinks, promptDoc = null, stageIndex = -1) {
    ...
    return doc.parts.map((part) => {
        ...
        if (carryLink && partSourceId === Number(carryLink.source_id)
            ...
            return stageIndex === 0 ? "<Picture 1>" : `${UNRESOLVED_REF_PREFIX}image__`;
        }
        ...
        if (index >= 0) {
            if (isFl2Target(node)) {
                let imageOrdinal = 0;
                for (let runtimeIndex = 0; runtimeIndex <= index; runtimeIndex += 1) {
                    if (String(runtimeLinks[runtimeIndex]?.media_type || "image").toLowerCase() === "image") imageOrdinal += 1;
                }
                return imageOrdinal > 0 ? `<Picture ${imageOrdinal}>` : String(part.token || "");
            }
...
function stagePromptMapping(node) {
    ...
    const labels = { image: ZH_BROWSER ? "图" : "Image", video: ZH_BROWSER ? "视频" : "Video", audio: ZH_BROWSER ? "音频" : "Audio" };
    const seen = new Set();
    const values = [];
...
function syncRefineWidgets(node, enabled = Boolean(getWidgetValue(node, "enable_refine", false))) {
    if (!isMulTarget(node)) return false;
    const changed = ["refine_model", "refine_denoise"]
        .map((name) => setConditionalWidgetVisible(node, getWidget(node, name), enabled))
        .some(Boolean);
    if (changed) {
        node._widgetSlotsDirty = true;
        node.setDirtyCanvas?.(true, true);
...
function reorderMulWidgets(node) {
    ...
    const enableRefine = getWidget(node, "enable_refine");
    const fpsIndex = node.widgets.indexOf(fps);
    const enableRefineIndex = node.widgets.indexOf(enableRefine);
    if (fpsIndex < 0 || enableRefineIndex < 0 || fpsIndex < enableRefineIndex) return false;
    node.widgets.splice(fpsIndex, 1);
    node.widgets.splice(node.widgets.indexOf(enableRefine), 0, fps);
    refreshVueNodeWidgets(node);
    node._widgetSlotsDirty = true;
    node.setDirtyCanvas?.(true, true);
    return true;
...
function installRefineWidgetSync(node) {
    if (!isMulTarget(node)) return;
    const toggle = getWidget(node, "enable_refine");
    if (!toggle) return;
    if (!toggle.__adGuideRefineSyncInstalled) {
        toggle.__adGuideRefineSyncInstalled = true;
        const originalCallback = toggle.callback;
        toggle.callback = function onEnableRefineChange(value) {
            const result = originalCallback?.apply(this, arguments);
            syncRefineWidgets(node, Boolean(value));
            return result;
        };
    }
    syncRefineWidgets(node);
}
...
function ensurePromptEditor(node) {
    ...
    const textTagTray = isChxTarget(node) ? null : createTextTagTray(node);
    ...
    if (textTagTray) wrap.append(textTagTray);
    wrap.append(editor);
    ...
    const domWidget = node.addDOMWidget("h3_prompt_mentions", "h3_prompt_mentions", wrap, {
        ...
        margin: 10,
        serialize: false,
        getMinHeight: () => (stagePromptBar ? 50 : 0) + (Number(materialTray.dataset.trayHeight) || 70) + (textTagTray ? Number(textTagTray.dataset.trayHeight) || 34 : 0) + 124,
        afterResize: () => {
            applyNativeEditorTheme(wrap);
            refreshMaterialTray(node);
            resizeTextTagTray(node);
...
function seedControlWidget(node) {
    ...
    if (next && !["stage_prompts", "vram_reserved", "enable_refine", "refine_model", "refine_denoise", "fps"].includes(String(next.name || ""))) return next;
...
function currentMulWidgetValues(node) {
    ...
    return {
        ...
        vram_reserved: String(getWidgetValue(node, "vram_reserved", "None")),
        ...
        enable_refine: Boolean(getWidgetValue(node, "enable_refine", false)),
...
function repairMulConfiguredWidgetValues(node, info) {
    ...
    if (stored && typeof stored === "object") {
        ...
        values = {
            ...
            seed_control: SEED_CONTROL_MODES.has(String(stored.seed_control || "").toLowerCase())
                ? String(stored.seed_control).toLowerCase() : "randomize",
            stage_prompts: typeof stored.stage_prompts === "string" ? stored.stage_prompts : "[]",
            vram_reserved: ["None", "auto", "0.6GB", "1GB", "1.5GB", "2.0GB"].includes(stored.vram_reserved)
                ? stored.vram_reserved : "None",
            single_long_video_split: Boolean(stored.single_long_video_split),
            single_long_audio_split: Boolean(stored.single_long_audio_split),
            enable_refine: Boolean(stored.enable_refine),
            refine_model: typeof stored.refine_model === "string" ? stored.refine_model : "None",
            refine_denoise: Number.isFinite(Number(stored.refine_denoise)) ? Number(stored.refine_denoise) : 0.3,
            fps: Number.isFinite(Number(stored.fps)) ? Number(stored.fps) : 24,
        };
    } else {
        let cursor = typeof raw[0] === "string" ? 1 : 0;
        if (raw[cursor] == null) cursor += 1;
        const directRefImageSize = canonicalOption("ref_image_size", raw[cursor + 3]);
        const shiftedRefImageSize = canonicalOption("ref_image_size", raw[cursor + 4]);
        const valuesShifted = !Object.prototype.hasOwnProperty.call(OPTION_DEFS.ref_image_size, directRefImageSize)
            && Object.prototype.hasOwnProperty.call(OPTION_DEFS.ref_image_size, shiftedRefImageSize);
        const refImageSize = valuesShifted ? shiftedRefImageSize : directRefImageSize;
        const seedControl = String(raw[cursor + 5] || "").toLowerCase();
        const hasSeedControl = SEED_CONTROL_MODES.has(seedControl);
        const stagePromptsIndex = cursor + (hasSeedControl ? 6 : 5);
        const reservedOptions = ["None", "auto", "0.6GB", "1GB", "1.5GB", "2.0GB"];
        const hasVramReserved = reservedOptions.includes(raw[stagePromptsIndex + 1]);
        const oldRefineLayout = typeof raw[stagePromptsIndex + 1] === "boolean";
        const newRefineLayout = !hasVramReserved && typeof raw[stagePromptsIndex + 2] === "boolean";
        const fpsIndex = hasVramReserved ? stagePromptsIndex + 5
            : oldRefineLayout ? stagePromptsIndex + 4 : stagePromptsIndex + 1;
        const enableRefineIndex = hasVramReserved ? stagePromptsIndex + 2
            : oldRefineLayout ? stagePromptsIndex + 1 : stagePromptsIndex + 2;
        const refineModelIndex = enableRefineIndex + 1;
        const refineDenoiseIndex = enableRefineIndex + 2;
        const fps = Number(raw[fpsIndex]);
        values = {
            prompt: configuredDocText ?? (typeof raw[0] === "string" ? raw[0] : ""),
            width: Number.isFinite(Number(raw[cursor + (valuesShifted ? 1 : 0)])) ? Number(raw[cursor + (valuesShifted ? 1 : 0)]) : 1344,
            height: Number.isFinite(Number(raw[cursor + (valuesShifted ? 2 : 1)])) ? Number(raw[cursor + (valuesShifted ? 2 : 1)]) : 768,
            length: Number.isFinite(Number(raw[cursor + (valuesShifted ? 3 : 2)])) ? Number(raw[cursor + (valuesShifted ? 3 : 2)]) : 124,
            ref_image_size: Object.prototype.hasOwnProperty.call(OPTION_DEFS.ref_image_size, refImageSize) ? refImageSize : REF_IMAGE_MATCH,
            seed: Number.isFinite(Number(raw[cursor + (valuesShifted ? 0 : 4)])) ? Number(raw[cursor + (valuesShifted ? 0 : 4)]) : 0,
            seed_control: hasSeedControl ? seedControl : "randomize",
            stage_prompts: typeof raw[stagePromptsIndex] === "string" ? raw[stagePromptsIndex] : "[]",
            vram_reserved: hasVramReserved ? raw[stagePromptsIndex + 1] : "None",
            single_long_video_split: false,
            single_long_audio_split: false,
            enable_refine: hasVramReserved || oldRefineLayout || newRefineLayout ? Boolean(raw[enableRefineIndex]) : false,
            refine_model: (hasVramReserved || oldRefineLayout || newRefineLayout) && typeof raw[refineModelIndex] === "string"
                ? raw[refineModelIndex] : "None",
            refine_denoise: (hasVramReserved || oldRefineLayout || newRefineLayout) && Number.isFinite(Number(raw[refineDenoiseIndex]))
                ? Number(raw[refineDenoiseIndex]) : 0.3,
            fps: Number.isFinite(fps) ? fps : 24,
        };
    }
    for (const name of ["prompt", "width", "height", "length", "ref_image_size", "seed", "stage_prompts", "vram_reserved", "single_long_video_split", "single_long_audio_split", "enable_refine", "refine_model", "refine_denoise", "fps"]) {
...
function installNode(nodeType, nodeData) {
    if (![NODE_CLASS, REF2_NODE_CLASS, REF2_MUL_NODE_CLASS, FL2_NODE_CLASS].includes(nodeData?.name)) return;
    pruneTransportInputs(nodeData);
    if (Object.prototype.hasOwnProperty.call(nodeType.prototype, "__adGuideEasyNodeInstalledUI30")) return;
    nodeType.prototype.__adGuideEasyNodeInstalledUI30 = true;
    const originalCreated = nodeType.prototype.onNodeCreated;
    ...
    nodeType.prototype.onConfigure = function onConfigureH3Easy(info) {
        ...
        if (isMulTarget(this)) {
            normalizeMulOutputs(this);
            repairMulConfiguredWidgetValues(this, info);
            this.properties ||= {};
...
function normalizeFlowStageBeginSlots(node, force = false) {
    ...
    if (node.__adFlowStageSlotsNormalizedUI30 && !force) return;
    ...
    node._widgetSlotsDirty = true;
    node.setDirtyCanvas?.(true, true);
    node.__adFlowStageSlotsNormalizedUI30 = true;
}
...
function installFlowStageBeginNode(nodeType, nodeData) {
    ...
    if (Object.prototype.hasOwnProperty.call(nodeType.prototype, "__adFlowStageBeginInstalledUI30")) return;
    nodeType.prototype.__adFlowStageBeginInstalledUI30 = true;
    const originalCreated = nodeType.prototype.onNodeCreated;
    nodeType.prototype.onNodeCreated = function onNodeCreatedFlowStageBeginUI30() {
        const result = originalCreated?.apply(this, arguments);
        syncFlowStageBeginWidgets(this);
        return result;
    };
    const originalConfigure = nodeType.prototype.onConfigure;
    nodeType.prototype.onConfigure = function onConfigureFlowStageBeginUI30() {
        const result = originalConfigure?.apply(this, arguments);
        syncFlowStageBeginWidgets(this);
        return result;
    };
    const originalDraw = nodeType.prototype.onDrawForeground;
    nodeType.prototype.onDrawForeground = function onDrawForegroundFlowStageBeginUI30() {
        const result = originalDraw?.apply(this, arguments);
        syncFlowStageBeginWidgets(this);
        return result;
    };
...
if (!globalThis.__AD_MINIMAX_GUIDE_UI30_REGISTERED__) {
    globalThis.__AD_MINIMAX_GUIDE_UI30_REGISTERED__ = true;
    app.registerExtension({
        name: "ADMiniMaxGuide.ui30",
        setup() {
            install();
        },

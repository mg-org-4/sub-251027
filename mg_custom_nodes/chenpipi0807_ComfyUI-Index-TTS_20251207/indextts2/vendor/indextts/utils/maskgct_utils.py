...
def _load_config(config_fn, lowercase=False):
    ...
    with open(config_fn, "r", encoding="utf-8") as f:
        data = f.read()
    try:
        config_ = _json.loads(data)
    except Exception as e:
        raise RuntimeError(
            f"Failed to parse config {config_fn} using {_json_name}. "
            "If the file contains JSON5-specific syntax (comments, trailing commas), please install the 'json5' package."
        ) from e
...
def build_semantic_model(path_='./models/tts/maskgct/ckpt/wav2vec2bert_stats.pt'):
    semantic_model = Wav2Vec2BertModel.from_pretrained("./models/IndexTTS-2/w2v-bert-2.0")
    semantic_model.eval()

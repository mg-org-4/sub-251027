...
# [ComfyUI-Index-TTS patch] 不在导入时改写 HF_HUB_CACHE，避免污染 ComfyUI 工作目录
import json
import re
import time
import librosa
...
class IndexTTS2:
    def __init__(
        ...
        # [ComfyUI-Index-TTS patch] 优先使用模型根目录下的 w2v-bert-2.0（离线直读，与 TTS2 习惯一致）
        ...
        if os.path.isdir(local_w2v):
            print(f"[IndexTTS2.5] Using local w2v-bert-2.0 at {local_w2v}")
            w2v_bert_dir = local_w2v
        elif not os.path.isdir(w2v_bert_dir):
        ...
        # load campplus_model
        # [ComfyUI-Index-TTS patch] 优先使用模型根目录下的 campplus_cn_common.bin
        ...
        if os.path.isfile(local_campplus):
            print(f"[IndexTTS2.5] Using local campplus checkpoint: {local_campplus}")
            campplus_ckpt_path = local_campplus
        elif not os.path.isfile(campplus_ckpt_path):
            from indextts.utils.model_download import ensure_models_available
            aux_paths = ensure_models_available(self.model_dir)
        ...
        # [ComfyUI-Index-TTS patch] 优先使用模型根目录下的 bigvgan（离线直读）
        local_bigvgan = os.path.join(self.model_dir, "bigvgan")
        bigvgan_dir = os.path.join(self.model_dir, "hf_cache", "bigvgan")
        if os.path.isdir(local_bigvgan) and os.path.isfile(os.path.join(local_bigvgan, "bigvgan_generator.pt")):
            print(f"[IndexTTS2.5] Using local bigvgan at {local_bigvgan}")
            bigvgan_dir = local_bigvgan
        elif not os.path.isdir(bigvgan_dir):
            from indextts.utils.model_download import ensure_models_available
            aux_paths = ensure_models_available(self.model_dir)

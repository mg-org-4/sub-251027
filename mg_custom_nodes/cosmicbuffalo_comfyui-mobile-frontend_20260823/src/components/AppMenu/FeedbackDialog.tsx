...
export function FeedbackDialog({ systemStats, workflow, onClose }: FeedbackDialogProps) {
      ...
      <Dialog
        ...
        description={
          <div className="space-y-3">
            <p>{t('Your feedback was submitted as a GitHub issue.')}</p>
            <a
              href={submit.url}
              target="_blank"
        ...
        actions={[{ label: t('Close'), onClick: onClose, variant: 'primary' }]}
      />
    );
  }
  ...
  const fallbackLink = (
    ...
    >
      {t('open a GitHub issue directly')}
    </a>
  );
    ...
    return (
      <Dialog
        ...
        description={
          <div className="space-y-3">
            <p>
              {t("The in-app feedback service isn't configured for this build. You can still file an issue directly on GitHub.")}
            </p>
            <p>{fallbackLink}</p>
          </div>
        }
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-4">
        <div className="mx-auto w-full max-w-2xl space-y-3 pb-28">
          <p className="text-sm">
            {t('Report a bug or request a feature. Your feedback becomes a public GitHub issue.')}
          </p>

          <label className="block">
            <span className={FIELD_LABEL_CLASS}>{t('Title')}</span>
            <input
              type="text"
              value={title}
          ...
          <label className="block">
            <span className={FIELD_LABEL_CLASS}>{t('Description')}</span>
            <textarea
              value={body}
              onChange={(e) => setBody(e.target.value)}
              maxLength={8000}
              rows={5}
              placeholder={t("What happened? What were you trying to do? Steps to reproduce if it's a bug.")}
          ...
          <label className="block">
            <span className={FIELD_LABEL_CLASS}>
              {t('Contact')} <span className="text-slate-500 font-normal">({t('optional')})</span>
            </span>
            <input
              type="text"
              value={contact}
              ...
              maxLength={200}
              placeholder={t("GitHub @username or email if you'd like a reply")}
              className={INPUT_CLASS}
            />
            <span className={`block mt-1 text-xs ${menuMutedTextClassName}`}>
              {t('GitHub handles get @-mentioned in the public issue. Anything else (emails, phone numbers, etc.) is forwarded privately to the maintainer and never appears in the issue body.')}
            </span>
          </label>
          ...
          <label className="flex items-start gap-2 cursor-pointer select-none">
            ...
            <span className="text-sm text-slate-300">
              {t('Include system info to help with debugging (see preview when checked)')}
            </span>
          </label>
          {includeDiagnostics && (
...
function errorMessageFor(error: string, status?: number): string {
  switch (error) {
    case 'rate_limited':
      return globalT('Too many submissions in a short window. Please try again in a minute.');
    case 'invalid_fields':
      return globalT('The submission was rejected as invalid. Please review your inputs, or');
    case 'github_create_failed':
      return globalT("We couldn't create the issue right now (the feedback service is having trouble). Please");
    case 'timeout':
      return globalT('The request timed out. The feedback service may be slow — please try again, or');
    case 'network_error':
      return globalT("Couldn't reach the feedback service. Check your connection, or");
    default:
      return status
        ? globalT('Submission failed (HTTP {status}). Please', { status })
        : globalT('Submission failed. Please');
  }
}

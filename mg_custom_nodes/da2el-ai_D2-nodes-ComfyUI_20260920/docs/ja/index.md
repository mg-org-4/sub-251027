...
- `D2_CreateMasks`: キャンバス上の長方形をドラッグして複数のマスクを出力するノードを新規追加

**2026.09.15**

- `D2_TagReport`: タグ辞書を指定して集計から除外する `exclude_tags` を追加。辞書に無いタグだけが残るので、カンマ忘れや空白ミスなど誤って入力したプロンプトを炙り出せる

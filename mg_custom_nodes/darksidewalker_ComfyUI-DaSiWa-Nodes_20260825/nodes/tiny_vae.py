"""Verbatim copy of ComfyUI-KJNodes/nodes/tiny_vae.py (KJNodes, GPL-3.0).

Loader for TAESD-family tiny VAE decoders that ComfyUI core can't build yet
(core's Decoder hardcodes a 64-wide stack with 3 upsample stages; taeh3 needs
96-wide first half + 4 upsample stages for the 16x ratio, 24 latent channels).

Original body kept verbatim (see upstream repo for the MIT/GPL dual-licensed
KJNodes project); only the docstring header is replaced by this attribution and
the three `comfy.*` imports are hoisted into the functions that use them so the
module imports cleanly outside a running ComfyUI install (e.g. this repo's dev
test env). Runtime behavior inside ComfyUI is unchanged.
"""
...
# ComfyUI core (comfy.model_management / comfy.utils / comfy.taesd) is only on
# the path inside a running ComfyUI install. The loader functions below import
# them lazily so this module stays import-safe outside ComfyUI (e.g. the repo's
# dev test env) while running identically in production.

...
- **NATIVE ROUTING & LAZY LOADING:** hands validated data to ComfyUI's built-in MiniMaxH3ImageToVideo / MiniMaxH3ReferenceToVideo nodes (Image Inpaint uses the image-to-video node); only the selected FL2VA or REF2VA model is requested. REF2VA binds the native inputs by name, keeping the Guide compatible with future native input reordering (v0.4.36).

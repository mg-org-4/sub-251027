...
# TTS Audio Suite Patch: Transformers compatibility (some builds expose AUTO_TO_BASE_CLASS_MAPPING only).
...
class MossTTSDelayProcessor(ProcessorMixin):
    ...
    # TTS Audio Suite Patch: ProcessorMixin strict type check should accept concrete
    # audio tokenizer models returned by AutoModel.from_pretrained.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # TTS Audio Suite Patch: support divergent ProcessorMixin signatures and
        # strict class checks across transformers builds.
        try:
            super().__init__(tokenizer=tokenizer, audio_tokenizer=audio_tokenizer, **kwargs)
        except Exception:
            try:
                super().__init__(tokenizer=tokenizer, feature_extractor=audio_tokenizer, **kwargs)
            except Exception:
                # Some builds reject dynamic processor class metadata; we fall back
                # to direct attribute assignment below.
                pass

        # Explicit assignments for type-checkers; ProcessorMixin sets these too.
        self.tokenizer = tokenizer
        self.audio_tokenizer = audio_tokenizer
        self.feature_extractor = audio_tokenizer

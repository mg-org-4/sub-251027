...
try:
    ...
    # _crop_past_key_values doesn't exist in newer transformers versions
    from transformers.generation.candidate_generator import (
        AssistedCandidateGenerator,
        AssistedCandidateGeneratorDifferentTokenizers,
        CandidateGenerator,
    ...
    # Implement _crop_past_key_values for newer transformers
    def _crop_past_key_values(model, past_key_values, new_cache_size):
        """
        Crop past key values to new_cache_size.
        
        In transformers 4.56.1, cache objects have a crop() method.
        """
        if past_key_values is not None:
            if hasattr(past_key_values, 'crop'):
                # Cache object with crop method
                past_key_values.crop(new_cache_size)
                return past_key_values
            elif isinstance(past_key_values, (list, tuple)):
                # Tuple/list of tensors - crop each tensor
                cropped = []
                for layer_past in past_key_values:
                    if isinstance(layer_past, (list, tuple)) and len(layer_past) == 2:
                        # (key_states, value_states)
                        key_states, value_states = layer_past
                        if key_states.shape[-2] > new_cache_size:
                            cropped_key = key_states[..., :new_cache_size, :]
                            cropped_value = value_states[..., :new_cache_size, :]
                            cropped.append((cropped_key, cropped_value))
                        else:
                            cropped.append(layer_past)
                    else:
                        cropped.append(layer_past)
                return type(past_key_values)(cropped)
        return past_key_values
# Handle transformers version compatibility for cache constants
try:
    ...
    # These constants don't exist in newer transformers versions, define them
    from transformers.generation.configuration_utils import (
        GenerationConfig,
        GenerationMode,
    )
...
class GenerationMixin:
    ...
    def _get_candidate_generator(
        ...
        if hasattr(generation_config, 'prompt_lookup_num_tokens') and generation_config.prompt_lookup_num_tokens is not None:
            candidate_generator = PromptLookupCandidateGenerator(
                eos_token_id=getattr(generation_config, '_eos_token_tensor', None),
                num_output_tokens=generation_config.prompt_lookup_num_tokens,
                max_matching_ngram_size=getattr(generation_config, 'max_matching_ngram_size', 1),
    ...
    def _get_logits_processor(
        ...
        if hasattr(generation_config, 'guidance_scale') and generation_config.guidance_scale is not None and generation_config.guidance_scale != 1:
            processors.append(
                UnbatchedClassifierFreeGuidanceLogitsProcessor(

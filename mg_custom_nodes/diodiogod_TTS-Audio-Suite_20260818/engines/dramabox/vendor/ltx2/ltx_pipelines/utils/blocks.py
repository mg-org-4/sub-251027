...
class PromptEncoder:
    ...
    def __init__(
        ...
        self._defer_warm_load = defer_warm_load
        ...
        # Fast mode: build and keep models on GPU
        self._warm_text_encoder = None
        self._warm_embeddings_processor = None
        if warm and not defer_warm_load:
            if use_bnb_4bit:
    ...
    def _ensure_warm_models(self) -> None:
        """TTS Audio Suite patch: lazily build models for staged execution."""
        if self._warm_text_encoder is not None and self._warm_embeddings_processor is not None:
            return
        loaded = PromptEncoder(
            checkpoint_path=self._checkpoint_path,
            gemma_root=self._gemma_root,
            dtype=self._dtype,
            device=self._device,
            warm=True,
            use_bnb_4bit=self._use_bnb_4bit,
            audio_only=self._audio_only,
        )
        self._warm_text_encoder = loaded._warm_text_encoder
        self._warm_embeddings_processor = loaded._warm_embeddings_processor
        loaded._warm_text_encoder = None
        loaded._warm_embeddings_processor = None

    def release_warm_models(self) -> None:
        """Release staged Gemma and embeddings models after prompt encoding."""
        self._warm_text_encoder = None
        self._warm_embeddings_processor = None
        cleanup_memory()
    ...
    def _load_bnb_4bit_encoder(self, gemma_root: str):
        ...
        from_kwargs = {
            ...
            # TTS Audio Suite patch: model downloads must stay in the organized
            # ComfyUI model folder instead of silently falling back to HF cache.

...
class IndexTTS2:
    def __init__(
            ...
            use_gpt_latent=False, use_cuda_kernel=None, use_deepspeed=False, use_accel=False,
            use_torch_compile=False, use_qwen_emo=False, low_vram=False
    ):
        """
        Args:
        ...
        if self.use_bf16 and self.device.startswith("cuda") and not torch.cuda.is_bf16_supported():
            # TTS Audio Suite patch: Older CUDA GPUs cannot execute BF16 safely.
            print("⚠️ BF16 is unsupported on this CUDA device; falling back to float32")
            self.use_bf16 = False
        ...
        if use_qwen_emo:
            try:
                ...
                # TTS Audio Suite patch: Qwen emotion is optional and must not
                # prevent audio/vector-controlled speech from loading.
                self.qwen_emo = None
                print(f"⚠️ QwenEmotion failed to load ({exc}); text emotion disabled")
        else:
            self.qwen_emo = None
        ...
        # TTS Audio Suite patch: Resolve auxiliary models through the suite's
        # unified downloader and extra_model_paths layout, not upstream's cwd cache.
        from engines.index_tts.index_tts_downloader import index_tts_downloader
        w2v_bert_dir = index_tts_downloader.download_model("w2v-bert-2.0")
        ...
        if self.use_torch_compile:
            try:
                ...
                # TTS Audio Suite patch: Optional compilation must degrade cleanly.
                self.use_torch_compile = False
                print(f"⚠️ torch.compile unavailable ({exc}); using eager inference")
        ...
        bigvgan_dir = index_tts_downloader.download_model("bigvgan_v2_22khz_80band_256x")
    ...
    def _load_and_cut_audio(self,audio_path,max_audio_length_seconds,verbose=False,sr=None):
        # TTS Audio Suite patch: Avoid librosa/TorchCodec backend failures in
        # embedded ComfyUI environments while preserving mono resampling.
        if not sr:
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path, lang,
        ...
        # TTS Audio Suite patch: Keep reference-audio emotion active when vector
        # or text guidance is also supplied; the latent mixer supports both.

        if use_emo_text:
            # automatically generate emotion vectors from text prompt
            if self.qwen_emo is None:
                print("⚠️ QwenEmotion is unavailable; continuing with audio emotion")
                emo_vector = None
                use_emo_text = False
            else:
                if emo_text is None:
                ...
                try:
                    ...
                    # TTS Audio Suite patch: Text-emotion failure must not abort or
                    # poison otherwise valid speaker/audio-emotion generation.
                    print(f"⚠️ QwenEmotion failed ({exc}); continuing with audio emotion")
                    emo_vector = None
                    use_emo_text = False

        if emo_vector is not None:
            emo_vector = self.normalize_emo_vec(list(emo_vector))
            ...
            with torch.no_grad():
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    ...
                    codes, speech_conditioning_latent = self.gpt.inference_speech(
                        ...
                        # TTS Audio Suite patch: Honor the exposed deterministic/sampling control.
                        do_sample=do_sample,
                        top_p=top_p,
                        top_k=top_k,
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=dtype is not None, dtype=dtype):
                    ...
                    # Official IndexTTS-2.5 duration control. This resizes the
                    # internal semantic feature sequence; it does not re-plan
                    # phoneme timing or prosody for a requested duration.
                    target_lengths = torch.LongTensor([
                        int(S_infer.shape[1] * 1.72 * float(duration_factor))
                    ]).to(codes.device)

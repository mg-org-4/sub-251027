...
If you must include an edit LoRA in the stack, use `additive` optimization mode and disable sparsification to avoid weight trimming.
...
| `patch_compression` | aggressive | Compress everything, even TIES (slight quality loss) |
...
3. **Try enhanced quality** — `merge_refinement=refine` adds DO-orthogonalization and TALL-mask protection.
...
- Set `patch_compression=aggressive`
- Set `vram_budget=0.0`
...
- `merge_refinement=refine` with `della_conflict` sparsification is a strong default for quality
- `merge_refinement=full` adds KnOTS-inspired SVD alignment — strongest quality mode for critical merges
- `strategy_set=full` gives the optimizer the full algorithm repertoire

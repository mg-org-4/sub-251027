...
class Fixture:
    def setUp(self):
        ...
        self.final = self.run / "final/final.mp4"
    ...
    def save(self, scene, revision=None, scope="", **extra):
        ...
        for key, directory, suffix in (("checkpoint", "checkpoints", ".safetensors"),
            ...
            path = folder / directory / (stem + suffix)
            path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def exists(self, segment):
        return (self.output / segment["checkpoint"]).exists()
...
class CleanupTests(Fixture, unittest.TestCase):
    def test_deletes_only_checkpoint_payloads_preserves_all_other_bytes(self):
        ...
        untouched = {p: p.read_bytes() for p in self.run.rglob("*")
    ...
    def test_changed_and_hardlinked_checkpoints_kept(self):
        os.link(self.output / self.segments[0]["checkpoint"], self.output / "backup.safetensors")
        job = self.prepare()
        (self.output / self.segments[1]["checkpoint"]).write_bytes(b"changed")
    ...
    def test_symlinked_file_or_metadata_directory_kept(self):
        path = self.output / self.segments[0]["checkpoint"]
...
class AssembleTests(Fixture, unittest.TestCase):
    ...
    def setUp(self):
        ...
        for segment in self.segments:
            path = self.output / segment["segment"]

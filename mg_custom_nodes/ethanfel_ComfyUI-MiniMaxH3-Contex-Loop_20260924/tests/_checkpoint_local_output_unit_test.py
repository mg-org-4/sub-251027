...
def main():
    with tempfile.TemporaryDirectory() as temporary:
        ...
        # Workflow ownership is deliberately nightly-only. The stable port
        # still guarantees no changes to any project-wide active pointers.
        ...
        assert snapshot(root) == before, "local selection modified project files"
        ...
        # not adopt/copy them into this run as a side effect.
        ...
        assert snapshot(root) == changed
    print("Checkpoint local output: read-only, exact revisions, stable source hash, legacy cache and fail-closed checks pass")

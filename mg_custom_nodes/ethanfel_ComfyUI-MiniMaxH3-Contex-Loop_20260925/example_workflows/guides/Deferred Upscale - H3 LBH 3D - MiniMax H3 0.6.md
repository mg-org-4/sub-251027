...
Defaults: 2x spatial scale, align 32, FP16, 20 pass-2 steps, denoise 0.24, simple schedule, res_multistep. Raise denoise only after checking identity and scene boundaries. The Comfy Kitchen attention override remains bypassed because large H3 target sequences can exceed Sage prequantized int32 stride limits; this graph uses ComfyUI's PyTorch attention path. Functional nodes intentionally retain their original display names.

...
Save and Loop End both receive the final RAW images. They trim the repeated prefix once and retain each source checkpoint's delivered audio unchanged. Do not wire Pixel Current source_audio into recovered_audio (which expects RAW audio). Assemble recovers the whole-run source track from its saved timeline. The image reader's source_audio output is delivered-only and optional, intended for inspection/preview, not for a RAW-image mux. No audio VAE is required to preserve saved delivered sound.

...
async function browserChecks() {
    ...
    try {
        ...
        for (const type of ["MiniMaxH3ProjectAssetManager", "MiniMaxH3ProjectAssetTree",
            ...
            const ownership = node.root.querySelector(":scope > .h3pa-ownership");
            const status = node.root.querySelector(":scope > .h3pa-status");
            check(ownership.getBoundingClientRect().top >= toolbar.getBoundingClientRect().bottom,
                type + " ownership controls must not overlap the Run-name toolbar");
            check(status.getBoundingClientRect().top >= ownership.getBoundingClientRect().bottom,
                type + " project status must stay below ownership controls");
            if (tree) {
                check(getComputedStyle(toolbar).gridArea === "top", "Tree toolbar has its own grid area");
                check(getComputedStyle(ownership).gridArea === "ownership", "Tree ownership has its own grid area");

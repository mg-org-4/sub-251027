...
# Global instances for optional language-specific dependencies
_kakasi = None
_dicta = None
...
def hiragana_normalize(text: str) -> str:
    """Japanese text normalization: converts kanji to hiragana."""
    global _kakasi
...
def add_hebrew_diacritics(text: str) -> str:
    """Hebrew text normalization: adds diacritics."""
    global _dicta
...
def korean_normalize(text: str) -> str:
    """Korean text normalization: decompose syllables into Jamo."""
...
def add_russian_stress(text: str) -> str:
    """Russian text normalization: adds stress marks."""
...
class ChineseCangjieConverter:
    ...
    def _load_cangjie_mapping(self, model_dir=None):
        """Load Cangjie mapping from model directory."""
        try:
            if model_dir:
                cangjie_file = Path(model_dir) / "Cangjie5_TC.json"
                if cangjie_file.exists():
...
class MTLTokenizer:
    """Multilingual tokenizer supporting 23 languages."""
    ...
    def preprocess_text(self, raw_text: str, language_id: str = None, lowercase: bool = True, nfkd_normalize: bool = True):
        """Text preprocessor for lowercase and NFKD normalization."""

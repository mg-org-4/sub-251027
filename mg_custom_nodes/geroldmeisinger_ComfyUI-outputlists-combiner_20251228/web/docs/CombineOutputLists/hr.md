...
`unzip_a` .. `unzip_d` koriste `is_output_list=True` (označeno simbolom `𝌠`) i će biti obradjeni redom preko odgovarajućih nodova.

Sve liste su opcionalne i prazne liste će biti ignorirane.

Teknički izračunava *Kartezijev produkt* i izlazi svaku kombinaciju razdvojenu na pojedinačne elemente (`unzip`), dok prazne liste zamjenjuju jedinicama `None` i emitiraju `None` na odgovarajući izlaz.

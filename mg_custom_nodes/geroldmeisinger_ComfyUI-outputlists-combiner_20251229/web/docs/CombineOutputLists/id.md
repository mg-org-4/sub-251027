...
`unzip_a` .. `unzip_d` menggunakan `is_output_list=True` (ditandai dengan simbol `𝌠`) dan akan diproses secara berurutan oleh node yang sesuai.

Semua daftar opsional dan daftar kosong akan diabaikan.

Secara teknis, ini menghitung *produk Kartesius* dan menghasilkan setiap kombinasi yang dibagi menjadi elemennya (`unzip`), sementara daftar kosong akan diganti dengan nilai `None` dan akan menghasilkan `None` pada output yang sesuai.

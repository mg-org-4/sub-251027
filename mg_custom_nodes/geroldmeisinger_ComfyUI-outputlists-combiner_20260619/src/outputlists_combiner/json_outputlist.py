...
class JSONOutputList(io.ComfyNode):
	...
	def define_schema(cls) -> io.Schema:
		ret = io.Schema(
			description	= f"""Creates an OutputList by extracting arrays or dictionaries from JSON objects.
...
""",
			...
			inputs      	= [
				...
				io.String.Input("json",
					...
					tooltip     	= "A JSON string which is translated to an object."
				),
			...
			outputs	= [
				...
				io.Int   	.Output("int"  	, display_name="int"  	, is_output_list=True 	, tooltip=f"The value as a int (if it cannot parse the number, defaults to 0). {OUTPUTLIST_NOTE}"),
				io.Float 	.Output("float"	, display_name="float"	, is_output_list=True 	, tooltip=f"The value as a float (if it cannot parse the number, defaults to 0). {OUTPUTLIST_NOTE}"),

...
# TODO: For security reason only the following directories are supported: `[input] [output] [temp]`.
			...
			outputs	= [
				io.AnyType	.Output("filepath_annotated"	, display_name="filepath+"	, is_output_list=True	, tooltip="Full filepath (relative to a ComfyUI directory) including annotations. Recommended if you want to be specific and adhere to ComfyUI's path notation.\ne.g. `examples/animals/myfile.png [input]`"),
				io.String	.Output("filepath_rel"	, display_name="filepath"	, is_output_list=True	, tooltip="Full filepath (relative to a ComfyUI directory) without annotations. Recommended if you only load files from input directory anways.\ne.g. `examples/animals/myfile.png`"),
				io.String	.Output("filename"	, display_name="filename"	, is_output_list=True	, tooltip="Full filename\ne.g. `myfile.png`"),
				io.String	.Output("basename"	, display_name="basename"	, is_output_list=True	, tooltip="Basename part of the file without extension\ne.g. `myfile`"),
				io.String	.Output("ext"	, display_name="ext"	, is_output_list=True	, tooltip="Extension. Note that hidden-files (e.g. `.bashrc`) are considered files without a extension.\n`png` if `bare_strings=True` else `.png`"),
				io.String	.Output("dir_rel"	, display_name="full_dir"	, is_output_list=True	, tooltip="Full directory of the file (relative to a ComfyUI directory)\ne.g. `examples/animals` if `bare_strings=True` else `examples/animals/` (note the trailing slash)"),
				io.String	.Output("dir_parent"	, display_name="parent_dir"	, is_output_list=True	, tooltip="Immediate parent directory of the file\ne.g. `animals` or empty for empty parent if `bare_strings=True` else `./`"),
				io.String	.Output("annotation"	, display_name="annotation"	, is_output_list=True	, tooltip="Annotation to reference the ComfyUI user directory\n`input` if `bare_strings=True` else ` [input]` (note the leading whitespace)"),

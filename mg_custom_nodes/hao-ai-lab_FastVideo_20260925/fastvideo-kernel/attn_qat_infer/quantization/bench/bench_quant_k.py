...
def scale_and_fp4_tensor(x: torch.Tensor, packed_dim: int = 3, all_ones: bool = False, permuted: bool = False):
    ...
    dequant_x = (MXFP4Tensor(x_scaled).to(torch.float32) *
                 scales.to(torch.float8_e4m3fn).to(torch.float32)).flatten(start_dim=-2)
    ...
    if permuted:
        ...
        permuted_fp8_scale = scales.view(B, H // 64, 64, M, N // 64,
                                         4).permute(0, 1, 4, 3, 2, 5).reshape(B, H, M, N // 16).to(torch.float8_e4m3fn)
...
if n % 128 != 0:
    ...
    o_s_permuted_gt = torch.cat(
        [o_s, torch.zeros((b, h, n_padded - n, d // 16), dtype=torch.float8_e4m3fn, device='cuda')], dim=2)
else:
    o_permuted_gt = o
    o_s_permuted_gt = o_s

# use scale_and_fp4_tensor + torch permutation to get the ground truth
o_permuted_gt = o_permuted_gt.reshape(b, h, n_padded // 32, 32, d // 2)[:, :, :,
                                                                        k_permute, :].reshape(b, h, n_padded, d // 2)
o_s_permuted_gt = o_s_permuted_gt.reshape(b, h, n_padded // 32, 32,
                                          d // 16)[:, :, :, k_permute, :].reshape(b, h, n_padded, d // 16)

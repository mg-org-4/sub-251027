# ComfyUI - WAN 2.2 I2V/T2V - RTX 5090 Blackwell

> **[Create an Instance](https://cloud.vast.ai/?ref_id=188688&creator_id=188688&name=ComfyUI%20-%20WAN%202.2%20I2V%2FT2V%20-%20RTX%205090%20Blackwell)**

## What is this template?

This template gives you a **complete AI video generation environment** optimized for RTX 5090 Blackwell with ComfyUI running in a Docker container. Features QwenVL-Mod enhanced vision-language capabilities and professional WAN 2.2 video generation workflows with next-gen GPU acceleration.

**Think:** *"A visual programming environment for AI video generation with cutting-edge RTX 5090 performance - connect nodes to build any workflow you can imagine!"*
...
### **⚡ RTX 5090 Blackwell Optimized Performance**
- **SageAttention + FP16**: Proven optimal for WAN2.2 video generation
- **FP8 Compute Support**: Leverages RTX 5090's hardware FP8 acceleration
- **CUDA 12.8 Next-Gen**: Latest optimizations for Blackwell architecture
...
- Have an **RTX 5090 Blackwell** and want cutting-edge performance
- Want **FP8 acceleration** for next-gen model inference
- Need **multilingual support** with automatic language detection
- Want **visual style detection** for anime, 3D, pixel art styles
...
- Want **maximum performance** with RTX 5090 Blackwell optimization
...
Click **"[Rent](https://cloud.vast.ai/?ref_id=188688)"** when you've found a suitable RTX 5090 instance
...
| `COMFYUI_ARGS` | `--disable-auto-launch --port 18188 --enable-cors-header --fast fp16_accumulation --use-sage-attention --reserve-vram 2 --cuda-malloc --async-offload --supports-fp8-compute` | ComfyUI startup arguments optimized for RTX 5090 |
...
### **Why These Settings?**
- **SageAttention + FP16**: Proven optimal for WAN2.2 video generation
- **FP8 Support**: Leverages RTX 5090's hardware FP8 acceleration
- **Reserve VRAM 2GB**: Perfect balance for RTX 5090 + WAN2.2
- **Conda Isolation**: Prevents dependency conflicts with Python 3.12.12
- **CUDA 12.8**: Latest optimizations for Blackwell architecture

### **RTX 5090 Blackwell Advantages**
- **FP8 Compute**: Native hardware acceleration for FP8 models
- **Blackwell Architecture**: Next-gen GPU optimizations
- **Enhanced Memory Bandwidth**: Faster model loading and inference
- **Advanced Tensor Cores**: Optimized for AI workloads
...
| FP8 Models | 12 GB | 16 GB |
| Professional Workflows | 24 GB | 32 GB+ |
...
**Built with ❤️ for the ComfyUI community - Optimized for RTX 5090 Blackwell**

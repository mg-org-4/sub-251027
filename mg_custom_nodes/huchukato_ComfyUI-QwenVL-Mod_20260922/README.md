...
Open **Qwen Chat** from the ComfyUI sidebar to talk to a local Qwen model while working on the current graph. The assistant can inspect existing nodes, change validated widget values, enable or bypass nodes, and queue the workflow when explicitly requested.

- Select either the **GGUF** or **HF / Transformers** backend and one of the locally configured models.
- Ask for changes such as `Set the KSampler to 25 steps and queue the workflow`.
- Enable **Unload Qwen before running the workflow** to release chat model memory before heavy WAN, LTX, or MiniMax executions.
- Chat history and preferences are stored only in the browser's local storage and can be cleared with **New chat**.
- The first version cannot add, connect, or delete nodes and never executes arbitrary JavaScript, shell, filesystem, or network operations.
...
> **Refined interface**: Repeat-last-message control, busy spinner, improved message layout, and persistent English/Italian interface switch.
> **Models and memory**: Qwen3.8 9B Q8 GGUF is the default chat model, thinking receives a larger token budget, and the chat model unloads before workflow execution.
> **Diagnostics**: Reduced repeated cache lookup output while preserving useful model loading, image processing, generation, and failure logs.
...
> 🏷️ **Explicit Tag Handling**: MiniMax H3 system prompts now expand short explicit tags (e.g., `blowjob`, `deepthroat`) into full cinematic NSFW descriptions instead of refusing or asking for clarification.
...
* **Deaquay**: [Deaquay/ComfyUI-Qwen3.5-Uncensored](https://github.com/Deaquay/ComfyUI-Qwen3.5-Uncensored) - For automatic local model discovery (GGUF and HF auto-scan with extra_model_paths.yaml support), Qwen3.5 architecture detection, and the VRAM headroom and image-handling fixes ported in v2.4.

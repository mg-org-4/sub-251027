...
class JoyCaptionPredictorGGUF:
    ...
    def generate(self, image: Image.Image, system: str, prompt: str, max_new_tokens: int, temperature: float, top_p: float, top_k: int, caption_length: str = "medium-length") -> str:
        ...
        try:
            ...
            if any(tag_type in system.lower() for tag_type in ["booru", "danbooru", "e621", "rule34"]):
            ...
            if isinstance(caption, str):
                ...
                if "words or less" in prompt:
                    ...
                    try:
                        target_length = int(''.join(filter(str.isdigit, prompt.split("words or less")[0].split()[-2])))
                    except:
                        pass
...
class JoyCaptionGGUF:
    ...
    def generate(self, image, gguf_model, mmproj_file, n_gpu_layers, n_ctx, caption_type, caption_length, 
        ...
        response = self.predictor_gguf.generate(pil_image, system_prompt, prompt, max_new_tokens, temperature, top_p, top_k, caption_length)

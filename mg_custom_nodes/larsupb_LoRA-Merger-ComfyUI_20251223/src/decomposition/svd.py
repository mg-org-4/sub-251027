...
class QRDecomposer(TensorDecomposer):
    """
    QR-based decomposition.

    Faster alternative to SVD for low-rank approximation.
    Does not compute singular values, uses QR factorization instead.
    """
    ...
    def _decompose_2d(
        ...
        Note: This creates a synthetic S (all ones) since QR doesn't
        produce singular values. Suitable when singular values aren't needed.

        Args:
            weight_2d: 2D weight tensor

        Returns:
            Tuple of (Q, ones, R) mimicking SVD output format
        """
        try:
            Q, R = torch.linalg.qr(weight_2d)

            # Create synthetic singular values (all ones)
            rank = min(weight_2d.shape)
            S = torch.ones(rank, dtype=weight_2d.dtype, device=weight_2d.device)

            # R plays the role of Vh
            Vh = R

            return Q, S, Vh

        except RuntimeError as e:
            if "qr" in str(e).lower():
                raise RuntimeError(
                    f"QR decomposition failed for tensor shape {weight_2d.shape}. "

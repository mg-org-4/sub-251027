            ...
            "required": {
                "select_topk": ("FLOAT", {
                    ...
                    "tooltip": "Fraction of elements with the highest variance in the delta parameters to retain. "
                               "1.0 keeps everything (strongest); lower values merge more selectively (and weaker).",
                }),
                "average_weights": ("BOOLEAN", {
                    "default": False,
                    "tooltip": "OFF: additive SUM of the sign-agreeing, selected contributions, so per-LoRA "
                               "strengths act as gains and stacked LoRAs keep full magnitude (matches ComfyUI's "
                               "native LoRA stacking and the other merge nodes, the default). ON: SCE's normalized "
                               "weighted AVERAGE, so strengths act as ratios and the result is a blend/interpolation "
                               "(weaker magnitude). Turn ON only to blend LoRAs.",
                }),
            },
        }

    def get_settings(self, select_topk: float = 0.5, average_weights: bool = False):
        return {"select_topk": select_topk, "normalize": average_weights}
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "average_weights": ("BOOLEAN", {
                    "default": False,
                    "tooltip": "ON: divide by the per-element sum of contributing weights — a weighted "
                               "AVERAGE, so per-LoRA strengths act as ratios (two LoRAs at strength 1.0 "
                               "each land at ~50%). OFF: additive SUM, so strengths act as gains and "
                               "stacked LoRAs keep full magnitude (matches ComfyUI's native LoRA "
...
"""
    ...
    def get_method(self, rescale_norm: str = "default", average_weights: bool = False):
        method_def = {
...
class TIESMergeMethod:
    ...
    def get_method(self, rescale_norm:str = "default", average_weights: bool = False, density: float = 1):
        method_def = {
            "name": "ties",
            "settings": {
                "rescale_norm": rescale_norm,
...
class BreadcrumbsMergeMethod:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "sign_consensus": ("BOOLEAN", {
                    "default": False,
                    "tooltip": "TIES sign consensus. ON: elect one sign per weight element "
                               "(majority vote across the LoRAs) and keep only contributions that "
                               "agree with it — cancels conflicting edits between LoRAs. "
                               "OFF: plain weighted combine (the '_linear' variant, no sign vote).",
                }),
                "rescale_norm": (["default", "l1", "l2", "linf", "none"],
                                 {"default": "default", "tooltip": "Rescaling strategy:\n"
                                  "• default: Auto-select (L1 for methods needing it, none otherwise)\n"
    ...
    def get_method(self, sign_consensus: bool = False, rescale_norm: str = "default", density: float = .9, gamma: float = .1,
        ...
        method_def = {
            ...
            "settings": {
                ...
                "density": density,
                "gamma": gamma,
                "sign_consensus_algorithm": sign_consensus,
            }
        }
        return (method_def,)
...
class DAREMergeMethod:
    ...
    def get_method(self, sign_consensus: bool = False, rescale_norm: str = "default", density: float = .8,
                   average_weights: bool = False):
        method_def = {
            "name": "dare",
            "settings": {
...
class DELLAMergeMethod:
    ...
    def get_method(self, sign_consensus: bool = False, rescale_norm: str = "default", density: float = 1, epsilon: float = .01,
                   average_weights: bool = False):
        method_def = {
            "name": "della",
            "settings": {

...
def _load_json(path, default):
    if os.path.exists(path):
        try:
            ...
            print(f"[OreX LLM] Предупреждение при чтении JSON ({os.path.basename(path)}): {str(e)}")
            return default
    return default
...
class KontextPresetsOrex(object):
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                # --- Переключатели с поддержкой исходного регистра и красивыми маркерами ---
    ...
    def _apply_image_substitution(self, text: str, image_desc: str, enabled: bool) -> str:
        if not text:
            return ""
        if "*image*" in text:
            if enabled and image_desc and image_desc.strip():
    ...
    def build_prompt(self,
        ...
        if start_instruction and start_instruction.strip():
        ...
        if manual_prompt and manual_prompt.strip():
            ...
            if manual_cleaned.strip():
                parts.append(manual_cleaned.strip())
        ...
        if end_instruction and end_instruction.strip():
            parts.append(end_instruction.strip())

        # Чистая сборка с двойным переносом строк для лучшего понимания текста LLM моделями
        final = "\n\n".join(parts).strip()
...
# ========= Register (Убираем дублирование системных имён) =========

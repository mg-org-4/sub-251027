...
| Change Prompt Multi Pixaroma rows / queue-loop behavior / size | `js/prompt_multi/` (index.js for queuePrompt patch + extension wiring; core.mjs for state schema; render.mjs for DOM; interaction.mjs for events). Python in `nodes/node_prompt_multi.py`. State on `node.properties.promptMultiState` (`{version, rows: [{id, enabled, label, text}], activeIndex}`) + `app.graphToPrompt` hook injecting the active row's text into hidden `PromptMultiState`. **The new piece vs Prompt Stack**: `app.queuePrompt` patch in index.js — on every Run, finds the first Prompt Multi node (`findFirstPromptMultiNode`: top-level pass first, then recurses into subgraphs), reads enabled rows via `enabledRowsWithIndex`, loops `await _origQueuePrompt(num, 1)` once per enabled row, mutating `state.activeIndex` (ABSOLUTE row index, NOT enabled-filtered) before each call. 0 enabled → toast (`showNoEnabledToast`, modern `extensionManager.toast` first, brand-bordered `<div>` fallback) + bail without calling original; 1 enabled → 1 queue item; N enabled → N queue items each individually cancellable in the queue panel. Multiple PM nodes in one workflow → only first drives count, others use their own `activeIndex`. CSS prefix `.pix-pm-*` keeps it isolated from Prompt Stack's `.pix-ps-*`. Visual UX mirrors Prompt Stack exactly (drag handle, ON/OFF pill, label, textarea, ✕ delete with `pixConfirm`). Default fresh-node: 2 empty rows both ON, size `DEFAULT_W = 400` / `DEFAULT_H = 200`. NO separator setting (Prompt Multi #7 — would shift semantics toward Prompt Stack). |

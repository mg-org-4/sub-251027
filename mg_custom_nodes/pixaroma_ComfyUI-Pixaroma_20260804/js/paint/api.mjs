...
export const PaintAPI = {
  async saveComposite(projectId, dataURL) {
    const res = await api.fetchApi("/api/pixaroma/api/paint/save", {
      method: "POST",
  ...
  async uploadLayer(layerId, dataURL) {
    const res = await api.fetchApi("/api/pixaroma/api/layer/upload", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async removeBg(dataURL, model = "auto") {
    const res = await api.fetchApi("/api/pixaroma/remove_bg", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async removeBgInfo() {
    try {
      const res = await api.fetchApi("/api/pixaroma/remove_bg_info", {
        method: "GET",
      });
      return await res.json();
    } catch {

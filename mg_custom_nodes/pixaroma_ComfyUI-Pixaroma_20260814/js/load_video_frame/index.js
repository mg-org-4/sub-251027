...
async function fetchMeta(node) {
  ...
  try {
    const res = await fetch(
      pixApiUrl(`/pixaroma/api/load_video_frame/meta?video=${encodeURIComponent(val)}`)
    );
...
async function uploadVideo(node) {
  ...
  inp.onchange = async () => {
    ...
    try {
      ...
      const res = await fetch(pixApiUrl("/pixaroma/api/load_video/upload"), { method: "POST", body: fd });
      if (!res.ok) {

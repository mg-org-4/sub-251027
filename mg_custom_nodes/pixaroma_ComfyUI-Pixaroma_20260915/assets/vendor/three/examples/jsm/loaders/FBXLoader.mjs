/* three.js r184 examples/jsm/loaders/FBXLoader.js - MIT licensed, (c) three.js authors.
 * Taken from the readable source ComfyUI ships in comfyui_frontend_package 1.51.10.
 * Only the import paths were changed, to point at the pack's vendored three.mjs.
 * Plus a small, marked polygon counter (pixCountPolygons) for the quad readout, and
 * pixColorSpaceToWorking: r184 renamed ColorManagement.toWorkingColorSpace, and the
 * vendored three.mjs is r170. */
import {
	AmbientLight,
	AnimationClip,
	Bone,
...
import { unzlibSync } from '../libs/fflate.module.mjs';
...
class FBXLoader extends Loader {
	...
	parse( FBXBuffer, path ) {
		...
		const pixScene = new FBXTreeParser( textureLoader, this.manager ).parse( fbxTree );
		// Pixaroma addition: count the file's own polygon sizes BEFORE they are split
		// into triangles, so Load 3D Pixaroma can report quads.
		try {

			pixScene.userData.pixPolygonStats = pixCountPolygons( fbxTree );
...
class FBXTreeParser {
	...
	parseParameters( materialNode, textureMap, ID ) {
		...
		if ( materialNode.Diffuse ) {
			...
			parameters.color = pixColorSpaceToWorking( new Color().fromArray( materialNode.Diffuse.value ), SRGBColorSpace );

		} else if ( materialNode.DiffuseColor && ( materialNode.DiffuseColor.type === 'Color' || materialNode.DiffuseColor.type === 'ColorRGB' ) ) {

			// The blender exporter exports diffuse here instead of in materialNode.Diffuse
			parameters.color = pixColorSpaceToWorking( new Color().fromArray( materialNode.DiffuseColor.value ), SRGBColorSpace );
		...
		if ( materialNode.Emissive ) {
			...
			parameters.emissive = pixColorSpaceToWorking( new Color().fromArray( materialNode.Emissive.value ), SRGBColorSpace );

		} else if ( materialNode.EmissiveColor && ( materialNode.EmissiveColor.type === 'Color' || materialNode.EmissiveColor.type === 'ColorRGB' ) ) {

			// The blender exporter exports emissive color here instead of in materialNode.Emissive
			parameters.emissive = pixColorSpaceToWorking( new Color().fromArray( materialNode.EmissiveColor.value ), SRGBColorSpace );
		...
		if ( materialNode.Specular ) {
			...
			parameters.specular = pixColorSpaceToWorking( new Color().fromArray( materialNode.Specular.value ), SRGBColorSpace );

		} else if ( materialNode.SpecularColor && materialNode.SpecularColor.type === 'Color' ) {

			// The blender exporter exports specular color here instead of in materialNode.Specular
			parameters.specular = pixColorSpaceToWorking( new Color().fromArray( materialNode.SpecularColor.value ), SRGBColorSpace );
	...
	createLight( relationships ) {
		...
		if ( lightAttribute === undefined ) {
			...
			if ( lightAttribute.Color !== undefined ) {
				...
				color = pixColorSpaceToWorking( new Color().fromArray( lightAttribute.Color.value ), SRGBColorSpace );
...
class GeometryParser {
	...
	parseVertexColors( ColorNode ) {
		...
		for ( let i = 0, c = new Color(); i < buffer.length; i += 4 ) {
			...
			pixColorSpaceToWorking( c, SRGBColorSpace );
			c.toArray( buffer, i );
...
// Pixaroma addition (see parse above). PolygonVertexIndex marks the LAST corner of
// each polygon with a negative index, so the run length between negatives is the
// polygon size.
function pixCountPolygons( tree ) {

	const out = { tris: 0, quads: 0, ngons: 0, polys: 0 };
	const geos = tree && tree.Objects && tree.Objects.Geometry;
	if ( ! geos ) return out;
	for ( const id in geos ) {

		const pvi = geos[ id ] && geos[ id ].PolygonVertexIndex;
		const idx = pvi && pvi.a;
		if ( ! idx ) continue;
		let n = 0;
		for ( let i = 0; i < idx.length; i ++ ) {
			...
			if ( idx[ i ] < 0 ) {
				...
				out.polys ++;
				if ( n === 3 ) out.tris ++;
				else if ( n === 4 ) out.quads ++;
				else if ( n > 4 ) out.ngons ++;
				n = 0;
...
// Pixaroma addition: three r184 calls ColorManagement.colorSpaceToWorking, which r170 (the
// pack's vendored three.mjs) still names toWorkingColorSpace. Same arguments, same result.
function pixColorSpaceToWorking( color, colorSpace ) {

	if ( typeof ColorManagement.colorSpaceToWorking === 'function' ) return ColorManagement.colorSpaceToWorking( color, colorSpace );
	return ColorManagement.toWorkingColorSpace( color, colorSpace );

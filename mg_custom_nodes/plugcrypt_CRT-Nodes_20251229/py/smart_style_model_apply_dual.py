...
class StyleModelDualCache:
    ...
    def clear(self):
        ...
        if self.cache is not None:
            ...
            self.cache = None
            self.cache_key = None
            colored_print(f"   💾 Dual Style cache cleared", Colors.BLUE)
            return cleared
        return False
...
class SmartStyleModelApplyDual:
    ...
    def apply_dual_smart_stylemodel(self, conditioning, style_model, clip_vision, image_1, strength_1, strength_2, strength_type, crop, image_2=None):
        """
        ...
        if not isinstance(image_1, torch.Tensor):
            colored_print(f"   ❌ Invalid input: image_1 must be a torch.Tensor", Colors.RED)
            raise ValueError("image_1 must be a torch.Tensor")
        if image_2 is not None and not isinstance(image_2, torch.Tensor):
            colored_print(f"   ❌ Invalid input: image_2 must be a torch.Tensor", Colors.RED)
        ...
        if cached_cond is not None:
            colored_print(f"   💾 Using cached conditioning", Colors.GREEN)
            total_time = time.time() - start_time
            if dual_mode:
                colored_print(f"   🎉 Dual Smart Style Model completed in {total_time:.3f}s", Colors.GREEN)
            else:
        ...
        if dual_mode:
            ...
            final_conditioning = self._process_single_style(
                ...
                strength_2, strength_type, 2
            )
            style_2_time = time.time() - style_2_start
            colored_print(f"   ✅ Style 2 completed in {style_2_time:.3f}s", Colors.GREEN)
        
        # Cache the final result
        _style_dual_cache.put(style_model, image_1, image_2, strength_1, strength_2, strength_type, crop, final_conditioning)

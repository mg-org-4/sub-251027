...
class ClarityFX:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "ClarityOffset": ("FLOAT", {"default": 2.00, "min": 1.00, "max": 5.00, "step": 0.01, "display": "slider"}),
                "ClarityStrength": ("FLOAT", {"default": 0.400, "min": 0.00, "max": 2.00, "step": 0.01, "display": "slider"}),
                "ClarityBlendMode": (["Hard Light", "Soft Light", "Overlay", "Multiply", "Vivid Light", "Linear Light", "Addition"],),
                ...
                "ClarityDarkIntensity": ("FLOAT", {"default": 0.400, "min": 0.00, "max": 1.00, "step": 0.01, "display": "slider"}),
                "ClarityLightIntensity": ("FLOAT", {"default": 0.000, "min": 0.00, "max": 1.00, "step": 0.01, "display": "slider"}),
            }
        }
    ...
    def apply_clarity(self, image: torch.Tensor, ClarityRadius, ClarityOffset, ClarityStrength, ClarityBlendMode, ClarityBlendIfDark, ClarityBlendIfLight, ClarityDarkIntensity, ClarityLightIntensity):
        if image.is_cuda:
            image = image.cpu()
        ...
        for i in range(batch_size):
            ...
            luma = (original_rgb[..., 0] * 0.32786885 +
                    original_rgb[..., 1] * 0.655737705 +
                    original_rgb[..., 2] * 0.0163934436).unsqueeze(-1)
            ...
            if sigma > 0:
                ...
                kernel_h = torch.exp(-torch.pow(torch.arange(-radius, radius + 1, dtype=torch.float32), 2) / (2 * sigma**2))
                kernel_h /= kernel_h.sum()
                kernel_h = kernel_h.view(1, 1, 1, 2 * radius + 1)

                kernel_v = torch.exp(-torch.pow(torch.arange(-radius, radius + 1, dtype=torch.float32), 2) / (2 * sigma**2))
            ...
            if ClarityBlendMode == "Soft Light":
                sharp = torch.where(sharp < 0.5, 2 * luma * sharp + luma**2 * (1 - 2 * sharp), torch.sqrt(luma.clamp(min=0)) * (2 * sharp - 1) + 2 * luma * (1 - sharp))

    ...
    def get_frame_count(self, audio, fps, bypass, manual_frame_count, quantize_for_wan):
        ...
        if bypass:
            ...
            if not isinstance(waveform, torch.Tensor) or not isinstance(sample_rate, int):
                 print(f"Warning (Frame Count Node): Invalid audio data types. Returning 0 frames.")
                 return (0,)

            if waveform.numel() == 0 or sample_rate == 0:
                print(f"Warning (Frame Count Node): Empty audio data or zero sample rate. Returning 0 frames.")
                return (0,)

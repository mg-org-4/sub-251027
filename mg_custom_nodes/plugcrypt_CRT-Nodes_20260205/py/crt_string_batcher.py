...
class CRT_StringBatcher:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                # --- Batching Controls ---
                "consolidate_lines": ("BOOLEAN", {"default": True, "tooltip": "If True, removes line breaks within each individual string input"}),
                "join_with": (["Newline", "Space"], {"tooltip": "'Newline' for 'return to line' behavior between strings"}),
                "suffix": ("STRING", {"default": "", "multiline": False, "tooltip": "Character(s) to add to the end of each string before joining"}),
                ...
                # --- File Saving Options ---
                "save_to_file": ("BOOLEAN", {"default": False, "tooltip": "Enable to save each processed string as a .txt file"}),
                # This path is relative to the ComfyUI output directory.
                # The script correctly handles creating this subfolder.
                "file_path": ("STRING", {"default": "SavedPrompts", "tooltip": "The directory within the main output folder to save the .txt files to"}),
    ...
    def batch_strings(self, consolidate_lines, join_with, suffix, randomize, seed, save_to_file, file_path, filename_prefix, **kwargs):
        # Collect and sort all incoming string inputs to ensure correct order
        string_keys = sorted([key for key in kwargs if key.startswith('string_')], 

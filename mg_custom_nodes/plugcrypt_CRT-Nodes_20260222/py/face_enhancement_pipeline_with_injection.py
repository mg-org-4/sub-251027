...
def inference_segm(model, image: Image.Image, confidence: float = 0.3):
    ...
    for i in range(bboxes.shape[0]):
        ...
        scaled_mask = F_interpolate(
            mask.unsqueeze(0).unsqueeze(0), size=(image.size[1], image.size[0]), mode='bilinear', align_corners=False
        ).squeeze()
...
class GrowMaskWithBlur:
    def expand_mask(self, mask, expand, tapered_corners, blur_radius, taper_radius=0):
        colored_print(
            f"🎭 Processing mask (expand: {expand}, blur: {blur_radius}, taper: {taper_radius})...", Colors.CYAN
        )
        ...
        for m in mask.reshape((-1, mask.shape[-2], mask.shape[-1])).cpu():
            ...
            if expand != 0:
                colored_print(
                    f"   🔧 Applying {abs(expand)} iterations of {'dilation' if expand > 0 else 'erosion'}", Colors.BLUE
                )
...
class BoundedImageCropWithMask:
    def crop(self, image, mask, top, bottom, left, right):
        ...
        image, mask = (image.unsqueeze(0) if image.dim() == 3 else image), (
        ...
        for i in range(len(image)):
            ...
            if rmax < rmin or cmax < cmin:
                colored_print(
                    f"   ❌ Invalid crop region for image {i+1} (zero or negative size), skipping", Colors.RED
                )
                continue
...
class FaceEnhancementPipelineWithInjection:
            ...
            "required": {
                ...
                "initial_upscale_resolution": (
                    "INT",
                    {
                        ...
                        "max": 4096,
                        "step": 64,
                        "tooltip": "Resolution to upscale the entire image to before processing",
                    },
                ),
                "upscale_resolution": (
                    "INT",
                    {
                        ...
                        "max": 4096,
                        "step": 64,
                        "tooltip": "Resolution for face enhancement",
                    },
                ),
                "resize_back_to_original": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Whether to resize the final result back to original image dimensions",
                    },
                ),
                ...
                "enable_noise_injection": (
                    ["disable", "enable"],
                    {"default": "enable", "tooltip": "Enable noise injection during sampling"},
                ),
                "injection_point": (
                    "FLOAT",
                    {
                ...
                "injection_strength": (
                    "FLOAT",
                    {"default": 0.5, "min": -20.0, "max": 20.0, "step": 0.01, "tooltip": "Strength of injected noise"},
                ),
    ...
    def execute(
        ...
        colored_print("📈 Initial image processing...", Colors.HEADER)
        colored_print(f"   Target resolution: {initial_upscale_resolution}px", Colors.BLUE)
        resized_image = ImageResize().execute(
            image, initial_upscale_resolution, initial_upscale_resolution, method="keep proportion"
        )
        ...
        cropped_face_image, face_bounds = BoundedImageCropWithMask().crop(
            resized_image, face_bbox_mask, padding, padding, padding, padding
        )
        ...
        if enable_noise_injection == "enable":
            ...
            if first_stage_steps >= actual_steps:
                ...
                colored_print(
                    f"   💉 Injection Point: {injection_point:.2f} ({first_stage_steps}/{actual_steps} steps)",
                    Colors.CYAN,
                )
                colored_print(
        ...
        upscaled_face = ImageResize().execute(
            cropped_face_image, upscale_resolution, upscale_resolution, method='keep proportion'
        )
        ...
        cnet_positive, cnet_negative = apply_controlnet_advanced(
            positive, negative, control_net, upscaled_face, controlnet_strength, control_start, control_end, vae
        )
        ...
        if enhancement_mix < 1.0:
            colored_print("🎭 Blending original and enhanced face...", Colors.HEADER)
            colored_print(
                f"   Mix ratio: {(1.0-enhancement_mix)*100:.1f}% original + {enhancement_mix*100:.1f}% enhanced",
                Colors.BLUE,
            )

            if upscaled_face.shape != enhanced_face_image.shape:
                colored_print("   ⚠️ Dimension mismatch detected!", Colors.YELLOW)
                ...
                enhanced_face_image = ImageResize().execute(
                    enhanced_face_image, target_w, target_h, method="stretch", interpolation="lanczos"
                )
            ...
            colored_print("✅ Enhancement blending completed!", Colors.GREEN)
        else:
        ...
        if color_match_strength > 0:
            ...
            color_matched_face = ColorMatch().colormatch(
                upscaled_face, enhanced_face_image, method='mkl', strength=color_match_strength
            )[0]
        else:
        ...
        if resize_back_to_original:
            ...
            colored_print("   🎯 Keeping enhanced resolution", Colors.CYAN)
        ...
        colored_print("   📊 Output Details:", Colors.BLUE)
...
NODE_CLASS_MAPPINGS = {"FaceEnhancementPipelineWithInjection": FaceEnhancementPipelineWithInjection}

NODE_DISPLAY_NAME_MAPPINGS = {"FaceEnhancementPipelineWithInjection": "Face Enhancement Pipeline with Injection (CRT)"}

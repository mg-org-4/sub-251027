...
class ImageResize:
    def execute(self, image, width, height, method="stretch", interpolation="lanczos"):
        ...
        colored_print("✅ Image resize completed!", Colors.GREEN)
...
class FluxControlnetSamplerWithInjection:
    ...
    def execute(
        ...
        colored_print("📈 Upscaling Configuration:", Colors.HEADER)
        ...
        upscaled_latent_samples = F.interpolate(
            initial_latent_samples, size=(new_h, new_w), mode='bicubic', align_corners=False
        )
        ...
        resizer = ImageResize()
        control_image = resizer.execute(
            source_image, target_pixel_w, target_pixel_h, method="keep proportion", interpolation="lanczos"
        )
        control_start = 0.0
        cnet_positive = positive
        cnet_negative = null_negative

        colored_print("🎮 ControlNet Configuration:", Colors.HEADER)
        ...
        if controlnet_strength > 0:
            ...
            for conditioning_list, out_list in [(positive, cnet_positive), (null_negative, cnet_negative)]:
                for c in conditioning_list:
                    ...
                    c_net.set_cond_hint(
                        control_image.movedim(-1, 1), controlnet_strength, (control_start, control_end), vae=vae
                    )

                    if 'control' in new_c[1]:
                        colored_print("   🔗 Chaining with existing ControlNet", Colors.BLUE)
        ...
        if color_match_strength > 0:
            ...
            matcher = ColorMatch()
            final_image = matcher.colormatch(control_image, sampled_image, method='mkl', strength=color_match_strength)[
                0
            ]
        else:
        ...
        colored_print(
            f"   🎯 Process: {w_latent*8}x{h_latent*8} → {final_w}x{final_h} ({upscale_by:.2f}x)", Colors.GREEN
        )
        colored_print(
            f"   🎮 ControlNet: {'Applied' if controlnet_strength > 0 else 'Skipped'} (strength: {controlnet_strength:.3f})",
            Colors.GREEN,
        )
        colored_print(
        ...
        colored_print(
            f"   🎨 Color Match: {'Applied' if color_match_strength > 0 else 'Skipped'} (strength: {color_match_strength:.3f})",
            Colors.GREEN,
        )
...
NODE_CLASS_MAPPINGS = {"FluxControlnetSamplerWithInjection": FluxControlnetSamplerWithInjection}
NODE_DISPLAY_NAME_MAPPINGS = {"FluxControlnetSamplerWithInjection": "Flux Controlnet Sampler with Injection (CRT)"}

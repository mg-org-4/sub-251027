...
class LensDistortFX:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "chromatic_radius": (
                    "FLOAT",
                    {"default": -0.2, "min": -0.2, "max": 0.2, "step": 0.001, "display": "slider"},
                ),
                "decentering_x": (
                    "FLOAT",
                    {"default": 0.0, "min": -0.1, "max": 0.1, "step": 0.001, "display": "slider"},
                ),
                "decentering_y": (
                    "FLOAT",
                    {"default": 0.0, "min": -0.1, "max": 0.1, "step": 0.001, "display": "slider"},
                ...
                "center_offset_x": (
                    "FLOAT",
                    {"default": 0.0, "min": -0.05, "max": 0.05, "step": 0.001, "display": "slider"},
                ),
                "center_offset_y": (
                    "FLOAT",
                    {"default": 0.0, "min": -0.05, "max": 0.05, "step": 0.001, "display": "slider"},
    ...
    def apply_lens_distortion(
        ...
        k = torch.tensor(
            [radial_distortion_1, radial_distortion_2, radial_distortion_3, radial_distortion_4], device=device
        )
        ...
        for i in range(batch_size):
            ...
            if chromatic_radius != 0.0:
                ...
                sampled_colors = torch.nn.functional.grid_sample(
                    img_tiled, sample_coords, mode='bilinear', padding_mode='border', align_corners=False
                )
                ...
                final_color_bchw = torch.nn.functional.grid_sample(
                    img_tensor_bchw, distorted_grid, mode='bilinear', padding_mode='border', align_corners=False
                )

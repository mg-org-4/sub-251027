...
class LensFX:
    ...
    def apply_lens_effects(self, image: torch.Tensor, chromatic_aberration, vignette, grain_amount, grain_scale, seed):
        ...
        processed_image_bchw = image.permute(0, 3, 1, 2)
        ...
        if chromatic_aberration > 0:
            ...
            grid_r = (center_vec * r_scale.unsqueeze(-1)).unsqueeze(0).expand(batch_size, -1, -1, -1)
            grid_b = (center_vec * b_scale.unsqueeze(-1)).unsqueeze(0).expand(batch_size, -1, -1, -1)
        ...
        if vignette > 0:
            ...
            processed_image_bhwc *= vignette_mask.clamp(0.0, 1.0).view(1, height, width, 1)

        if grain_amount > 0:
            generator = torch.Generator(device=device)
            generator.manual_seed(_safe_seed(seed))

            grain_h = max(1, int(height / grain_scale)) if grain_scale > 0 else height
            grain_w = max(1, int(width / grain_scale)) if grain_scale > 0 else width

            grain = torch.rand(batch_size, grain_h, grain_w, 1, generator=generator, device=device, dtype=dtype) * 2.0 - 1.0
        ...
        return (processed_image_bhwc.clamp(0.0, 1.0),)

...
class SmartStyleModelApplyDual:
    ...
    def _encode_clip_vision(self, clip_vision, image, crop):
        ...
        try:
            ...
            colored_print(f"   [ERROR] CLIP Vision encoding failed: {str(e)}", Colors.RED)
            raise
    ...
    def _apply_style_conditioning(self, conditioning, cond, strength, strength_type, style_num):
        ...
        try:
            ...
            colored_print(f"   [ERROR] Style {style_num} conditioning failed: {str(e)}", Colors.RED)
            raise
    ...
    def _process_single_style(self, conditioning, style_model, clip_vision_output, strength, strength_type, style_num):
        ...
        try:
            ...
            colored_print(f"   [ERROR] Style {style_num} model application failed: {str(e)}", Colors.RED)
            raise
    ...
    def apply_dual_smart_stylemodel(
        ...
        if not isinstance(image_1, torch.Tensor):
            colored_print("   [ERROR] Invalid input: image_1 must be a torch.Tensor", Colors.RED)
            raise ValueError("image_1 must be a torch.Tensor")
        if image_2 is not None and not isinstance(image_2, torch.Tensor):
            colored_print("   [ERROR] Invalid input: image_2 must be a torch.Tensor", Colors.RED)
        ...
        if not dual_mode and strength_1 == 0.0:
            ...
            colored_print(f"   [OK] Single Smart Style Model completed in {total_time:.3f}s", Colors.GREEN)
            return (conditioning,)
        ...
        if dual_mode and strength_1 == 0.0 and strength_2 == 0.0:
            ...
            total_time = time.time() - start_time
            colored_print(f"   [OK] Dual Smart Style Model completed in {total_time:.3f}s", Colors.GREEN)
            return (conditioning,)
        ...
        colored_print("    Encoding Image 1 with CLIP Vision...", Colors.CYAN)
        ...
        colored_print(f"    Image 1 CLIP Vision encoding completed in {vision_time_1:.3f}s", Colors.BLUE)

        # Encode Image 2 only if provided
        clip_vision_output_2 = None
        vision_time_2 = 0
        if dual_mode:
            colored_print("    Encoding Image 2 with CLIP Vision...", Colors.CYAN)
            ...
            colored_print(f"    Image 2 CLIP Vision encoding completed in {vision_time_2:.3f}s", Colors.BLUE)
        ...
        colored_print(f"   [OK] Style 1 completed in {style_1_time:.3f}s", Colors.GREEN)
        ...
        if dual_mode:
            ...
            colored_print(f"   [OK] Style 2 completed in {style_2_time:.3f}s", Colors.GREEN)

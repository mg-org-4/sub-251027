...
import {
    extractErrorInfo,
    truncateError,
    updateRightErrorPanel,
} from "./doctor_right_panel.js";
...
export class DoctorUI {
    ...
    /**
     * Extract right-panel error summary and suggestion details.
     */
    extractErrorInfo(data) {
        return extractErrorInfo(data, { getUIText: this.getUIText.bind(this) });
    }

    /**
     * Truncate long error text for right-panel display.
     */
    truncateError(text, maxLength = 500) {
        return truncateError(text, { getUIText: this.getUIText.bind(this) }, maxLength);
    }
     ...
     */
    updateLogCard(data) {
        return updateRightErrorPanel({
            container: document.getElementById('doctor-latest-log'),
            data,
            lastErrorData: this.lastErrorData,
            getUIText: this.getUIText.bind(this),
            escapeHtml: this.escapeHtml.bind(this),
            hasNodeId: this.hasNodeId.bind(this),
            getPreferredNodeId: this.getPreferredNodeId.bind(this),
            locateNodeOnCanvas: this.locateNodeOnCanvas.bind(this),
        });
    }
}

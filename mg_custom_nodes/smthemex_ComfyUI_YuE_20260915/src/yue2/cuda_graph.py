...
def _probe_flash_attention(config, device, dtype, branches):
    """Return whether the build can actually run the packed variable-length op.

    A registered op is not enough: the Windows CUDA wheel raises "USE_FLASH_ATTENTION
    was not enabled for build" at dispatch, so probe the production shape once.
    """
    if not (hasattr(torch.ops.aten, "_flash_attention_forward") and config.head_dim <= 256 and
    ...
    query = torch.zeros((branches, heads, dim), device=device, dtype=dtype)
    cached = torch.zeros((branches * 2, heads, dim), device=device, dtype=dtype)
    cumulative = torch.arange(branches + 1, dtype=torch.int32, device=device)
    used = torch.ones(branches, dtype=torch.int32, device=device)
    try:
        with torch.inference_mode():
            torch.ops.aten._flash_attention_forward(query, cached, cached, cumulative, cumulative * 2,
                                                    1, 2, 0.0, False, False, seqused_k=used)
    except RuntimeError:
        return False
    return True


def _probe_cudnn_attention(config, device, dtype, branches):
    """Return whether cuDNN SDPA accepts this mask/GQA/dtype/head-dim combination.

    cuDNN 9 fuses only some masks, so probe the production call before selecting it;
    a late failure inside capture is harder to recover from than an eager probe.
    """
    from torch.nn.attention import SDPBackend, sdpa_kernel
    version = torch.backends.cudnn.version()
    if not torch.backends.cudnn.is_available() or not version or version < 90000:
        return False
    heads, dim, capacity = config.num_key_value_heads, config.head_dim, 2
    query = torch.zeros((branches, 1, config.num_attention_heads, dim), device=device, dtype=dtype)
    cached = torch.zeros((branches, capacity, heads, dim), device=device, dtype=dtype)
    visible = torch.ones((branches, 1, 1, capacity), device=device, dtype=torch.bool)
    try:
        with torch.inference_mode(), sdpa_kernel(SDPBackend.CUDNN_ATTENTION):
            F.scaled_dot_product_attention(query.transpose(1, 2), cached.transpose(1, 2),
                                           cached.transpose(1, 2), attn_mask=visible, is_causal=False,
                                           enable_gqa=config.num_attention_heads != heads)
    except RuntimeError:
        return False
    return True
...
class GraphAR:
    ...
    def __init__(self, model, prefixes, max_tokens, *, capture=True, attention_backend="auto", fuse_projections=False):
        ...
        # Both fused paths are probed, because registration and cuDNN 9 support
        # do not guarantee that the installed build can execute them.
        flash = fused and _probe_flash_attention(config, self.device, self.dtype, self.branches)
        cudnn = fused and _probe_cudnn_attention(config, self.device, self.dtype, self.branches)
        if attention_backend == "auto":
            attention_backend = "flash" if flash else "cudnn" if cudnn else "sdpa"
        if attention_backend == "flash" and not flash:
            raise ValueError("Packed variable-length CUDA FlashAttention is unavailable in this torch build")

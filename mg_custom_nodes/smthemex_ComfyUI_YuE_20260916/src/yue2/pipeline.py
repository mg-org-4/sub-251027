...
import numpy as np
import torch
from safetensors.torch import load_file as load_safetensors_normal
...
class YuE2Pipeline:
    def __init__(self, model_dir, vae_dir, *, device="auto", memory_budget_gib=24,
                 backend="torch", generation_config=None, verify_hashes=True,yue_ckpt=None,
    ...
    @classmethod
    def from_pretrained(cls, model="m-a-p/YuE2-3B", *, yue_ckpt=None, vae=None,
        ...
        result = cls(model, vae, yue_ckpt=yue_ckpt, progress=progress, **kwargs)
    ...
    def _load_model(self, for_nar=False):
        ...
        with self._status("Loading model") if loading else nullcontext():
            if self._model is None:
                from .modeling_yue2 import YuE2ForCausalLM,YuE2Config
                start = time.perf_counter()
                if  self.yue_ckpt is not None:
                    from accelerate import init_empty_weights
                    config=YuE2Config.from_pretrained(self.model_dir)
                    with init_empty_weights():
                        self._model = YuE2ForCausalLM(config)
                    sd = load_safetensors_normal(self.yue_ckpt)
                    self._model.load_state_dict(sd, strict=False, assign=True)
                    #match_state_dict(self._model, sd,show_num=10)
                    del sd
                    self._model.eval().to(torch.bfloat16)
                else:
                    try:
    ...
    def decode(self, latents, *, full=False, vae=None,model=None):
    ...
    def __call__(self, style=None, lyrics=None, *, tags=None, abc_sampling=None,
                 semantic_sampling=None, cancelled=None, on_token=None, vae_model=None,**kwargs):
        ...
        audio = self.decode(latents,model=vae_model)

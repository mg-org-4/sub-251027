...
def _sdpa(query, key, value, *, mask, is_causal, grouped, backend="sdpa"):
    """Fused SDPA with a cuDNN preference on CUDA.

    This build ships no flash-attn, and the CUTLASS memory-efficient kernel
    rejects grouped-query heads ("both fused kernels require query, key and
    value to have the same num_heads"), so the plain dispatch silently falls
    back to the math backend and materializes an fp32 [heads, query, key] score
    matrix: measured 7.3 GiB peak at 7123x7123 causal prefill and 14.1 GiB at
    7124x14247 for the ODE step, against 120/148 MiB for the same work on
    cuDNN.  Keep the default dispatch as a fallback for shapes cuDNN cannot
    serve, so a missing kernel degrades instead of raising.
    """
    if query.device.type == "cuda" and backend == "sdpa":
        from torch.nn.attention import SDPBackend, sdpa_kernel
        try:
            with sdpa_kernel(SDPBackend.CUDNN_ATTENTION):
                return F.scaled_dot_product_attention(
                    query, key, value, attn_mask=mask, is_causal=is_causal, enable_gqa=grouped)
        except RuntimeError as error:
            text = str(error)
            if "No available kernel" not in text and "not supported" not in text.lower():
                raise
    return F.scaled_dot_product_attention(
        query, key, value, attn_mask=mask, is_causal=is_causal, enable_gqa=grouped)
...
def attention(q, k, v, *, causal=False, backend="sdpa", query_chunk_size=None):
    ...
    with context:
        for start in range(0, len(q), block):
            ...
            outputs.append(_sdpa(
                query[..., start:end, :], used_key, used_value,
                mask=mask, is_causal=causal and start == 0, grouped=grouped, backend=backend,
            ))

...
def sdpa(query, key, value, *, attn_mask=None, is_causal=False):
    ...
    if query.device.type == "cuda":
        # Grouped-query heads rule out the CUTLASS memory-efficient kernel and
        # this build has no flash-attn, so the plain dispatch would pick math and
        # materialize an fp32 score matrix (measured 7.3 GiB for a 7123-token
        # eager prefill).  cuDNN does the same work in ~120 MiB.

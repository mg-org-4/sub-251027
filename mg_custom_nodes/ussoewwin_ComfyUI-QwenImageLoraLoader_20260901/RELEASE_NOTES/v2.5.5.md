...
# v2.5.5 Technical Documentation: Krea2 OpenPose ControlNet LoRA Support

> Baseline commit: `75f99be1614b18e22c5d58a7c24115d5218e488e` (v2.5.4)
> Code commits: `b7cbf11` / `042fe16` / `07ef7ec` — Docs commits: `999bac6` / `533495f` / `67f84e6` / `5797d06`
...
def _krea2_target_key(base):
    ...
    if base.startswith("txtfusion."):          # ← added
        return f"diffusion_model.{base}.weight"
    return None
```
...
Krea2ControlNetLoraLoader (unchanged)
        │  MODEL_PATCH (carries the LoRA safetensors state_dict)
        ▼
NunchakuQI&ZITDiffsynthControlnet.diffsynth_controlnet_nunchaku
        ...
        ▼  route == "krea2"
   _classify_krea2_control_type(state_dict)      ← presence of first.weight
        ├─ "depth"    → _apply_krea2_control()        (existing, unchanged)
        └─ "openpose" → _apply_krea2_openpose_control() (new)
                          ├─ add_patches(256 block LoRA keys)
                          ├─ extra_conds patch (reference_latents injection)
                          └─ diffusion_model.forward patch (kv_cache mode)
```

### 4.1 `_classify_krea2_control_type` — depth / openpose exclusive classification

Decides by the presence of a `first.weight`-style key in the LoRA state_dict.

- **depth**: has the expanded `first.weight` ((6144, 128)) → existing depth route
- **openpose**: no `first` key (pure block LoRA) → openpose route
...
### 4.2 `_krea2_fit_ref_image` — reference image preprocessing

Follows the ai-toolkit Krea2 reference-latent convention:

- Downscale **only** (never upscale) to at most **1MP** (1024×1024)
...
### 4.3 `_krea2_make_ref_cond_patch` — native ref injection (extra_conds patch)
...
- **Double-injection guard**: skipped when `kwargs["reference_latents"]` already exists
  ...
  → Works standalone, and is safe to combine with a VLM text encoder.

### 4.4 `_krea2_ref_pack` / `_krea2_patchify` — ref tokenization and RoPE positions

`_krea2_patchify` is the regular image tokenization `(B,C,H,W) → (B, L, C·p²)`.
...
```
rid[..., 0] = i        ← reference index (axis-0)
rid[..., 1] = y grid
rid[..., 2] = x grid
```
...
### 4.5 `_krea2_ref_attn_kv` — Attention with K/V capture / injection
...
Order: `wq/wk/wv/gate` → reshape → `qknorm` → `apply_rope` → capture/cache →
GQA expansion (`repeat_interleave`) → `optimized_attention_masked` → `wo(out·sigmoid(gate))`.

### 4.6 `_krea2_ref_precompute_kv` — t=0 reference-only pass
...
### 4.7 `_krea2_forward_with_cached_refs` — the denoising forward
...
- Text processed via `_unpack_context` → `txtfusion` → `txtmlp`
- RoPE positions: text at axis-0=0, image tokens on a y/x grid
- Output: `dit.last` → slice image tokens → patch unpatch → restore original size

### 4.8 `_krea2_ref_fingerprint` — content key for the cache
...
### 4.9 `_krea2_make_ref_kv_forward` — forward replacement state management

Replaces `diffusion_model.forward`:

1. No `ref_latents` → **delegate to the original forward** (identical to non-openpose behavior)
...
### 4.10 `_apply_krea2_openpose_control` — entry point from the node
...
2. **Ref injection**: `_krea2_fit_ref_image` → `vae.encode` → `_krea2_make_ref_cond_patch`
...
4. Missing vae/image → LoRA-only with a warning log (no crash even without ref injection)
...
- `_classify_controlnet_target` checks zimage → nunchaku → qwenimage_standard → krea2 in order.
...
- Live run log: `route=krea2` → `sub-route: openpose` → `Applied 256 block LoRA patches` →
  `control ref latent shape=(1, 16, 1, 128, 128)` → `kv_cache isolated-ref forward armed`
  → 12/12 steps completed (no errors).
